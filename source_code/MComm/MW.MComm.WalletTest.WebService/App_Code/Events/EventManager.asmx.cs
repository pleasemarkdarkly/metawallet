...
namespace MW.MComm.WalletTest.WebService.Events
{
	...
	public class EventManager : System.Web.Services.WebService
	{
		...
		///
		/// <param name="eventCode">A property of Event item to be managed</param>
		/// <param name="eventName">A property of Event item to be managed</param>
		/// <param name="eventTypeCode">A property of Event item to be managed</param>
		/// <param name="description">A property of Event item to be managed</param>
		/// <param name="isActive">A property of Event item to be managed</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Events.EventResults AddOneEvent(int eventCode, string eventName, int eventTypeCode, string description, bool isActive)
		{
			...
			try
			{
				...
				BLL.Events.EventManager.AddOneEvent(item, false);
		...
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Events.EventResults UpdateOneEvent(int eventCode, string eventName, int eventTypeCode, string description, bool isActive)
		{
			Components.Events.EventResults results = new Components.Events.EventResults();
			results.StatusDescription = "Results OK";
			try
			{
				...
				item.Description = description;
				item.IsActive = isActive;
				BLL.Events.EventManager.UpdateOneEvent(item, false);
				results.Results.Add(item);
			}
			catch (System.Exception ex)
			{
		...
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Events.EventResults DeleteOneEvent(int eventCode)
		{
			Components.Events.EventResults results = new Components.Events.EventResults();
			results.StatusDescription = "Results OK";
			try
			{
				...
				BLL.Events.Event item = new BLL.Events.Event();
				item.EventCode = eventCode;
				BLL.Events.EventManager.DeleteOneEvent(item, false);
			}
			catch (System.Exception ex)
			{
				results.StatusCode = -1;
		...
		public Components.Events.EventResults GetOneEvent(int eventCode, string sortColumn, string sortDirection)
		{
			...
			try
			{
				...
				isAccessed = item.SpecificEventAttributeList.IsDirty;
				isAccessed = item.NotificationList.IsDirty;
				isAccessed = item.EventFeeList.IsDirty;
				isAccessed = item.EventPromotionList.IsDirty;
				isAccessed = item.AuditLogList.IsDirty;
				results.Results.Add(item);
			}
			catch (System.Exception ex)
			{
