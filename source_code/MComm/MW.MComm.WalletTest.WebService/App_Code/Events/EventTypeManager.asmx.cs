...
namespace MW.MComm.WalletTest.WebService.Events
{
	...
	public class EventTypeManager : System.Web.Services.WebService
	{
		...
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Events.EventTypeResults AddOneEventType(int eventTypeCode, string eventTypeName, string description, bool isActive)
		{
			...
			try
			{
				...
				BLL.Events.EventType item = new BLL.Events.EventType();
				item.EventTypeCode = eventTypeCode;
				item.EventTypeName = eventTypeName;
				item.Description = description;
				item.IsActive = isActive;
				BLL.Events.EventTypeManager.AddOneEventType(item, false);
		...
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Events.EventTypeResults DeleteOneEventType(int eventTypeCode)
		{
			Components.Events.EventTypeResults results = new Components.Events.EventTypeResults();
			results.StatusDescription = "Results OK";
			try
			{
				...
				BLL.Events.EventType item = new BLL.Events.EventType();
				item.EventTypeCode = eventTypeCode;
				BLL.Events.EventTypeManager.DeleteOneEventType(item, false);
			}
			catch (System.Exception ex)
			{
				results.StatusCode = -1;
		...
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Events.EventTypeResults UpdateOneEventType(int eventTypeCode, string eventTypeName, string description, bool isActive)
		{
			Components.Events.EventTypeResults results = new Components.Events.EventTypeResults();
			results.StatusDescription = "Results OK";
			try
			{
				...
				item.Description = description;
				item.IsActive = isActive;
				BLL.Events.EventTypeManager.UpdateOneEventType(item, false);
				results.Results.Add(item);
			}
			catch (System.Exception ex)
			{
