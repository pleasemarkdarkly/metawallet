...
namespace MW.MComm.WalletTest.WebService.Accounts
{
	...
	public class CurrencyManager : System.Web.Services.WebService
	{
		...
		///
		/// <param name="currencyCode">A property of Currency item to be managed</param>
		/// <param name="currencyName">A property of Currency item to be managed</param>
		/// <param name="isPaymentCurrency">A property of Currency item to be managed</param>
		/// <param name="description">A property of Currency item to be managed</param>
		/// <param name="isActive">A property of Currency item to be managed</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Accounts.CurrencyResults AddOneCurrency(int currencyCode, string currencyName, bool isPaymentCurrency, string description, bool isActive)
		{
			...
			try
			{
				...
				item.IsPaymentCurrency = isPaymentCurrency;
				item.Description = description;
				item.IsActive = isActive;
				BLL.Accounts.CurrencyManager.AddOneCurrency(item, false);
		...
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Accounts.CurrencyResults DeleteOneCurrency(int currencyCode)
		{
			Components.Accounts.CurrencyResults results = new Components.Accounts.CurrencyResults();
			results.StatusDescription = "Results OK";
			try
			{
				...
				BLL.Accounts.Currency item = new BLL.Accounts.Currency();
				item.CurrencyCode = currencyCode;
				BLL.Accounts.CurrencyManager.DeleteOneCurrency(item, false);
			}
			catch (System.Exception ex)
			{
				results.StatusCode = -1;
		...
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Accounts.CurrencyResults UpdateOneCurrency(int currencyCode, string currencyName, bool isPaymentCurrency, string description, bool isActive)
		{
			Components.Accounts.CurrencyResults results = new Components.Accounts.CurrencyResults();
			results.StatusDescription = "Results OK";
			try
			{
				...
				item.Description = description;
				item.IsActive = isActive;
				BLL.Accounts.CurrencyManager.UpdateOneCurrency(item, false);
				results.Results.Add(item);
			}
			catch (System.Exception ex)
			{
