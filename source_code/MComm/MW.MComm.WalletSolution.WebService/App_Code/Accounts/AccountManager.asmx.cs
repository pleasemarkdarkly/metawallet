...
namespace MW.MComm.WalletSolution.WebService.Accounts
{
	...
	public class AccountManager : System.Web.Services.WebService
	{
		...
		/// <param name="performCascadeOperation">Flag, indicating if a cascade delete should be performed on the Account item.</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Accounts.AccountResults DeleteOneAccount(BLL.Accounts.Account item, bool performCascadeOperation)
		{
			Components.Accounts.AccountResults results = new Components.Accounts.AccountResults();
			results.StatusDescription = "Results OK";
			try
			{
				// perform main method tasks
				BLL.Accounts.AccountManager.DeleteOneAccount(item, performCascadeOperation);
			}
			catch (System.Exception ex)
			{
				results.StatusCode = -1;
		...
		///
		/// <param name="item">The Account to be managed.</param>
		/// <param name="performCascadeOperation">Flag, indicating if a cascade operation should be performed on the Account item.</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Accounts.AccountResults UpsertOneAccount(BLL.Accounts.Account item, bool performCascadeOperation)
		{
			Components.Accounts.AccountResults results = new Components.Accounts.AccountResults();
			results.StatusDescription = "Results OK";
			try
			{
				// perform main method tasks
				BLL.Accounts.AccountManager.UpsertOneAccount(item, performCascadeOperation);
				results.Results.Add(item);
			}
			catch (System.Exception ex)
			{
