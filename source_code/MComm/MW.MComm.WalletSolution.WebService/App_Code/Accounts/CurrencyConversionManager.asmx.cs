...
namespace MW.MComm.WalletSolution.WebService.Accounts
{
	...
	public class CurrencyConversionManager : System.Web.Services.WebService
	{
		...
		/// <param name="performCascadeOperation">Flag, indicating if a cascade delete should be performed on the CurrencyConversion item.</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Accounts.CurrencyConversionResults DeleteOneCurrencyConversion(BLL.Accounts.CurrencyConversion item, bool performCascadeOperation)
		{
			Components.Accounts.CurrencyConversionResults results = new Components.Accounts.CurrencyConversionResults();
			results.StatusDescription = "Results OK";
			try
			{
				// perform main method tasks
				BLL.Accounts.CurrencyConversionManager.DeleteOneCurrencyConversion(item, performCascadeOperation);
			}
			catch (System.Exception ex)
			{
				results.StatusCode = -1;
		...
		///
		/// <param name="item">The CurrencyConversion to be managed.</param>
		/// <param name="performCascadeOperation">Flag, indicating if a cascade operation should be performed on the CurrencyConversion item.</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Accounts.CurrencyConversionResults UpsertOneCurrencyConversion(BLL.Accounts.CurrencyConversion item, bool performCascadeOperation)
		{
			Components.Accounts.CurrencyConversionResults results = new Components.Accounts.CurrencyConversionResults();
			results.StatusDescription = "Results OK";
			try
			{
				// perform main method tasks
				BLL.Accounts.CurrencyConversionManager.UpsertOneCurrencyConversion(item, performCascadeOperation);
				results.Results.Add(item);
			}
			catch (System.Exception ex)
			{
