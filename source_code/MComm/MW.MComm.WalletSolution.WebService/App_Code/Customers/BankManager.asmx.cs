...
namespace MW.MComm.WalletSolution.WebService.Customers
{
	...
	public class BankManager : System.Web.Services.WebService
	{
		...
		///
		/// <param name="item">The Bank to be managed.</param>
		/// <param name="performCascadeOperation">Flag, indicating if a cascade operation should be performed on the Bank item.</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Customers.BankResults AddOneBank(BLL.Customers.Bank item, bool performCascadeOperation)
		{
			...
			try
			{
				...
				BLL.Customers.BankManager.AddOneBank(item, performCascadeOperation);
		...
		/// <param name="performCascadeOperation">Flag, indicating if a cascade delete should be performed on the Bank item.</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Customers.BankResults DeleteOneBank(BLL.Customers.Bank item, bool performCascadeOperation)
		{
			Components.Customers.BankResults results = new Components.Customers.BankResults();
			results.StatusDescription = "Results OK";
			try
			{
				// perform main method tasks
				BLL.Customers.BankManager.DeleteOneBank(item, performCascadeOperation);
			}
			catch (System.Exception ex)
			{
				results.StatusCode = -1;
		...
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Customers.BankResults UpdateOneBank(BLL.Customers.Bank item, bool performCascadeOperation)
		{
			Components.Customers.BankResults results = new Components.Customers.BankResults();
			results.StatusDescription = "Results OK";
			try
			{
				// perform main method tasks
				BLL.Customers.BankManager.UpdateOneBank(item, performCascadeOperation);
				results.Results.Add(item);
			}
			catch (System.Exception ex)
			{
