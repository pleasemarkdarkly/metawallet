...
namespace MW.MComm.WalletSolution.WebService.Customers
{
	...
	public class MetaPartnerManager : System.Web.Services.WebService
	{
		...
		/// <param name="performCascadeOperation">Flag, indicating if a cascade delete should be performed on the MetaPartner item.</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Customers.MetaPartnerResults DeleteOneMetaPartner(BLL.Customers.MetaPartner item, bool performCascadeOperation)
		{
			Components.Customers.MetaPartnerResults results = new Components.Customers.MetaPartnerResults();
			results.StatusDescription = "Results OK";
			try
			{
				// perform main method tasks
				BLL.Customers.MetaPartnerManager.DeleteOneMetaPartner(item, performCascadeOperation);
			}
			catch (System.Exception ex)
			{
				results.StatusCode = -1;
		...
		///
		/// <param name="item">The MetaPartner to be managed.</param>
		/// <param name="performCascadeOperation">Flag, indicating if a cascade operation should be performed on the MetaPartner item.</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Customers.MetaPartnerResults UpsertOneMetaPartner(BLL.Customers.MetaPartner item, bool performCascadeOperation)
		{
			Components.Customers.MetaPartnerResults results = new Components.Customers.MetaPartnerResults();
			results.StatusDescription = "Results OK";
			try
			{
				// perform main method tasks
				BLL.Customers.MetaPartnerManager.UpsertOneMetaPartner(item, performCascadeOperation);
				results.Results.Add(item);
			}
			catch (System.Exception ex)
			{
