...
namespace MW.MComm.WalletSolution.WebService.Promotions
{
	...
	public class MetaPartnerCouponManager : System.Web.Services.WebService
	{
		...
		/// <param name="performCascadeOperation">Flag, indicating if a cascade delete should be performed on the MetaPartnerCoupon item.</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Promotions.MetaPartnerCouponResults DeleteOneMetaPartnerCoupon(BLL.Promotions.MetaPartnerCoupon item, bool performCascadeOperation)
		{
			Components.Promotions.MetaPartnerCouponResults results = new Components.Promotions.MetaPartnerCouponResults();
			results.StatusDescription = "Results OK";
			try
			{
				// perform main method tasks
				BLL.Promotions.MetaPartnerCouponManager.DeleteOneMetaPartnerCoupon(item, performCascadeOperation);
			}
			catch (System.Exception ex)
			{
				results.StatusCode = -1;
		...
		///
		/// <param name="item">The MetaPartnerCoupon to be managed.</param>
		/// <param name="performCascadeOperation">Flag, indicating if a cascade operation should be performed on the MetaPartnerCoupon item.</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Promotions.MetaPartnerCouponResults UpsertOneMetaPartnerCoupon(BLL.Promotions.MetaPartnerCoupon item, bool performCascadeOperation)
		{
			Components.Promotions.MetaPartnerCouponResults results = new Components.Promotions.MetaPartnerCouponResults();
			results.StatusDescription = "Results OK";
			try
			{
				// perform main method tasks
				BLL.Promotions.MetaPartnerCouponManager.UpsertOneMetaPartnerCoupon(item, performCascadeOperation);
				results.Results.Add(item);
			}
			catch (System.Exception ex)
			{
