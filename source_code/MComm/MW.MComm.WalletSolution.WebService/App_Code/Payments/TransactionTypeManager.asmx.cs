...
namespace MW.MComm.WalletSolution.WebService.Payments
{
	...
	public class TransactionTypeManager : System.Web.Services.WebService
	{
		...
		///
		/// <param name="item">The TransactionType to be managed.</param>
		/// <param name="performCascadeOperation">Flag, indicating if a cascade operation should be performed on the TransactionType item.</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Payments.TransactionTypeResults AddOneTransactionType(BLL.Payments.TransactionType item, bool performCascadeOperation)
		{
			...
			try
			{
				...
				BLL.Payments.TransactionTypeManager.AddOneTransactionType(item, performCascadeOperation);
		...
		/// <param name="performCascadeOperation">Flag, indicating if a cascade delete should be performed on the TransactionType item.</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Payments.TransactionTypeResults DeleteOneTransactionType(BLL.Payments.TransactionType item, bool performCascadeOperation)
		{
			Components.Payments.TransactionTypeResults results = new Components.Payments.TransactionTypeResults();
			results.StatusDescription = "Results OK";
			try
			{
				// perform main method tasks
				BLL.Payments.TransactionTypeManager.DeleteOneTransactionType(item, performCascadeOperation);
			}
			catch (System.Exception ex)
			{
				results.StatusCode = -1;
		...
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Payments.TransactionTypeResults UpdateOneTransactionType(BLL.Payments.TransactionType item, bool performCascadeOperation)
		{
			Components.Payments.TransactionTypeResults results = new Components.Payments.TransactionTypeResults();
			results.StatusDescription = "Results OK";
			try
			{
				// perform main method tasks
				BLL.Payments.TransactionTypeManager.UpdateOneTransactionType(item, performCascadeOperation);
				results.Results.Add(item);
			}
			catch (System.Exception ex)
			{
