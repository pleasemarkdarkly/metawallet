...
namespace MW.MComm.WalletSolution.WebService.Payments
{
	...
	public class PaymentTransactionLogManager : System.Web.Services.WebService
	{
		...
		///
		/// <param name="item">The PaymentTransactionLog to be managed.</param>
		/// <param name="performCascadeOperation">Flag, indicating if a cascade operation should be performed on the PaymentTransactionLog item.</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Payments.PaymentTransactionLogResults LogOnePaymentTransactionLog(BLL.Payments.PaymentTransactionLog item, bool performCascadeOperation)
		{
			Components.Payments.PaymentTransactionLogResults results = new Components.Payments.PaymentTransactionLogResults();
			results.StatusDescription = "Results OK";
			try
			{
				// perform main method tasks
				BLL.Payments.PaymentTransactionLogManager.LogOnePaymentTransactionLog(item, performCascadeOperation);
				results.Results.Add(item);
			}
			catch (System.Exception ex)
			{
