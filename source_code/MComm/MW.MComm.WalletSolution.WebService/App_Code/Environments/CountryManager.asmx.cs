...
namespace MW.MComm.WalletSolution.WebService.Environments
{
	...
	public class CountryManager : System.Web.Services.WebService
	{
		...
		///
		/// <param name="item">The Country to be managed.</param>
		/// <param name="performCascadeOperation">Flag, indicating if a cascade operation should be performed on the Country item.</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Environments.CountryResults AddOneCountry(BLL.Environments.Country item, bool performCascadeOperation)
		{
			...
			try
			{
				...
				BLL.Environments.CountryManager.AddOneCountry(item, performCascadeOperation);
		...
		/// <param name="performCascadeOperation">Flag, indicating if a cascade delete should be performed on the Country item.</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Environments.CountryResults DeleteOneCountry(BLL.Environments.Country item, bool performCascadeOperation)
		{
			Components.Environments.CountryResults results = new Components.Environments.CountryResults();
			results.StatusDescription = "Results OK";
			try
			{
				// perform main method tasks
				BLL.Environments.CountryManager.DeleteOneCountry(item, performCascadeOperation);
			}
			catch (System.Exception ex)
			{
				results.StatusCode = -1;
		...
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Environments.CountryResults UpdateOneCountry(BLL.Environments.Country item, bool performCascadeOperation)
		{
			Components.Environments.CountryResults results = new Components.Environments.CountryResults();
			results.StatusDescription = "Results OK";
			try
			{
				// perform main method tasks
				BLL.Environments.CountryManager.UpdateOneCountry(item, performCascadeOperation);
				results.Results.Add(item);
			}
			catch (System.Exception ex)
			{
