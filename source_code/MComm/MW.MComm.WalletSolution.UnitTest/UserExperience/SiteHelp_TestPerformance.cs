...
namespace MW.MComm.WalletSolution.UnitTest.UserExperience
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.UserExperience"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.UserExperience.SiteHelp))]
    public partial class SiteHelp_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public SiteHelp_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for SiteHelp.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void UserExperience_SiteHelp_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** UserExperience_SiteHelp_PerformanceTest START ********************");
                ...
                TestHelper.ReportValueToConsole("UserExperience_SiteHelp_PerformanceTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                {
                    TestHelper.ReportValueToConsole("UserExperience_SiteHelp_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString((UnitTestPerformanceLevel)(i + 1)) + " time (in seconds)", perfTimeLevels[i]);
                }
                TestHelper.ReportValueToConsole("UserExperience_SiteHelp_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString(UnitTestPerformanceLevel.Unacceptable) + " time (in seconds) is beyond", perfTimeLevels[(((int)UnitTestPerformanceLevel.Unacceptable - 1) - 1)]);
                ...
                TestHelper.ReportValueToConsole("UserExperience_SiteHelp_PerformanceTest: page size", pageSize);
                TestHelper.ReportValueToConsole("UserExperience_SiteHelp_PerformanceTest: maxListSize", maxListSize);
                ...
                TestHelper.ReportValueToConsole("UserExperience_SiteHelp_PerformanceTest: GetManySiteHelpDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    long startTicks = 0;
                    long endTicks = 0;
                    double secondsInterval = 0.00;
                    BLL.SortableList<BLL.UserExperience.SiteHelp> items = null;

                    TestHelper.ReportValueToConsole("UserExperience_SiteHelp_PerformanceTest: Loop Iteration", i);
                    ...
                    TestHelper.ReportValueToConsole("UserExperience_SiteHelp_PerformanceTest: GetManySiteHelpDataByCriteria acceptable search time (in seconds)", acceptableTime);
                    try
                    {
                        // get many
                        Results.ResetCurrentPerfMetric();
                        Results.SetPerfMetricName("UserExperience.SiteHelp.GetMany." + i.ToString());
                        items = SiteHelp_Manager.GetManySiteHelpDataByCriteriaTest(dbOptions, Results);
                        TestHelper.ReportValueToConsole("UserExperience_SiteHelp_PerformanceTest: GetManySiteHelpDataByCriteria succeeded for SiteHelp. The count", items.Count.ToString());
                        ...
                        perfStr = "GetManySiteHelpDataByCriteria search time (in seconds) for SiteHelp count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                    ...
                    if (items != null)
                    {
                        TestHelper.ReportValueToConsole("UserExperience_SiteHelp_PerformanceTest: GetManySiteHelpDataByCriteria acceptable serialization time (in seconds)", acceptableTime);
                        try
                        {
                            // serialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("UserExperience.SiteHelp.Serialize." + i.ToString());
                            ...
                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManySiteHelpDataByCriteria serialization time (in seconds) for SiteHelp count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                        ...
                        TestHelper.ReportValueToConsole("UserExperience_SiteHelp_PerformanceTest: GetManySiteHelpDataByCriteria acceptable deserialization time (in seconds)", acceptableTime);
                        try
                        {
                            // deserialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("UserExperience.SiteHelp.Deserialize." + i.ToString());
                            ...
                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManySiteHelpDataByCriteria deserialization time (in seconds) for SiteHelp count " + itemCount + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                ...
                if (numGetDataSearches > 0)
                {
                    ...
                    TestHelper.ReportValueToConsole("UserExperience_SiteHelp_PerformanceTest: GetManySiteHelpDataByCriteria AVERAGE SEARCH TIME (in seconds)", avgGetDataSearchSecs);
                }
                else
                {
                    Console.WriteLine("UserExperience_SiteHelp_PerformanceTest: GetManySiteHelpDataByCriteria -- Could not perform search. NO AVERAGE SEARCH TIME AVAILABLE.");
                }
                if (numSerializations > 0)
                {
                    avgSerializeDataSecs /= numSerializations;
                    TestHelper.ReportValueToConsole("UserExperience_SiteHelp_PerformanceTest: GetManySiteHelpDataByCriteria AVERAGE SERIALIZATION TIME (in seconds)", avgSerializeDataSecs);
                }
                else
                {
                    Console.WriteLine("UserExperience_SiteHelp_PerformanceTest: GetManySiteHelpDataByCriteria -- Could not serialize. NO AVERAGE SERIALIZATION TIME AVAILABLE.");
                }
                if (numDeserializations > 0)
                {
                    avgDeserializeDataSecs /= numDeserializations;
                    TestHelper.ReportValueToConsole("UserExperience_SiteHelp_PerformanceTest: GetManySiteHelpDataByCriteria AVERAGE DESERIALIZATION TIME (in seconds)", avgDeserializeDataSecs);
                }
                else
                {
                    Console.WriteLine("UserExperience_SiteHelp_PerformanceTest: GetManySiteHelpDataByCriteria -- Could not deserialize. NO AVERAGE DESERIALIZATION TIME AVAILABLE.");
                }
                ...
                Console.WriteLine("******************** UserExperience_SiteHelp_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== UserExperience_SiteHelp_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
