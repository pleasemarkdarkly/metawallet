...
namespace MW.MComm.WalletSolution.UnitTest.UserExperience
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.UserExperience"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.UserExperience.SiteHelp))]
    public partial class SiteHelp_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public SiteHelp_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for SiteHelp.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void UserExperience_SiteHelp_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** UserExperience_SiteHelp_BasicTest START ********************");
                ...
                BLL.UserExperience.SiteHelp item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("UserExperience_SiteHelp_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("UserExperience_SiteHelp_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("UserExperience_SiteHelp_BasicTest: Add SiteHelp count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = SiteHelp_Manager.AddOneSiteHelpTest(true, Results);
                        ...
                        else
                        {
                            ...
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("UserExperience_SiteHelp_BasicTest: Add succeeded for SiteHelp", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.UserExperience.SiteHelp)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for SiteHelp", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for SiteHelp", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if SiteHelp Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("UserExperience_SiteHelp_BasicTest: Update SiteHelp count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = SiteHelp_Manager.UpdateOneSiteHelpTest((int)loopID["SiteHelpDefinitionCode"], (int)loopID["LocaleCode"], true, Results);
                        TestHelper.ReportValueToConsole("UserExperience_SiteHelp_BasicTest: Update succeeded for SiteHelp", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for SiteHelp", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("UserExperience_SiteHelp_BasicTest: Delete SiteHelp count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = SiteHelp_Manager.DeleteOneSiteHelpTest((int)loopID["SiteHelpDefinitionCode"], (int)loopID["LocaleCode"], true, Results);
                            TestHelper.ReportValueToConsole("UserExperience_SiteHelp_BasicTest: Delete succeeded for SiteHelp", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for SiteHelp", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** UserExperience_SiteHelp_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== UserExperience_SiteHelp_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
