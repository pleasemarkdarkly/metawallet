...
namespace MW.MComm.WalletSolution.UnitTest.UserExperience
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.UserExperience"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.UserExperience.SiteLabel))]
    public partial class SiteLabel_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public SiteLabel_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for SiteLabel.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void UserExperience_SiteLabel_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** UserExperience_SiteLabel_BasicTest START ********************");
                ...
                BLL.UserExperience.SiteLabel item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("UserExperience_SiteLabel_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("UserExperience_SiteLabel_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("UserExperience_SiteLabel_BasicTest: Add SiteLabel count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = SiteLabel_Manager.AddOneSiteLabelTest(true, Results);
                        ...
                        else
                        {
                            ...
                            entityID["LocaleCode"] = item.LocaleCode;
                            entityID["SiteLabelDefinitionCode"] = item.SiteLabelDefinitionCode;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("UserExperience_SiteLabel_BasicTest: Add succeeded for SiteLabel", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.UserExperience.SiteLabel)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for SiteLabel", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for SiteLabel", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if SiteLabel Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("UserExperience_SiteLabel_BasicTest: Update SiteLabel count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = SiteLabel_Manager.UpdateOneSiteLabelTest((int)loopID["SiteLabelDefinitionCode"], (int)loopID["LocaleCode"], true, Results);
                        TestHelper.ReportValueToConsole("UserExperience_SiteLabel_BasicTest: Update succeeded for SiteLabel", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for SiteLabel", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("UserExperience_SiteLabel_BasicTest: Delete SiteLabel count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = SiteLabel_Manager.DeleteOneSiteLabelTest((int)loopID["SiteLabelDefinitionCode"], (int)loopID["LocaleCode"], true, Results);
                            TestHelper.ReportValueToConsole("UserExperience_SiteLabel_BasicTest: Delete succeeded for SiteLabel", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for SiteLabel", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** UserExperience_SiteLabel_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== UserExperience_SiteLabel_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
