...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for EventType instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Events"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Events.EventType))]
    public partial class EventType_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public EventType_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for EventType.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Events_EventType_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Events_EventType_BasicTest START ********************");
                ...
                BLL.Events.EventType item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Events_EventType_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Events_EventType_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Events_EventType_BasicTest: Add EventType count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = EventType_Manager.AddOneEventTypeTest(true, Results);
                        if (Results.IsCollision == true)
                        {
                            throw new UnitTestException(UnitTestErrorType.Warning, "Add collision encountered for EventType: " + item.PrimaryKey.ToString(), null);
                        }
                        else
                        {
                            MOD.Data.NamedObjectCollection entityID = new MOD.Data.NamedObjectCollection();
                            entityID["EventTypeCode"] = item.EventTypeCode;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Events_EventType_BasicTest: Add succeeded for EventType", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Events.EventType)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for EventType", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for EventType", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if EventType Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Events_EventType_BasicTest: Update EventType count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = EventType_Manager.UpdateOneEventTypeTest((int)loopID["EventTypeCode"], true, Results);
                        TestHelper.ReportValueToConsole("Events_EventType_BasicTest: Update succeeded for EventType", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for EventType", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Events_EventType_BasicTest: Delete EventType count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = EventType_Manager.DeleteOneEventTypeTest((int)loopID["EventTypeCode"], true, Results);
                            TestHelper.ReportValueToConsole("Events_EventType_BasicTest: Delete succeeded for EventType", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for EventType", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Events_EventType_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Events_EventType_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
