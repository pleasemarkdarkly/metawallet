...
namespace MW.MComm.WalletSolution.UnitTest.Events
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Events"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Events.EventAttribute))]
    public partial class EventAttribute_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public EventAttribute_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for EventAttribute.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Events_EventAttribute_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Events_EventAttribute_BasicTest START ********************");
                ...
                BLL.Events.EventAttribute item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Events_EventAttribute_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Events_EventAttribute_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Events_EventAttribute_BasicTest: Add EventAttribute count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = EventAttribute_Manager.AddOneEventAttributeTest(true, Results);
                        ...
                        else
                        {
                            ...
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Events_EventAttribute_BasicTest: Add succeeded for EventAttribute", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Events.EventAttribute)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for EventAttribute", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for EventAttribute", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if EventAttribute Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Events_EventAttribute_BasicTest: Update EventAttribute count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = EventAttribute_Manager.UpdateOneEventAttributeTest((Guid)loopID["BaseAttributeID"], true, Results);
                        TestHelper.ReportValueToConsole("Events_EventAttribute_BasicTest: Update succeeded for EventAttribute", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for EventAttribute", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Events_EventAttribute_BasicTest: Delete EventAttribute count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = EventAttribute_Manager.DeleteOneEventAttributeTest((Guid)loopID["BaseAttributeID"], true, Results);
                            TestHelper.ReportValueToConsole("Events_EventAttribute_BasicTest: Delete succeeded for EventAttribute", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for EventAttribute", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Events_EventAttribute_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Events_EventAttribute_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
