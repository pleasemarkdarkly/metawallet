...
namespace MW.MComm.WalletSolution.UnitTest.Events
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Search.Events"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Events.Event))]
    public partial class Event_TestSearch : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the search entity test for Event.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Events_Event_SearchTest()
        {
            try
            {
                Console.WriteLine("******************** Events_Event_SearchTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Events_Event_SearchTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Events_Event_SearchTest: GetAllEventData test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Events.Event> items = Event_Manager.GetAllEventDataTest(Results);
                        TestHelper.ReportValueToConsole("Events_Event_SearchTest: GetAllEventData succeeded for Event. The count", items.Count.ToString());
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllEventData failed for Event", itemStr, ex, strExtraInfo);
                        }
                    }
                }
                loadCount = TestHelper.GetInt(Globals.MinimumGetAllCountPerEntity, Globals.MaximumGetAllCountPerEntity);
                TestHelper.ReportValueToConsole("Events_Event_SearchTest: GetManyEventDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Events.Event> items = Event_Manager.GetManyEventDataByCriteriaTest(Globals.getDataOptions("","Random",1,200,2000), Results);
                        TestHelper.ReportValueToConsole("Events_Event_SearchTest: GetManyEventDataByCriteria succeeded for Event. The count", items.Count.ToString());
                ...
                Console.WriteLine("******************** Events_Event_SearchTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Events_Event_SearchTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
