...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for AuditLog instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Events"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Events.AuditLog))]
    public partial class AuditLog_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public AuditLog_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for AuditLog.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Events_AuditLog_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Events_AuditLog_PerformanceTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Events_AuditLog_PerformanceTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                {
                    TestHelper.ReportValueToConsole("Events_AuditLog_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString((UnitTestPerformanceLevel)(i + 1)) + " time (in seconds)", perfTimeLevels[i]);
                }
                TestHelper.ReportValueToConsole("Events_AuditLog_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString(UnitTestPerformanceLevel.Unacceptable) + " time (in seconds) is beyond", perfTimeLevels[(((int)UnitTestPerformanceLevel.Unacceptable - 1) - 1)]);
                ...
                TestHelper.ReportValueToConsole("Events_AuditLog_PerformanceTest: page size", pageSize);
                TestHelper.ReportValueToConsole("Events_AuditLog_PerformanceTest: maxListSize", maxListSize);
                ...
                TestHelper.ReportValueToConsole("Events_AuditLog_PerformanceTest: GetManyAuditLogDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    long startTicks = 0;
                    long endTicks = 0;
                    double secondsInterval = 0.00;
                    BLL.SortableList<BLL.Events.AuditLog> items = null;

                    TestHelper.ReportValueToConsole("Events_AuditLog_PerformanceTest: Loop Iteration", i);
                    ...
                    TestHelper.ReportValueToConsole("Events_AuditLog_PerformanceTest: GetManyAuditLogDataByCriteria acceptable search time (in seconds)", acceptableTime);
                    try
                    {
                        // get many
                        Results.ResetCurrentPerfMetric();
                        Results.SetPerfMetricName("Events.AuditLog.GetMany." + i.ToString());
                        items = AuditLog_Manager.GetManyAuditLogDataByCriteriaTest(dbOptions, Results);
                        TestHelper.ReportValueToConsole("Events_AuditLog_PerformanceTest: GetManyAuditLogDataByCriteria succeeded for AuditLog. The count", items.Count.ToString());
                        ...
                        perfStr = "GetManyAuditLogDataByCriteria search time (in seconds) for AuditLog count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional,"GetManyAuditLogDataByCriteria failed for AuditLog", itemStr, ex, strExtraInfo);
                        }
                    }

                    if (items != null)
                    {
                        TestHelper.ReportValueToConsole("Events_AuditLog_PerformanceTest: GetManyAuditLogDataByCriteria acceptable serialization time (in seconds)", acceptableTime);
                        try
                        {
                            // serialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Events.AuditLog.Serialize." + i.ToString());
                            ...
                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyAuditLogDataByCriteria serialization time (in seconds) for AuditLog count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                        ...
                        TestHelper.ReportValueToConsole("Events_AuditLog_PerformanceTest: GetManyAuditLogDataByCriteria acceptable deserialization time (in seconds)", acceptableTime);
                        try
                        {
                            // deserialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Events.AuditLog.Deserialize." + i.ToString());
                            ...
                            items = (BLL.SortableList<BLL.Events.AuditLog>)oItems;
                            int itemCount = items.Count;
                            oItems = null;
                            items = null;

                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyAuditLogDataByCriteria deserialization time (in seconds) for AuditLog count " + itemCount + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                ...
                if (numGetDataSearches > 0)
                {
                    ...
                    TestHelper.ReportValueToConsole("Events_AuditLog_PerformanceTest: GetManyAuditLogDataByCriteria AVERAGE SEARCH TIME (in seconds)", avgGetDataSearchSecs);
                }
                else
                {
                    Console.WriteLine("Events_AuditLog_PerformanceTest: GetManyAuditLogDataByCriteria -- Could not perform search. NO AVERAGE SEARCH TIME AVAILABLE.");
                }
                if (numSerializations > 0)
                {
                    avgSerializeDataSecs /= numSerializations;
                    TestHelper.ReportValueToConsole("Events_AuditLog_PerformanceTest: GetManyAuditLogDataByCriteria AVERAGE SERIALIZATION TIME (in seconds)", avgSerializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Events_AuditLog_PerformanceTest: GetManyAuditLogDataByCriteria -- Could not serialize. NO AVERAGE SERIALIZATION TIME AVAILABLE.");
                }
                if (numDeserializations > 0)
                {
                    avgDeserializeDataSecs /= numDeserializations;
                    TestHelper.ReportValueToConsole("Events_AuditLog_PerformanceTest: GetManyAuditLogDataByCriteria AVERAGE DESERIALIZATION TIME (in seconds)", avgDeserializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Events_AuditLog_PerformanceTest: GetManyAuditLogDataByCriteria -- Could not deserialize. NO AVERAGE DESERIALIZATION TIME AVAILABLE.");
                }
                ...
                Console.WriteLine("******************** Events_AuditLog_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Events_AuditLog_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
