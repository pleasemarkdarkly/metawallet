...
namespace MW.MComm.WalletSolution.UnitTest.Core
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Core"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Core.EventType))]
    public partial class EventType_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        [Test]
        public void Core_EventType_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Core_EventType_PerformanceTest START ********************");
                ...
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Core.EventType> items = EventType_Manager.GetAllEventTypeDataTest(Results);
                        ...
                        items = (BLL.SortableList<BLL.Core.EventType>)oItems;
                        oItems = null;
                        items = null;
                    }
                ...
                Console.WriteLine("******************** Core_EventType_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Core_EventType_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
