...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for DebugAttribute instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Core"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Core.DebugAttribute))]
    public partial class DebugAttribute_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public DebugAttribute_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for DebugAttribute.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Core_DebugAttribute_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Core_DebugAttribute_PerformanceTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Core_DebugAttribute_PerformanceTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                {
                    TestHelper.ReportValueToConsole("Core_DebugAttribute_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString((UnitTestPerformanceLevel)(i + 1)) + " time (in seconds)", perfTimeLevels[i]);
                }
                TestHelper.ReportValueToConsole("Core_DebugAttribute_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString(UnitTestPerformanceLevel.Unacceptable) + " time (in seconds) is beyond", perfTimeLevels[(((int)UnitTestPerformanceLevel.Unacceptable - 1) - 1)]);
                ...
                TestHelper.ReportValueToConsole("Core_DebugAttribute_PerformanceTest: page size", pageSize);
                TestHelper.ReportValueToConsole("Core_DebugAttribute_PerformanceTest: maxListSize", maxListSize);
                ...
                TestHelper.ReportValueToConsole("Core_DebugAttribute_PerformanceTest: GetManyDebugAttributeDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    long startTicks = 0;
                    long endTicks = 0;
                    double secondsInterval = 0.00;
                    BLL.SortableList<BLL.Core.DebugAttribute> items = null;

                    TestHelper.ReportValueToConsole("Core_DebugAttribute_PerformanceTest: Loop Iteration", i);
                    ...
                    TestHelper.ReportValueToConsole("Core_DebugAttribute_PerformanceTest: GetManyDebugAttributeDataByCriteria acceptable search time (in seconds)", acceptableTime);
                    try
                    {
                        // get many
                        Results.ResetCurrentPerfMetric();
                        Results.SetPerfMetricName("Core.DebugAttribute.GetMany." + i.ToString());
                        items = DebugAttribute_Manager.GetManyDebugAttributeDataByCriteriaTest(dbOptions, Results);
                        TestHelper.ReportValueToConsole("Core_DebugAttribute_PerformanceTest: GetManyDebugAttributeDataByCriteria succeeded for DebugAttribute. The count", items.Count.ToString());
                        ...
                        perfStr = "GetManyDebugAttributeDataByCriteria search time (in seconds) for DebugAttribute count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional,"GetManyDebugAttributeDataByCriteria failed for DebugAttribute", itemStr, ex, strExtraInfo);
                        }
                    }

                    if (items != null)
                    {
                        TestHelper.ReportValueToConsole("Core_DebugAttribute_PerformanceTest: GetManyDebugAttributeDataByCriteria acceptable serialization time (in seconds)", acceptableTime);
                        try
                        {
                            // serialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Core.DebugAttribute.Serialize." + i.ToString());
                            ...
                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyDebugAttributeDataByCriteria serialization time (in seconds) for DebugAttribute count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                        ...
                        TestHelper.ReportValueToConsole("Core_DebugAttribute_PerformanceTest: GetManyDebugAttributeDataByCriteria acceptable deserialization time (in seconds)", acceptableTime);
                        try
                        {
                            // deserialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Core.DebugAttribute.Deserialize." + i.ToString());
                            ...
                            items = (BLL.SortableList<BLL.Core.DebugAttribute>)oItems;
                            int itemCount = items.Count;
                            oItems = null;
                            items = null;

                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyDebugAttributeDataByCriteria deserialization time (in seconds) for DebugAttribute count " + itemCount + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                ...
                if (numGetDataSearches > 0)
                {
                    ...
                    TestHelper.ReportValueToConsole("Core_DebugAttribute_PerformanceTest: GetManyDebugAttributeDataByCriteria AVERAGE SEARCH TIME (in seconds)", avgGetDataSearchSecs);
                }
                else
                {
                    Console.WriteLine("Core_DebugAttribute_PerformanceTest: GetManyDebugAttributeDataByCriteria -- Could not perform search. NO AVERAGE SEARCH TIME AVAILABLE.");
                }
                if (numSerializations > 0)
                {
                    avgSerializeDataSecs /= numSerializations;
                    TestHelper.ReportValueToConsole("Core_DebugAttribute_PerformanceTest: GetManyDebugAttributeDataByCriteria AVERAGE SERIALIZATION TIME (in seconds)", avgSerializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Core_DebugAttribute_PerformanceTest: GetManyDebugAttributeDataByCriteria -- Could not serialize. NO AVERAGE SERIALIZATION TIME AVAILABLE.");
                }
                if (numDeserializations > 0)
                {
                    avgDeserializeDataSecs /= numDeserializations;
                    TestHelper.ReportValueToConsole("Core_DebugAttribute_PerformanceTest: GetManyDebugAttributeDataByCriteria AVERAGE DESERIALIZATION TIME (in seconds)", avgDeserializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Core_DebugAttribute_PerformanceTest: GetManyDebugAttributeDataByCriteria -- Could not deserialize. NO AVERAGE DESERIALIZATION TIME AVAILABLE.");
                }
                ...
                Console.WriteLine("******************** Core_DebugAttribute_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Core_DebugAttribute_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
