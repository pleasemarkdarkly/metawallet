...
namespace MW.MComm.WalletSolution.UnitTest.Core
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Core"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Core.SeverityLevel))]
    public partial class SeverityLevel_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public SeverityLevel_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for SeverityLevel.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Core_SeverityLevel_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Core_SeverityLevel_PerformanceTest START ********************");
                ...
                Console.WriteLine("GetAllSeverityLevelData test count is {0}.", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Core.SeverityLevel> items = SeverityLevel_Manager.GetAllSeverityLevelDataTest(Results);
                        Console.WriteLine("GetAllSeverityLevelData succeeded for SeverityLevel.");
                        ...
                        items = (BLL.SortableList<BLL.Core.SeverityLevel>)oItems;
                        oItems = null;
                        items = null;
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllSeverityLevelData failed for SeverityLevel", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                Console.WriteLine("******************** Core_SeverityLevel_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Core_SeverityLevel_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
