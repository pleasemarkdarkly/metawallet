...
namespace MW.MComm.WalletSolution.UnitTest.Core
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Core"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Core.DebugAttribute))]
    public partial class DebugAttribute_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public DebugAttribute_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for DebugAttribute.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Core_DebugAttribute_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Core_DebugAttribute_BasicTest START ********************");
                ...
                BLL.Core.DebugAttribute item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Core_DebugAttribute_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Core_DebugAttribute_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Core_DebugAttribute_BasicTest: Add DebugAttribute count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = DebugAttribute_Manager.AddOneDebugAttributeTest(true, Results);
                        ...
                        else
                        {
                            ...
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Core_DebugAttribute_BasicTest: Add succeeded for DebugAttribute", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Core.DebugAttribute)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for DebugAttribute", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for DebugAttribute", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if DebugAttribute Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Core_DebugAttribute_BasicTest: Update DebugAttribute count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = DebugAttribute_Manager.UpdateOneDebugAttributeTest((Guid)loopID["BaseAttributeID"], true, Results);
                        TestHelper.ReportValueToConsole("Core_DebugAttribute_BasicTest: Update succeeded for DebugAttribute", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for DebugAttribute", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Core_DebugAttribute_BasicTest: Delete DebugAttribute count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = DebugAttribute_Manager.DeleteOneDebugAttributeTest((Guid)loopID["BaseAttributeID"], true, Results);
                            TestHelper.ReportValueToConsole("Core_DebugAttribute_BasicTest: Delete succeeded for DebugAttribute", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for DebugAttribute", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Core_DebugAttribute_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Core_DebugAttribute_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
