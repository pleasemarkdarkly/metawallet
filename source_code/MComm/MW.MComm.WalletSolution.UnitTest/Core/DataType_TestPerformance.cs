...
namespace MW.MComm.WalletSolution.UnitTest.Core
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Core"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Core.DataType))]
    public partial class DataType_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public DataType_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for DataType.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Core_DataType_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Core_DataType_PerformanceTest START ********************");
                ...
                Console.WriteLine("GetAllDataTypeData test count is {0}.", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Core.DataType> items = DataType_Manager.GetAllDataTypeDataTest(Results);
                        Console.WriteLine("GetAllDataTypeData succeeded for DataType.");
                        ...
                        items = (BLL.SortableList<BLL.Core.DataType>)oItems;
                        oItems = null;
                        items = null;
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllDataTypeData failed for DataType", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                Console.WriteLine("******************** Core_DataType_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Core_DataType_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
