...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for Fee instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Payments"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Payments.Fee))]
    public partial class Fee_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public Fee_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for Fee.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Payments_Fee_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Payments_Fee_PerformanceTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Payments_Fee_PerformanceTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                {
                    TestHelper.ReportValueToConsole("Payments_Fee_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString((UnitTestPerformanceLevel)(i + 1)) + " time (in seconds)", perfTimeLevels[i]);
                }
                TestHelper.ReportValueToConsole("Payments_Fee_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString(UnitTestPerformanceLevel.Unacceptable) + " time (in seconds) is beyond", perfTimeLevels[(((int)UnitTestPerformanceLevel.Unacceptable - 1) - 1)]);
                ...
                TestHelper.ReportValueToConsole("Payments_Fee_PerformanceTest: page size", pageSize);
                TestHelper.ReportValueToConsole("Payments_Fee_PerformanceTest: maxListSize", maxListSize);
                ...
                TestHelper.ReportValueToConsole("Payments_Fee_PerformanceTest: GetManyFeeDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    long startTicks = 0;
                    long endTicks = 0;
                    double secondsInterval = 0.00;
                    BLL.SortableList<BLL.Payments.Fee> items = null;

                    TestHelper.ReportValueToConsole("Payments_Fee_PerformanceTest: Loop Iteration", i);
                    ...
                    TestHelper.ReportValueToConsole("Payments_Fee_PerformanceTest: GetManyFeeDataByCriteria acceptable search time (in seconds)", acceptableTime);
                    try
                    {
                        // get many
                        Results.ResetCurrentPerfMetric();
                        Results.SetPerfMetricName("Payments.Fee.GetMany." + i.ToString());
                        items = Fee_Manager.GetManyFeeDataByCriteriaTest(dbOptions, Results);
                        TestHelper.ReportValueToConsole("Payments_Fee_PerformanceTest: GetManyFeeDataByCriteria succeeded for Fee. The count", items.Count.ToString());
                        ...
                        perfStr = "GetManyFeeDataByCriteria search time (in seconds) for Fee count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional,"GetManyFeeDataByCriteria failed for Fee", itemStr, ex, strExtraInfo);
                        }
                    }

                    if (items != null)
                    {
                        TestHelper.ReportValueToConsole("Payments_Fee_PerformanceTest: GetManyFeeDataByCriteria acceptable serialization time (in seconds)", acceptableTime);
                        try
                        {
                            // serialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Payments.Fee.Serialize." + i.ToString());
                            ...
                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyFeeDataByCriteria serialization time (in seconds) for Fee count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                        ...
                        TestHelper.ReportValueToConsole("Payments_Fee_PerformanceTest: GetManyFeeDataByCriteria acceptable deserialization time (in seconds)", acceptableTime);
                        try
                        {
                            // deserialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Payments.Fee.Deserialize." + i.ToString());
                            ...
                            items = (BLL.SortableList<BLL.Payments.Fee>)oItems;
                            int itemCount = items.Count;
                            oItems = null;
                            items = null;

                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyFeeDataByCriteria deserialization time (in seconds) for Fee count " + itemCount + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                ...
                if (numGetDataSearches > 0)
                {
                    ...
                    TestHelper.ReportValueToConsole("Payments_Fee_PerformanceTest: GetManyFeeDataByCriteria AVERAGE SEARCH TIME (in seconds)", avgGetDataSearchSecs);
                }
                else
                {
                    Console.WriteLine("Payments_Fee_PerformanceTest: GetManyFeeDataByCriteria -- Could not perform search. NO AVERAGE SEARCH TIME AVAILABLE.");
                }
                if (numSerializations > 0)
                {
                    avgSerializeDataSecs /= numSerializations;
                    TestHelper.ReportValueToConsole("Payments_Fee_PerformanceTest: GetManyFeeDataByCriteria AVERAGE SERIALIZATION TIME (in seconds)", avgSerializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Payments_Fee_PerformanceTest: GetManyFeeDataByCriteria -- Could not serialize. NO AVERAGE SERIALIZATION TIME AVAILABLE.");
                }
                if (numDeserializations > 0)
                {
                    avgDeserializeDataSecs /= numDeserializations;
                    TestHelper.ReportValueToConsole("Payments_Fee_PerformanceTest: GetManyFeeDataByCriteria AVERAGE DESERIALIZATION TIME (in seconds)", avgDeserializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Payments_Fee_PerformanceTest: GetManyFeeDataByCriteria -- Could not deserialize. NO AVERAGE DESERIALIZATION TIME AVAILABLE.");
                }
                ...
                Console.WriteLine("******************** Payments_Fee_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Payments_Fee_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
