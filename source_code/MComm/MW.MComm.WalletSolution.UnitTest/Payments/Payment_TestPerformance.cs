...
namespace MW.MComm.WalletSolution.UnitTest.Payments
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Payments"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Payments.Payment))]
    public partial class Payment_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public Payment_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for Payment.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Payments_Payment_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Payments_Payment_PerformanceTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Payments_Payment_PerformanceTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                {
                    TestHelper.ReportValueToConsole("Payments_Payment_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString((UnitTestPerformanceLevel)(i + 1)) + " time (in seconds)", perfTimeLevels[i]);
                }
                TestHelper.ReportValueToConsole("Payments_Payment_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString(UnitTestPerformanceLevel.Unacceptable) + " time (in seconds) is beyond", perfTimeLevels[(((int)UnitTestPerformanceLevel.Unacceptable - 1) - 1)]);
                ...
                TestHelper.ReportValueToConsole("Payments_Payment_PerformanceTest: page size", pageSize);
                TestHelper.ReportValueToConsole("Payments_Payment_PerformanceTest: maxListSize", maxListSize);
                ...
                TestHelper.ReportValueToConsole("Payments_Payment_PerformanceTest: GetManyPaymentDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    long startTicks = 0;
                    long endTicks = 0;
                    double secondsInterval = 0.00;
                    BLL.SortableList<BLL.Payments.Payment> items = null;

                    TestHelper.ReportValueToConsole("Payments_Payment_PerformanceTest: Loop Iteration", i);
                    ...
                    TestHelper.ReportValueToConsole("Payments_Payment_PerformanceTest: GetManyPaymentDataByCriteria acceptable search time (in seconds)", acceptableTime);
                    try
                    {
                        // get many
                        Results.ResetCurrentPerfMetric();
                        Results.SetPerfMetricName("Payments.Payment.GetMany." + i.ToString());
                        items = Payment_Manager.GetManyPaymentDataByCriteriaTest(dbOptions, Results);
                        TestHelper.ReportValueToConsole("Payments_Payment_PerformanceTest: GetManyPaymentDataByCriteria succeeded for Payment. The count", items.Count.ToString());
                        ...
                        perfStr = "GetManyPaymentDataByCriteria search time (in seconds) for Payment count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                    ...
                    if (items != null)
                    {
                        TestHelper.ReportValueToConsole("Payments_Payment_PerformanceTest: GetManyPaymentDataByCriteria acceptable serialization time (in seconds)", acceptableTime);
                        try
                        {
                            // serialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Payments.Payment.Serialize." + i.ToString());
                            ...
                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyPaymentDataByCriteria serialization time (in seconds) for Payment count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                        ...
                        TestHelper.ReportValueToConsole("Payments_Payment_PerformanceTest: GetManyPaymentDataByCriteria acceptable deserialization time (in seconds)", acceptableTime);
                        try
                        {
                            // deserialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Payments.Payment.Deserialize." + i.ToString());
                            ...
                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyPaymentDataByCriteria deserialization time (in seconds) for Payment count " + itemCount + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                ...
                if (numGetDataSearches > 0)
                {
                    ...
                    TestHelper.ReportValueToConsole("Payments_Payment_PerformanceTest: GetManyPaymentDataByCriteria AVERAGE SEARCH TIME (in seconds)", avgGetDataSearchSecs);
                }
                else
                {
                    Console.WriteLine("Payments_Payment_PerformanceTest: GetManyPaymentDataByCriteria -- Could not perform search. NO AVERAGE SEARCH TIME AVAILABLE.");
                }
                if (numSerializations > 0)
                {
                    avgSerializeDataSecs /= numSerializations;
                    TestHelper.ReportValueToConsole("Payments_Payment_PerformanceTest: GetManyPaymentDataByCriteria AVERAGE SERIALIZATION TIME (in seconds)", avgSerializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Payments_Payment_PerformanceTest: GetManyPaymentDataByCriteria -- Could not serialize. NO AVERAGE SERIALIZATION TIME AVAILABLE.");
                }
                if (numDeserializations > 0)
                {
                    avgDeserializeDataSecs /= numDeserializations;
                    TestHelper.ReportValueToConsole("Payments_Payment_PerformanceTest: GetManyPaymentDataByCriteria AVERAGE DESERIALIZATION TIME (in seconds)", avgDeserializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Payments_Payment_PerformanceTest: GetManyPaymentDataByCriteria -- Could not deserialize. NO AVERAGE DESERIALIZATION TIME AVAILABLE.");
                }
                ...
                Console.WriteLine("******************** Payments_Payment_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Payments_Payment_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
