...
namespace MW.MComm.WalletSolution.UnitTest.Payments
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Payments"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Payments.PaymentStatus))]
    public partial class PaymentStatus_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public PaymentStatus_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for PaymentStatus.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Payments_PaymentStatus_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Payments_PaymentStatus_PerformanceTest START ********************");
                ...
                Console.WriteLine("GetAllPaymentStatusData test count is {0}.", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Payments.PaymentStatus> items = PaymentStatus_Manager.GetAllPaymentStatusDataTest(Results);
                        Console.WriteLine("GetAllPaymentStatusData succeeded for PaymentStatus.");
                        ...
                        items = (BLL.SortableList<BLL.Payments.PaymentStatus>)oItems;
                        oItems = null;
                        items = null;
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllPaymentStatusData failed for PaymentStatus", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                Console.WriteLine("******************** Payments_PaymentStatus_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Payments_PaymentStatus_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
