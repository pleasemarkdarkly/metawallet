...
namespace MW.MComm.WalletSolution.UnitTest.Payments
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Payments"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Payments.Payment))]
    public partial class Payment_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for Payment.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Payments_Payment_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Payments_Payment_BasicTest START ********************");
                ...
                BLL.Payments.Payment item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Payments_Payment_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Payments_Payment_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Payments_Payment_BasicTest: Add Payment count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = Payment_Manager.AddOnePaymentTest(true, Results);
                        ...
                        else
                        {
                            ...
                            entityID["PaymentID"] = item.PaymentID;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Payments_Payment_BasicTest: Add succeeded for Payment", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Payments.Payment)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for Payment", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for Payment", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if Payment Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Payments_Payment_BasicTest: Update Payment count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = Payment_Manager.UpdateOnePaymentTest((Guid)loopID["PaymentID"], true, Results);
                        TestHelper.ReportValueToConsole("Payments_Payment_BasicTest: Update succeeded for Payment", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for Payment", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Payments_Payment_BasicTest: Delete Payment count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = Payment_Manager.DeleteOnePaymentTest((Guid)loopID["PaymentID"], true, Results);
                            TestHelper.ReportValueToConsole("Payments_Payment_BasicTest: Delete succeeded for Payment", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for Payment", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Payments_Payment_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Payments_Payment_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
