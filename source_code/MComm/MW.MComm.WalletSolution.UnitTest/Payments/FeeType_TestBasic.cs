...
namespace MW.MComm.WalletSolution.UnitTest.Payments
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Payments"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Payments.FeeType))]
    public partial class FeeType_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for FeeType.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Payments_FeeType_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Payments_FeeType_BasicTest START ********************");
                ...
                BLL.Payments.FeeType item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Payments_FeeType_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Payments_FeeType_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Payments_FeeType_BasicTest: Add FeeType count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = FeeType_Manager.AddOneFeeTypeTest(true, Results);
                        if (Results.IsCollision == true)
                        {
                            throw new UnitTestException(UnitTestErrorType.Warning, "Add collision encountered for FeeType: " + item.PrimaryKey.ToString(), null);
                        }
                        else
                        {
                            MOD.Data.NamedObjectCollection entityID = new MOD.Data.NamedObjectCollection();
                            entityID["FeeTypeCode"] = item.FeeTypeCode;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Payments_FeeType_BasicTest: Add succeeded for FeeType", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Payments.FeeType)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for FeeType", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for FeeType", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if FeeType Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Payments_FeeType_BasicTest: Update FeeType count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = FeeType_Manager.UpdateOneFeeTypeTest((int)loopID["FeeTypeCode"], true, Results);
                        TestHelper.ReportValueToConsole("Payments_FeeType_BasicTest: Update succeeded for FeeType", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for FeeType", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Payments_FeeType_BasicTest: Delete FeeType count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = FeeType_Manager.DeleteOneFeeTypeTest((int)loopID["FeeTypeCode"], true, Results);
                            TestHelper.ReportValueToConsole("Payments_FeeType_BasicTest: Delete succeeded for FeeType", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for FeeType", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Payments_FeeType_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Payments_FeeType_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
