...
namespace MW.MComm.WalletSolution.UnitTest.Environments
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Environments"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Environments.DateFormat))]
    public partial class DateFormat_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public DateFormat_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for DateFormat.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Environments_DateFormat_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Environments_DateFormat_PerformanceTest START ********************");
                ...
                Console.WriteLine("GetAllDateFormatData test count is {0}.", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Environments.DateFormat> items = DateFormat_Manager.GetAllDateFormatDataTest(Results);
                        Console.WriteLine("GetAllDateFormatData succeeded for DateFormat.");
                        ...
                        items = (BLL.SortableList<BLL.Environments.DateFormat>)oItems;
                        oItems = null;
                        items = null;
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllDateFormatData failed for DateFormat", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                Console.WriteLine("******************** Environments_DateFormat_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Environments_DateFormat_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
