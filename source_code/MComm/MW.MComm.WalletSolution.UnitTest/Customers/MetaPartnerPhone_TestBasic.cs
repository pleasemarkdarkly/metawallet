...
namespace MW.MComm.WalletSolution.UnitTest.Customers
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Customers"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Customers.MetaPartnerPhone))]
    public partial class MetaPartnerPhone_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public MetaPartnerPhone_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for MetaPartnerPhone.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Customers_MetaPartnerPhone_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Customers_MetaPartnerPhone_BasicTest START ********************");
                ...
                BLL.Customers.MetaPartnerPhone item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Customers_MetaPartnerPhone_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Customers_MetaPartnerPhone_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Customers_MetaPartnerPhone_BasicTest: Add MetaPartnerPhone count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = MetaPartnerPhone_Manager.AddOneMetaPartnerPhoneTest(true, Results);
                        ...
                        else
                        {
                            ...
                            entityID["MetaPartnerPhoneID"] = item.MetaPartnerPhoneID;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Customers_MetaPartnerPhone_BasicTest: Add succeeded for MetaPartnerPhone", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Customers.MetaPartnerPhone)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for MetaPartnerPhone", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for MetaPartnerPhone", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if MetaPartnerPhone Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Customers_MetaPartnerPhone_BasicTest: Update MetaPartnerPhone count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = MetaPartnerPhone_Manager.UpdateOneMetaPartnerPhoneTest((Guid)loopID["MetaPartnerPhoneID"], true, Results);
                        TestHelper.ReportValueToConsole("Customers_MetaPartnerPhone_BasicTest: Update succeeded for MetaPartnerPhone", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for MetaPartnerPhone", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Customers_MetaPartnerPhone_BasicTest: Delete MetaPartnerPhone count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = MetaPartnerPhone_Manager.DeleteOneMetaPartnerPhoneTest((Guid)loopID["MetaPartnerPhoneID"], true, Results);
                            TestHelper.ReportValueToConsole("Customers_MetaPartnerPhone_BasicTest: Delete succeeded for MetaPartnerPhone", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for MetaPartnerPhone", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Customers_MetaPartnerPhone_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Customers_MetaPartnerPhone_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
