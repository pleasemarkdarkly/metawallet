...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for LocationType instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Customers"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Customers.LocationType))]
    public partial class LocationType_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public LocationType_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for LocationType.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Customers_LocationType_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Customers_LocationType_BasicTest START ********************");
                ...
                BLL.Customers.LocationType item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Customers_LocationType_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Customers_LocationType_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Customers_LocationType_BasicTest: Add LocationType count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = LocationType_Manager.AddOneLocationTypeTest(true, Results);
                        if (Results.IsCollision == true)
                        {
                            throw new UnitTestException(UnitTestErrorType.Warning, "Add collision encountered for LocationType: " + item.PrimaryKey.ToString(), null);
                        }
                        else
                        {
                            MOD.Data.NamedObjectCollection entityID = new MOD.Data.NamedObjectCollection();
                            entityID["LocationTypeCode"] = item.LocationTypeCode;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Customers_LocationType_BasicTest: Add succeeded for LocationType", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Customers.LocationType)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for LocationType", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for LocationType", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if LocationType Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Customers_LocationType_BasicTest: Update LocationType count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = LocationType_Manager.UpdateOneLocationTypeTest((int)loopID["LocationTypeCode"], true, Results);
                        TestHelper.ReportValueToConsole("Customers_LocationType_BasicTest: Update succeeded for LocationType", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for LocationType", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Customers_LocationType_BasicTest: Delete LocationType count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = LocationType_Manager.DeleteOneLocationTypeTest((int)loopID["LocationTypeCode"], true, Results);
                            TestHelper.ReportValueToConsole("Customers_LocationType_BasicTest: Delete succeeded for LocationType", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for LocationType", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Customers_LocationType_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Customers_LocationType_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
