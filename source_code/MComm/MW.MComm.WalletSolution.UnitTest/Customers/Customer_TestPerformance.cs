...
namespace MW.MComm.WalletSolution.UnitTest.Customers
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Customers"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Customers.Customer))]
    public partial class Customer_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public Customer_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for Customer.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Customers_Customer_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Customers_Customer_PerformanceTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Customers_Customer_PerformanceTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                {
                    TestHelper.ReportValueToConsole("Customers_Customer_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString((UnitTestPerformanceLevel)(i + 1)) + " time (in seconds)", perfTimeLevels[i]);
                }
                TestHelper.ReportValueToConsole("Customers_Customer_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString(UnitTestPerformanceLevel.Unacceptable) + " time (in seconds) is beyond", perfTimeLevels[(((int)UnitTestPerformanceLevel.Unacceptable - 1) - 1)]);
                ...
                TestHelper.ReportValueToConsole("Customers_Customer_PerformanceTest: page size", pageSize);
                TestHelper.ReportValueToConsole("Customers_Customer_PerformanceTest: maxListSize", maxListSize);
                ...
                TestHelper.ReportValueToConsole("Customers_Customer_PerformanceTest: GetManyCustomerDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    long startTicks = 0;
                    long endTicks = 0;
                    double secondsInterval = 0.00;
                    BLL.SortableList<BLL.Customers.Customer> items = null;

                    TestHelper.ReportValueToConsole("Customers_Customer_PerformanceTest: Loop Iteration", i);
                    ...
                    TestHelper.ReportValueToConsole("Customers_Customer_PerformanceTest: GetManyCustomerDataByCriteria acceptable search time (in seconds)", acceptableTime);
                    try
                    {
                        // get many
                        Results.ResetCurrentPerfMetric();
                        Results.SetPerfMetricName("Customers.Customer.GetMany." + i.ToString());
                        items = Customer_Manager.GetManyCustomerDataByCriteriaTest(dbOptions, Results);
                        TestHelper.ReportValueToConsole("Customers_Customer_PerformanceTest: GetManyCustomerDataByCriteria succeeded for Customer. The count", items.Count.ToString());
                        ...
                        perfStr = "GetManyCustomerDataByCriteria search time (in seconds) for Customer count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional,"GetManyCustomerDataByCriteria failed for Customer", itemStr, ex, strExtraInfo);
                        }
                    }

                    if (items != null)
                    {
                        TestHelper.ReportValueToConsole("Customers_Customer_PerformanceTest: GetManyCustomerDataByCriteria acceptable serialization time (in seconds)", acceptableTime);
                        try
                        {
                            // serialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Customers.Customer.Serialize." + i.ToString());
                            ...
                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyCustomerDataByCriteria serialization time (in seconds) for Customer count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                        ...
                        TestHelper.ReportValueToConsole("Customers_Customer_PerformanceTest: GetManyCustomerDataByCriteria acceptable deserialization time (in seconds)", acceptableTime);
                        try
                        {
                            // deserialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Customers.Customer.Deserialize." + i.ToString());
                            ...
                            items = (BLL.SortableList<BLL.Customers.Customer>)oItems;
                            int itemCount = items.Count;
                            oItems = null;
                            items = null;

                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyCustomerDataByCriteria deserialization time (in seconds) for Customer count " + itemCount + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                ...
                if (numGetDataSearches > 0)
                {
                    ...
                    TestHelper.ReportValueToConsole("Customers_Customer_PerformanceTest: GetManyCustomerDataByCriteria AVERAGE SEARCH TIME (in seconds)", avgGetDataSearchSecs);
                }
                else
                {
                    Console.WriteLine("Customers_Customer_PerformanceTest: GetManyCustomerDataByCriteria -- Could not perform search. NO AVERAGE SEARCH TIME AVAILABLE.");
                }
                if (numSerializations > 0)
                {
                    avgSerializeDataSecs /= numSerializations;
                    TestHelper.ReportValueToConsole("Customers_Customer_PerformanceTest: GetManyCustomerDataByCriteria AVERAGE SERIALIZATION TIME (in seconds)", avgSerializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Customers_Customer_PerformanceTest: GetManyCustomerDataByCriteria -- Could not serialize. NO AVERAGE SERIALIZATION TIME AVAILABLE.");
                }
                if (numDeserializations > 0)
                {
                    avgDeserializeDataSecs /= numDeserializations;
                    TestHelper.ReportValueToConsole("Customers_Customer_PerformanceTest: GetManyCustomerDataByCriteria AVERAGE DESERIALIZATION TIME (in seconds)", avgDeserializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Customers_Customer_PerformanceTest: GetManyCustomerDataByCriteria -- Could not deserialize. NO AVERAGE DESERIALIZATION TIME AVAILABLE.");
                }
                ...
                Console.WriteLine("******************** Customers_Customer_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Customers_Customer_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
