...
namespace MW.MComm.WalletSolution.UnitTest.Customers
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Customers"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Customers.LocationType))]
    public partial class LocationType_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public LocationType_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for LocationType.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Customers_LocationType_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Customers_LocationType_PerformanceTest START ********************");
                ...
                Console.WriteLine("GetAllLocationTypeData test count is {0}.", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Customers.LocationType> items = LocationType_Manager.GetAllLocationTypeDataTest(Results);
                        Console.WriteLine("GetAllLocationTypeData succeeded for LocationType.");
                        ...
                        items = (BLL.SortableList<BLL.Customers.LocationType>)oItems;
                        oItems = null;
                        items = null;
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllLocationTypeData failed for LocationType", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                Console.WriteLine("******************** Customers_LocationType_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Customers_LocationType_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
