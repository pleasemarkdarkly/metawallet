...
namespace MW.MComm.WalletSolution.UnitTest.Customers
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Customers"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Customers.MetaPartner))]
    public partial class MetaPartner_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public MetaPartner_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for MetaPartner.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Customers_MetaPartner_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Customers_MetaPartner_BasicTest START ********************");
                ...
                BLL.Customers.MetaPartner item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Customers_MetaPartner_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Customers_MetaPartner_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Customers_MetaPartner_BasicTest: Add MetaPartner count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = MetaPartner_Manager.AddOneMetaPartnerTest(true, Results);
                        ...
                        else
                        {
                            ...
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Customers_MetaPartner_BasicTest: Add succeeded for MetaPartner", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Customers.MetaPartner)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for MetaPartner", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for MetaPartner", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if MetaPartner Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Customers_MetaPartner_BasicTest: Update MetaPartner count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = MetaPartner_Manager.UpdateOneMetaPartnerTest((Guid)loopID["MetaPartnerID"], true, Results);
                        TestHelper.ReportValueToConsole("Customers_MetaPartner_BasicTest: Update succeeded for MetaPartner", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for MetaPartner", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Customers_MetaPartner_BasicTest: Delete MetaPartner count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = MetaPartner_Manager.DeleteOneMetaPartnerTest((Guid)loopID["MetaPartnerID"], true, Results);
                            TestHelper.ReportValueToConsole("Customers_MetaPartner_BasicTest: Delete succeeded for MetaPartner", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for MetaPartner", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Customers_MetaPartner_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Customers_MetaPartner_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
