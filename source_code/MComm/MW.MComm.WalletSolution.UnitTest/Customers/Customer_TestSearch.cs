...
namespace MW.MComm.WalletSolution.UnitTest.Customers
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Search.Customers"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Customers.Customer))]
    public partial class Customer_TestSearch : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public Customer_TestSearch() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the search entity test for Customer.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Customers_Customer_SearchTest()
        {
            try
            {
                Console.WriteLine("******************** Customers_Customer_SearchTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Customers_Customer_SearchTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Customers_Customer_SearchTest: GetAllCustomerData test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Customers.Customer> items = Customer_Manager.GetAllCustomerDataTest(Results);
                        TestHelper.ReportValueToConsole("Customers_Customer_SearchTest: GetAllCustomerData succeeded for Customer. The count", items.Count.ToString());
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllCustomerData failed for Customer", itemStr, ex, strExtraInfo);
                        }
                    }
                }
                loadCount = TestHelper.GetInt(Globals.MinimumGetAllCountPerEntity, Globals.MaximumGetAllCountPerEntity);
                TestHelper.ReportValueToConsole("Customers_Customer_SearchTest: GetManyCustomerDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Customers.Customer> items = Customer_Manager.GetManyCustomerDataByCriteriaTest(Globals.getDataOptions("","Random",1,200,2000), Results);
                        TestHelper.ReportValueToConsole("Customers_Customer_SearchTest: GetManyCustomerDataByCriteria succeeded for Customer. The count", items.Count.ToString());
                ...
                Console.WriteLine("******************** Customers_Customer_SearchTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Customers_Customer_SearchTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
