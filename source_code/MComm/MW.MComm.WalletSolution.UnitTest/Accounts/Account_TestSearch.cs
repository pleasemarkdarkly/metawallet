...
namespace MW.MComm.WalletSolution.UnitTest.Accounts
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Search.Accounts"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Accounts.Account))]
    public partial class Account_TestSearch : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public Account_TestSearch() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the search entity test for Account.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Accounts_Account_SearchTest()
        {
            try
            {
                Console.WriteLine("******************** Accounts_Account_SearchTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Accounts_Account_SearchTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Accounts_Account_SearchTest: GetAllAccountData test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Accounts.Account> items = Account_Manager.GetAllAccountDataTest(Results);
                        TestHelper.ReportValueToConsole("Accounts_Account_SearchTest: GetAllAccountData succeeded for Account. The count", items.Count.ToString());
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllAccountData failed for Account", itemStr, ex, strExtraInfo);
                        }
                    }
                }
                loadCount = TestHelper.GetInt(Globals.MinimumGetAllCountPerEntity, Globals.MaximumGetAllCountPerEntity);
                TestHelper.ReportValueToConsole("Accounts_Account_SearchTest: GetManyAccountDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Accounts.Account> items = Account_Manager.GetManyAccountDataByCriteriaTest(Globals.getDataOptions("","Random",1,200,2000), Results);
                        TestHelper.ReportValueToConsole("Accounts_Account_SearchTest: GetManyAccountDataByCriteria succeeded for Account. The count", items.Count.ToString());
                ...
                Console.WriteLine("******************** Accounts_Account_SearchTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Accounts_Account_SearchTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
