...
namespace MW.MComm.WalletSolution.UnitTest.Accounts
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Accounts"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Accounts.Currency))]
    public partial class Currency_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public Currency_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for Currency.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Accounts_Currency_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Accounts_Currency_PerformanceTest START ********************");
                ...
                Console.WriteLine("GetAllCurrencyData test count is {0}.", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Accounts.Currency> items = Currency_Manager.GetAllCurrencyDataTest(Results);
                        Console.WriteLine("GetAllCurrencyData succeeded for Currency.");
                        ...
                        items = (BLL.SortableList<BLL.Accounts.Currency>)oItems;
                        oItems = null;
                        items = null;
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllCurrencyData failed for Currency", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                Console.WriteLine("******************** Accounts_Currency_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Accounts_Currency_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
