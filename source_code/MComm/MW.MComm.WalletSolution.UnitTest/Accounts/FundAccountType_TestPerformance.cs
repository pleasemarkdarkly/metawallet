...
namespace MW.MComm.WalletSolution.UnitTest.Accounts
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Accounts"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Accounts.FundAccountType))]
    public partial class FundAccountType_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public FundAccountType_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for FundAccountType.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Accounts_FundAccountType_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Accounts_FundAccountType_PerformanceTest START ********************");
                ...
                Console.WriteLine("GetAllFundAccountTypeData test count is {0}.", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Accounts.FundAccountType> items = FundAccountType_Manager.GetAllFundAccountTypeDataTest(Results);
                        Console.WriteLine("GetAllFundAccountTypeData succeeded for FundAccountType.");
                        ...
                        items = (BLL.SortableList<BLL.Accounts.FundAccountType>)oItems;
                        oItems = null;
                        items = null;
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllFundAccountTypeData failed for FundAccountType", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                Console.WriteLine("******************** Accounts_FundAccountType_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Accounts_FundAccountType_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
