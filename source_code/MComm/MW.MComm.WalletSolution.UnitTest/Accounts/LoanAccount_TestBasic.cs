...
namespace MW.MComm.WalletSolution.UnitTest.Accounts
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Accounts"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Accounts.LoanAccount))]
    public partial class LoanAccount_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public LoanAccount_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for LoanAccount.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Accounts_LoanAccount_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Accounts_LoanAccount_BasicTest START ********************");
                ...
                BLL.Accounts.LoanAccount item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Accounts_LoanAccount_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Accounts_LoanAccount_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Accounts_LoanAccount_BasicTest: Add LoanAccount count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = LoanAccount_Manager.AddOneLoanAccountTest(true, Results);
                        ...
                        else
                        {
                            ...
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Accounts_LoanAccount_BasicTest: Add succeeded for LoanAccount", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Accounts.LoanAccount)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for LoanAccount", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for LoanAccount", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if LoanAccount Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Accounts_LoanAccount_BasicTest: Update LoanAccount count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = LoanAccount_Manager.UpdateOneLoanAccountTest((Guid)loopID["AccountID"], true, Results);
                        TestHelper.ReportValueToConsole("Accounts_LoanAccount_BasicTest: Update succeeded for LoanAccount", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for LoanAccount", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Accounts_LoanAccount_BasicTest: Delete LoanAccount count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = LoanAccount_Manager.DeleteOneLoanAccountTest((Guid)loopID["AccountID"], true, Results);
                            TestHelper.ReportValueToConsole("Accounts_LoanAccount_BasicTest: Delete succeeded for LoanAccount", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for LoanAccount", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Accounts_LoanAccount_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Accounts_LoanAccount_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
