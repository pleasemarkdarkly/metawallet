...
namespace MW.MComm.WalletSolution.UnitTest.Accounts
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Search.Accounts"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Accounts.BankAccount))]
    public partial class BankAccount_TestSearch : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public BankAccount_TestSearch() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the search entity test for BankAccount.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Accounts_BankAccount_SearchTest()
        {
            try
            {
                Console.WriteLine("******************** Accounts_BankAccount_SearchTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Accounts_BankAccount_SearchTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Accounts_BankAccount_SearchTest: GetAllBankAccountData test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Accounts.BankAccount> items = BankAccount_Manager.GetAllBankAccountDataTest(Results);
                        TestHelper.ReportValueToConsole("Accounts_BankAccount_SearchTest: GetAllBankAccountData succeeded for BankAccount. The count", items.Count.ToString());
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllBankAccountData failed for BankAccount", itemStr, ex, strExtraInfo);
                        }
                    }
                }
                loadCount = TestHelper.GetInt(Globals.MinimumGetAllCountPerEntity, Globals.MaximumGetAllCountPerEntity);
                TestHelper.ReportValueToConsole("Accounts_BankAccount_SearchTest: GetManyBankAccountDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Accounts.BankAccount> items = BankAccount_Manager.GetManyBankAccountDataByCriteriaTest(Globals.getDataOptions("","Random",1,200,2000), Results);
                        TestHelper.ReportValueToConsole("Accounts_BankAccount_SearchTest: GetManyBankAccountDataByCriteria succeeded for BankAccount. The count", items.Count.ToString());
                ...
                Console.WriteLine("******************** Accounts_BankAccount_SearchTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Accounts_BankAccount_SearchTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
