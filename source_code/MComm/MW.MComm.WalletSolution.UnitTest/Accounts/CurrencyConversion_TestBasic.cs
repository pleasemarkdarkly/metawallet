...
namespace MW.MComm.WalletSolution.UnitTest.Accounts
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Accounts"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Accounts.CurrencyConversion))]
    public partial class CurrencyConversion_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public CurrencyConversion_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for CurrencyConversion.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Accounts_CurrencyConversion_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Accounts_CurrencyConversion_BasicTest START ********************");
                ...
                BLL.Accounts.CurrencyConversion item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Accounts_CurrencyConversion_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Accounts_CurrencyConversion_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Accounts_CurrencyConversion_BasicTest: Add CurrencyConversion count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = CurrencyConversion_Manager.AddOneCurrencyConversionTest(true, Results);
                        ...
                        else
                        {
                            ...
                            entityID["CurrencyConversionID"] = item.CurrencyConversionID;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Accounts_CurrencyConversion_BasicTest: Add succeeded for CurrencyConversion", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Accounts.CurrencyConversion)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for CurrencyConversion", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for CurrencyConversion", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if CurrencyConversion Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Accounts_CurrencyConversion_BasicTest: Update CurrencyConversion count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = CurrencyConversion_Manager.UpdateOneCurrencyConversionTest((Guid)loopID["CurrencyConversionID"], true, Results);
                        TestHelper.ReportValueToConsole("Accounts_CurrencyConversion_BasicTest: Update succeeded for CurrencyConversion", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for CurrencyConversion", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Accounts_CurrencyConversion_BasicTest: Delete CurrencyConversion count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = CurrencyConversion_Manager.DeleteOneCurrencyConversionTest((Guid)loopID["CurrencyConversionID"], true, Results);
                            TestHelper.ReportValueToConsole("Accounts_CurrencyConversion_BasicTest: Delete succeeded for CurrencyConversion", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for CurrencyConversion", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Accounts_CurrencyConversion_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Accounts_CurrencyConversion_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
