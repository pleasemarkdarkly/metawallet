...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for FundAccount instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Accounts"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Accounts.FundAccount))]
    public partial class FundAccount_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public FundAccount_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for FundAccount.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Accounts_FundAccount_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Accounts_FundAccount_PerformanceTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Accounts_FundAccount_PerformanceTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                {
                    TestHelper.ReportValueToConsole("Accounts_FundAccount_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString((UnitTestPerformanceLevel)(i + 1)) + " time (in seconds)", perfTimeLevels[i]);
                }
                TestHelper.ReportValueToConsole("Accounts_FundAccount_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString(UnitTestPerformanceLevel.Unacceptable) + " time (in seconds) is beyond", perfTimeLevels[(((int)UnitTestPerformanceLevel.Unacceptable - 1) - 1)]);
                ...
                TestHelper.ReportValueToConsole("Accounts_FundAccount_PerformanceTest: page size", pageSize);
                TestHelper.ReportValueToConsole("Accounts_FundAccount_PerformanceTest: maxListSize", maxListSize);
                ...
                TestHelper.ReportValueToConsole("Accounts_FundAccount_PerformanceTest: GetManyFundAccountDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    long startTicks = 0;
                    long endTicks = 0;
                    double secondsInterval = 0.00;
                    BLL.SortableList<BLL.Accounts.FundAccount> items = null;

                    TestHelper.ReportValueToConsole("Accounts_FundAccount_PerformanceTest: Loop Iteration", i);
                    ...
                    TestHelper.ReportValueToConsole("Accounts_FundAccount_PerformanceTest: GetManyFundAccountDataByCriteria acceptable search time (in seconds)", acceptableTime);
                    try
                    {
                        // get many
                        Results.ResetCurrentPerfMetric();
                        Results.SetPerfMetricName("Accounts.FundAccount.GetMany." + i.ToString());
                        items = FundAccount_Manager.GetManyFundAccountDataByCriteriaTest(dbOptions, Results);
                        TestHelper.ReportValueToConsole("Accounts_FundAccount_PerformanceTest: GetManyFundAccountDataByCriteria succeeded for FundAccount. The count", items.Count.ToString());
                        ...
                        perfStr = "GetManyFundAccountDataByCriteria search time (in seconds) for FundAccount count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional,"GetManyFundAccountDataByCriteria failed for FundAccount", itemStr, ex, strExtraInfo);
                        }
                    }

                    if (items != null)
                    {
                        TestHelper.ReportValueToConsole("Accounts_FundAccount_PerformanceTest: GetManyFundAccountDataByCriteria acceptable serialization time (in seconds)", acceptableTime);
                        try
                        {
                            // serialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Accounts.FundAccount.Serialize." + i.ToString());
                            ...
                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyFundAccountDataByCriteria serialization time (in seconds) for FundAccount count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                        ...
                        TestHelper.ReportValueToConsole("Accounts_FundAccount_PerformanceTest: GetManyFundAccountDataByCriteria acceptable deserialization time (in seconds)", acceptableTime);
                        try
                        {
                            // deserialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Accounts.FundAccount.Deserialize." + i.ToString());
                            ...
                            items = (BLL.SortableList<BLL.Accounts.FundAccount>)oItems;
                            int itemCount = items.Count;
                            oItems = null;
                            items = null;

                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyFundAccountDataByCriteria deserialization time (in seconds) for FundAccount count " + itemCount + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                ...
                if (numGetDataSearches > 0)
                {
                    ...
                    TestHelper.ReportValueToConsole("Accounts_FundAccount_PerformanceTest: GetManyFundAccountDataByCriteria AVERAGE SEARCH TIME (in seconds)", avgGetDataSearchSecs);
                }
                else
                {
                    Console.WriteLine("Accounts_FundAccount_PerformanceTest: GetManyFundAccountDataByCriteria -- Could not perform search. NO AVERAGE SEARCH TIME AVAILABLE.");
                }
                if (numSerializations > 0)
                {
                    avgSerializeDataSecs /= numSerializations;
                    TestHelper.ReportValueToConsole("Accounts_FundAccount_PerformanceTest: GetManyFundAccountDataByCriteria AVERAGE SERIALIZATION TIME (in seconds)", avgSerializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Accounts_FundAccount_PerformanceTest: GetManyFundAccountDataByCriteria -- Could not serialize. NO AVERAGE SERIALIZATION TIME AVAILABLE.");
                }
                if (numDeserializations > 0)
                {
                    avgDeserializeDataSecs /= numDeserializations;
                    TestHelper.ReportValueToConsole("Accounts_FundAccount_PerformanceTest: GetManyFundAccountDataByCriteria AVERAGE DESERIALIZATION TIME (in seconds)", avgDeserializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Accounts_FundAccount_PerformanceTest: GetManyFundAccountDataByCriteria -- Could not deserialize. NO AVERAGE DESERIALIZATION TIME AVAILABLE.");
                }
                ...
                Console.WriteLine("******************** Accounts_FundAccount_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Accounts_FundAccount_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
