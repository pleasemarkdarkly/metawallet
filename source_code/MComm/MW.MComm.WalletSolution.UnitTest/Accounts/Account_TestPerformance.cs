...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for Account instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Accounts"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Accounts.Account))]
    public partial class Account_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public Account_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for Account.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Accounts_Account_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Accounts_Account_PerformanceTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Accounts_Account_PerformanceTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                {
                    TestHelper.ReportValueToConsole("Accounts_Account_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString((UnitTestPerformanceLevel)(i + 1)) + " time (in seconds)", perfTimeLevels[i]);
                }
                TestHelper.ReportValueToConsole("Accounts_Account_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString(UnitTestPerformanceLevel.Unacceptable) + " time (in seconds) is beyond", perfTimeLevels[(((int)UnitTestPerformanceLevel.Unacceptable - 1) - 1)]);
                ...
                TestHelper.ReportValueToConsole("Accounts_Account_PerformanceTest: page size", pageSize);
                TestHelper.ReportValueToConsole("Accounts_Account_PerformanceTest: maxListSize", maxListSize);
                ...
                TestHelper.ReportValueToConsole("Accounts_Account_PerformanceTest: GetManyAccountDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    long startTicks = 0;
                    long endTicks = 0;
                    double secondsInterval = 0.00;
                    BLL.SortableList<BLL.Accounts.Account> items = null;

                    TestHelper.ReportValueToConsole("Accounts_Account_PerformanceTest: Loop Iteration", i);
                    ...
                    TestHelper.ReportValueToConsole("Accounts_Account_PerformanceTest: GetManyAccountDataByCriteria acceptable search time (in seconds)", acceptableTime);
                    try
                    {
                        // get many
                        Results.ResetCurrentPerfMetric();
                        Results.SetPerfMetricName("Accounts.Account.GetMany." + i.ToString());
                        items = Account_Manager.GetManyAccountDataByCriteriaTest(dbOptions, Results);
                        TestHelper.ReportValueToConsole("Accounts_Account_PerformanceTest: GetManyAccountDataByCriteria succeeded for Account. The count", items.Count.ToString());
                        ...
                        perfStr = "GetManyAccountDataByCriteria search time (in seconds) for Account count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional,"GetManyAccountDataByCriteria failed for Account", itemStr, ex, strExtraInfo);
                        }
                    }

                    if (items != null)
                    {
                        TestHelper.ReportValueToConsole("Accounts_Account_PerformanceTest: GetManyAccountDataByCriteria acceptable serialization time (in seconds)", acceptableTime);
                        try
                        {
                            // serialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Accounts.Account.Serialize." + i.ToString());
                            ...
                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyAccountDataByCriteria serialization time (in seconds) for Account count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                        ...
                        TestHelper.ReportValueToConsole("Accounts_Account_PerformanceTest: GetManyAccountDataByCriteria acceptable deserialization time (in seconds)", acceptableTime);
                        try
                        {
                            // deserialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Accounts.Account.Deserialize." + i.ToString());
                            ...
                            items = (BLL.SortableList<BLL.Accounts.Account>)oItems;
                            int itemCount = items.Count;
                            oItems = null;
                            items = null;

                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyAccountDataByCriteria deserialization time (in seconds) for Account count " + itemCount + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                ...
                if (numGetDataSearches > 0)
                {
                    ...
                    TestHelper.ReportValueToConsole("Accounts_Account_PerformanceTest: GetManyAccountDataByCriteria AVERAGE SEARCH TIME (in seconds)", avgGetDataSearchSecs);
                }
                else
                {
                    Console.WriteLine("Accounts_Account_PerformanceTest: GetManyAccountDataByCriteria -- Could not perform search. NO AVERAGE SEARCH TIME AVAILABLE.");
                }
                if (numSerializations > 0)
                {
                    avgSerializeDataSecs /= numSerializations;
                    TestHelper.ReportValueToConsole("Accounts_Account_PerformanceTest: GetManyAccountDataByCriteria AVERAGE SERIALIZATION TIME (in seconds)", avgSerializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Accounts_Account_PerformanceTest: GetManyAccountDataByCriteria -- Could not serialize. NO AVERAGE SERIALIZATION TIME AVAILABLE.");
                }
                if (numDeserializations > 0)
                {
                    avgDeserializeDataSecs /= numDeserializations;
                    TestHelper.ReportValueToConsole("Accounts_Account_PerformanceTest: GetManyAccountDataByCriteria AVERAGE DESERIALIZATION TIME (in seconds)", avgDeserializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Accounts_Account_PerformanceTest: GetManyAccountDataByCriteria -- Could not deserialize. NO AVERAGE DESERIALIZATION TIME AVAILABLE.");
                }
                ...
                Console.WriteLine("******************** Accounts_Account_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Accounts_Account_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
