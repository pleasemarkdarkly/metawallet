...
namespace MW.MComm.WalletSolution.UnitTest.Accounts
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Accounts"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Accounts.BillPayAccount))]
    public partial class BillPayAccount_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public BillPayAccount_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for BillPayAccount.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Accounts_BillPayAccount_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Accounts_BillPayAccount_BasicTest START ********************");
                ...
                BLL.Accounts.BillPayAccount item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Accounts_BillPayAccount_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Accounts_BillPayAccount_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Accounts_BillPayAccount_BasicTest: Add BillPayAccount count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = BillPayAccount_Manager.AddOneBillPayAccountTest(true, Results);
                        ...
                        else
                        {
                            ...
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Accounts_BillPayAccount_BasicTest: Add succeeded for BillPayAccount", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Accounts.BillPayAccount)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for BillPayAccount", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for BillPayAccount", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if BillPayAccount Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Accounts_BillPayAccount_BasicTest: Update BillPayAccount count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = BillPayAccount_Manager.UpdateOneBillPayAccountTest((Guid)loopID["AccountID"], true, Results);
                        TestHelper.ReportValueToConsole("Accounts_BillPayAccount_BasicTest: Update succeeded for BillPayAccount", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for BillPayAccount", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Accounts_BillPayAccount_BasicTest: Delete BillPayAccount count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = BillPayAccount_Manager.DeleteOneBillPayAccountTest((Guid)loopID["AccountID"], true, Results);
                            TestHelper.ReportValueToConsole("Accounts_BillPayAccount_BasicTest: Delete succeeded for BillPayAccount", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for BillPayAccount", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Accounts_BillPayAccount_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Accounts_BillPayAccount_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
