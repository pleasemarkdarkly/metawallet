...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for Currency instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Accounts"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Accounts.Currency))]
    public partial class Currency_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public Currency_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for Currency.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Accounts_Currency_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Accounts_Currency_BasicTest START ********************");
                ...
                BLL.Accounts.Currency item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Accounts_Currency_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Accounts_Currency_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Accounts_Currency_BasicTest: Add Currency count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = Currency_Manager.AddOneCurrencyTest(true, Results);
                        if (Results.IsCollision == true)
                        {
                            throw new UnitTestException(UnitTestErrorType.Warning, "Add collision encountered for Currency: " + item.PrimaryKey.ToString(), null);
                        }
                        else
                        {
                            MOD.Data.NamedObjectCollection entityID = new MOD.Data.NamedObjectCollection();
                            entityID["CurrencyCode"] = item.CurrencyCode;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Accounts_Currency_BasicTest: Add succeeded for Currency", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Accounts.Currency)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for Currency", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for Currency", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if Currency Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Accounts_Currency_BasicTest: Update Currency count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = Currency_Manager.UpdateOneCurrencyTest((int)loopID["CurrencyCode"], true, Results);
                        TestHelper.ReportValueToConsole("Accounts_Currency_BasicTest: Update succeeded for Currency", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for Currency", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Accounts_Currency_BasicTest: Delete Currency count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = Currency_Manager.DeleteOneCurrencyTest((int)loopID["CurrencyCode"], true, Results);
                            TestHelper.ReportValueToConsole("Accounts_Currency_BasicTest: Delete succeeded for Currency", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for Currency", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Accounts_Currency_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Accounts_Currency_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
