...
namespace MW.MComm.WalletSolution.UnitTest.Promotions
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Promotions"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Promotions.MetaPartnerCoupon))]
    public partial class MetaPartnerCoupon_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public MetaPartnerCoupon_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for MetaPartnerCoupon.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Promotions_MetaPartnerCoupon_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Promotions_MetaPartnerCoupon_BasicTest START ********************");
                ...
                BLL.Promotions.MetaPartnerCoupon item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Promotions_MetaPartnerCoupon_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Promotions_MetaPartnerCoupon_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Promotions_MetaPartnerCoupon_BasicTest: Add MetaPartnerCoupon count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = MetaPartnerCoupon_Manager.AddOneMetaPartnerCouponTest(true, Results);
                        ...
                        else
                        {
                            ...
                            entityID["MetaPartnerCouponID"] = item.MetaPartnerCouponID;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Promotions_MetaPartnerCoupon_BasicTest: Add succeeded for MetaPartnerCoupon", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Promotions.MetaPartnerCoupon)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for MetaPartnerCoupon", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for MetaPartnerCoupon", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if MetaPartnerCoupon Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Promotions_MetaPartnerCoupon_BasicTest: Update MetaPartnerCoupon count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = MetaPartnerCoupon_Manager.UpdateOneMetaPartnerCouponTest((Guid)loopID["MetaPartnerCouponID"], true, Results);
                        TestHelper.ReportValueToConsole("Promotions_MetaPartnerCoupon_BasicTest: Update succeeded for MetaPartnerCoupon", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for MetaPartnerCoupon", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Promotions_MetaPartnerCoupon_BasicTest: Delete MetaPartnerCoupon count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = MetaPartnerCoupon_Manager.DeleteOneMetaPartnerCouponTest((Guid)loopID["MetaPartnerCouponID"], true, Results);
                            TestHelper.ReportValueToConsole("Promotions_MetaPartnerCoupon_BasicTest: Delete succeeded for MetaPartnerCoupon", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for MetaPartnerCoupon", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Promotions_MetaPartnerCoupon_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Promotions_MetaPartnerCoupon_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
