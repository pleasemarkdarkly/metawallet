...
namespace MW.MComm.WalletSolution.UnitTest.Promotions
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Promotions"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Promotions.Coupon))]
    public partial class Coupon_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for Coupon.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Promotions_Coupon_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Promotions_Coupon_BasicTest START ********************");
                ...
                BLL.Promotions.Coupon item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Promotions_Coupon_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Promotions_Coupon_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Promotions_Coupon_BasicTest: Add Coupon count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = Coupon_Manager.AddOneCouponTest(true, Results);
                        ...
                        else
                        {
                            ...
                            entityID["CouponCode"] = item.CouponCode;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Promotions_Coupon_BasicTest: Add succeeded for Coupon", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Promotions.Coupon)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for Coupon", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for Coupon", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if Coupon Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Promotions_Coupon_BasicTest: Update Coupon count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = Coupon_Manager.UpdateOneCouponTest((int)loopID["CouponCode"], true, Results);
                        TestHelper.ReportValueToConsole("Promotions_Coupon_BasicTest: Update succeeded for Coupon", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for Coupon", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Promotions_Coupon_BasicTest: Delete Coupon count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = Coupon_Manager.DeleteOneCouponTest((int)loopID["CouponCode"], true, Results);
                            TestHelper.ReportValueToConsole("Promotions_Coupon_BasicTest: Delete succeeded for Coupon", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for Coupon", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Promotions_Coupon_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Promotions_Coupon_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
