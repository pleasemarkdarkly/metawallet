...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for Coupon instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Promotions"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Promotions.Coupon))]
    public partial class Coupon_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public Coupon_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for Coupon.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Promotions_Coupon_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Promotions_Coupon_PerformanceTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Promotions_Coupon_PerformanceTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                {
                    TestHelper.ReportValueToConsole("Promotions_Coupon_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString((UnitTestPerformanceLevel)(i + 1)) + " time (in seconds)", perfTimeLevels[i]);
                }
                TestHelper.ReportValueToConsole("Promotions_Coupon_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString(UnitTestPerformanceLevel.Unacceptable) + " time (in seconds) is beyond", perfTimeLevels[(((int)UnitTestPerformanceLevel.Unacceptable - 1) - 1)]);
                ...
                TestHelper.ReportValueToConsole("Promotions_Coupon_PerformanceTest: page size", pageSize);
                TestHelper.ReportValueToConsole("Promotions_Coupon_PerformanceTest: maxListSize", maxListSize);
                ...
                TestHelper.ReportValueToConsole("Promotions_Coupon_PerformanceTest: GetManyCouponDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    long startTicks = 0;
                    long endTicks = 0;
                    double secondsInterval = 0.00;
                    BLL.SortableList<BLL.Promotions.Coupon> items = null;

                    TestHelper.ReportValueToConsole("Promotions_Coupon_PerformanceTest: Loop Iteration", i);
                    ...
                    TestHelper.ReportValueToConsole("Promotions_Coupon_PerformanceTest: GetManyCouponDataByCriteria acceptable search time (in seconds)", acceptableTime);
                    try
                    {
                        // get many
                        Results.ResetCurrentPerfMetric();
                        Results.SetPerfMetricName("Promotions.Coupon.GetMany." + i.ToString());
                        items = Coupon_Manager.GetManyCouponDataByCriteriaTest(dbOptions, Results);
                        TestHelper.ReportValueToConsole("Promotions_Coupon_PerformanceTest: GetManyCouponDataByCriteria succeeded for Coupon. The count", items.Count.ToString());
                        ...
                        perfStr = "GetManyCouponDataByCriteria search time (in seconds) for Coupon count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional,"GetManyCouponDataByCriteria failed for Coupon", itemStr, ex, strExtraInfo);
                        }
                    }

                    if (items != null)
                    {
                        TestHelper.ReportValueToConsole("Promotions_Coupon_PerformanceTest: GetManyCouponDataByCriteria acceptable serialization time (in seconds)", acceptableTime);
                        try
                        {
                            // serialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Promotions.Coupon.Serialize." + i.ToString());
                            ...
                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyCouponDataByCriteria serialization time (in seconds) for Coupon count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                        ...
                        TestHelper.ReportValueToConsole("Promotions_Coupon_PerformanceTest: GetManyCouponDataByCriteria acceptable deserialization time (in seconds)", acceptableTime);
                        try
                        {
                            // deserialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Promotions.Coupon.Deserialize." + i.ToString());
                            ...
                            items = (BLL.SortableList<BLL.Promotions.Coupon>)oItems;
                            int itemCount = items.Count;
                            oItems = null;
                            items = null;

                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyCouponDataByCriteria deserialization time (in seconds) for Coupon count " + itemCount + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                ...
                if (numGetDataSearches > 0)
                {
                    ...
                    TestHelper.ReportValueToConsole("Promotions_Coupon_PerformanceTest: GetManyCouponDataByCriteria AVERAGE SEARCH TIME (in seconds)", avgGetDataSearchSecs);
                }
                else
                {
                    Console.WriteLine("Promotions_Coupon_PerformanceTest: GetManyCouponDataByCriteria -- Could not perform search. NO AVERAGE SEARCH TIME AVAILABLE.");
                }
                if (numSerializations > 0)
                {
                    avgSerializeDataSecs /= numSerializations;
                    TestHelper.ReportValueToConsole("Promotions_Coupon_PerformanceTest: GetManyCouponDataByCriteria AVERAGE SERIALIZATION TIME (in seconds)", avgSerializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Promotions_Coupon_PerformanceTest: GetManyCouponDataByCriteria -- Could not serialize. NO AVERAGE SERIALIZATION TIME AVAILABLE.");
                }
                if (numDeserializations > 0)
                {
                    avgDeserializeDataSecs /= numDeserializations;
                    TestHelper.ReportValueToConsole("Promotions_Coupon_PerformanceTest: GetManyCouponDataByCriteria AVERAGE DESERIALIZATION TIME (in seconds)", avgDeserializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Promotions_Coupon_PerformanceTest: GetManyCouponDataByCriteria -- Could not deserialize. NO AVERAGE DESERIALIZATION TIME AVAILABLE.");
                }
                ...
                Console.WriteLine("******************** Promotions_Coupon_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Promotions_Coupon_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
