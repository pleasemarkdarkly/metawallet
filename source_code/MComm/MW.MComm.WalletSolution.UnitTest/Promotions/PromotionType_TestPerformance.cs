...
namespace MW.MComm.WalletSolution.UnitTest.Promotions
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Promotions"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Promotions.PromotionType))]
    public partial class PromotionType_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public PromotionType_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for PromotionType.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Promotions_PromotionType_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Promotions_PromotionType_PerformanceTest START ********************");
                ...
                Console.WriteLine("GetAllPromotionTypeData test count is {0}.", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Promotions.PromotionType> items = PromotionType_Manager.GetAllPromotionTypeDataTest(Results);
                        Console.WriteLine("GetAllPromotionTypeData succeeded for PromotionType.");
                        ...
                        items = (BLL.SortableList<BLL.Promotions.PromotionType>)oItems;
                        oItems = null;
                        items = null;
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllPromotionTypeData failed for PromotionType", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                Console.WriteLine("******************** Promotions_PromotionType_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Promotions_PromotionType_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
