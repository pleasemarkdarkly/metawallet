...
namespace MW.MComm.WalletSolution.UnitTest.Users
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Users"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Users.User))]
    public partial class User_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for User.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Users_User_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Users_User_BasicTest START ********************");
                ...
                BLL.Users.User item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Users_User_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Users_User_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Users_User_BasicTest: Add User count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = User_Manager.AddOneUserTest(true, Results);
                        ...
                        else
                        {
                            ...
                            entityID["UserID"] = item.UserID;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Users_User_BasicTest: Add succeeded for User", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for User", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for User", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if User Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Users_User_BasicTest: Update User count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = User_Manager.UpdateOneUserTest((Guid)loopID["UserID"], true, Results);
                        TestHelper.ReportValueToConsole("Users_User_BasicTest: Update succeeded for User", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for User", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Users_User_BasicTest: Delete User count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = User_Manager.DeleteOneUserTest((Guid)loopID["UserID"], true, Results);
                            TestHelper.ReportValueToConsole("Users_User_BasicTest: Delete succeeded for User", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for User", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Users_User_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Users_User_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
