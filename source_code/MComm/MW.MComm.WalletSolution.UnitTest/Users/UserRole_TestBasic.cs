...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for UserRole instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Users"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Users.UserRole))]
    public partial class UserRole_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public UserRole_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for UserRole.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Users_UserRole_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Users_UserRole_BasicTest START ********************");
                ...
                BLL.Users.UserRole item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Users_UserRole_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Users_UserRole_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Users_UserRole_BasicTest: Add UserRole count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = UserRole_Manager.AddOneUserRoleTest(true, Results);
                        if (Results.IsCollision == true)
                        {
                            throw new UnitTestException(UnitTestErrorType.Warning, "Add collision encountered for UserRole: " + item.PrimaryKey.ToString(), null);
                        }
                        else
                        {
                            MOD.Data.NamedObjectCollection entityID = new MOD.Data.NamedObjectCollection();
                            entityID["UserRoleCode"] = item.UserRoleCode;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Users_UserRole_BasicTest: Add succeeded for UserRole", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for UserRole", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for UserRole", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if UserRole Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Users_UserRole_BasicTest: Update UserRole count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = UserRole_Manager.UpdateOneUserRoleTest((int)loopID["UserRoleCode"], true, Results);
                        TestHelper.ReportValueToConsole("Users_UserRole_BasicTest: Update succeeded for UserRole", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for UserRole", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Users_UserRole_BasicTest: Delete UserRole count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = UserRole_Manager.DeleteOneUserRoleTest((int)loopID["UserRoleCode"], true, Results);
                            TestHelper.ReportValueToConsole("Users_UserRole_BasicTest: Delete succeeded for UserRole", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for UserRole", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Users_UserRole_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Users_UserRole_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
