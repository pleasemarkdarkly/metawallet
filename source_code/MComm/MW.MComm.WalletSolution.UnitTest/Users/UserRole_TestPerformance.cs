...
namespace MW.MComm.WalletSolution.UnitTest.Users
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Users"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Users.UserRole))]
    public partial class UserRole_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public UserRole_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for UserRole.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Users_UserRole_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Users_UserRole_PerformanceTest START ********************");
                ...
                Console.WriteLine("GetAllUserRoleData test count is {0}.", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Users.UserRole> items = UserRole_Manager.GetAllUserRoleDataTest(Results);
                        Console.WriteLine("GetAllUserRoleData succeeded for UserRole.");
                        ...
                        items = (BLL.SortableList<BLL.Users.UserRole>)oItems;
                        oItems = null;
                        items = null;
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllUserRoleData failed for UserRole", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                Console.WriteLine("******************** Users_UserRole_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Users_UserRole_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
