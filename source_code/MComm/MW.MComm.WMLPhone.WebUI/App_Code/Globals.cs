...
namespace MW.MComm.WMLPhone.WebUI
{
  ...
  public class Globals : ApplicationOptionsProvider
  {
    ...
    public static BLL.Customers.MetaPartnerPhone CurrentPhone
    {
      get
      {
        BLL.Customers.MetaPartnerPhone phone = null;
        try
        {
          phone = (BLL.Customers.MetaPartnerPhone)HttpContext.Current.Session["CurrentPhone"];
        }
        catch
        {
        }
      ...
      set
      {
        if (value == null)
        {
          HttpContext.Current.Session.Remove("CurrentPhone");
        }
        else
        {
          HttpContext.Current.Session["CurrentPhone"] = value;
          BLL.Customers.MetaPartnerPhone phone = value;
        }
      }
    }
    public static BLL.Customers.MetaPartner CurrentPartner
    {
      get
      {
        BLL.Customers.MetaPartner partner = null;
        try
        {
          partner = (BLL.Customers.MetaPartner)HttpContext.Current.Session["CurrentPartner"];
        }
        catch
        {
        }
      ...
      set
      {
        if (value == null)
        {
          HttpContext.Current.Session.Remove("CurrentPartner");
        }
        else
        {
          HttpContext.Current.Session["CurrentPartner"] = value;
          BLL.Customers.MetaPartner partner = value;
        }
      }
    }
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    /// <summary>This method retrieves the best text for a site label.</summary>
    // ------------------------------------------------------------------
    public static string GetSiteLabel(BLL.UserExperience.SiteLabelDefinitionCode siteLabelDefinitionCode)
    {
      int localeCode = (int)BLL.Environments.LocaleCode.EnglishUSA;
      if (CurrentPartner != null && CurrentPartner.LocaleCode != null)
      {
        localeCode = (int)CurrentPartner.LocaleCode;
      }
      // get the best matching site label
      return BLL.UserExperience.SiteLabelManager.GetSiteLabelText((int)siteLabelDefinitionCode, localeCode);
    }
    // ------------------------------------------------------------------
    /// <summary>Get the selected object from the application cache.</summary>
    /// 
