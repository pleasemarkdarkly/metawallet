...
namespace MW.MComm.Ganadero.WebService.Events
{
	...
	public class AuditLogManager : System.Web.Services.WebService
	{
		...
		///
		/// <param name="item">The AuditLog to be managed.</param>
		/// <param name="performCascadeOperation">Flag, indicating if a cascade operation should be performed on the AuditLog item.</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Events.AuditLogResults LogOneAuditLog(BLL.Events.AuditLog item, bool performCascadeOperation)
		{
			Components.Events.AuditLogResults results = new Components.Events.AuditLogResults();
			results.StatusDescription = "Results OK";
			try
			{
				// perform main method tasks
				BLL.Events.AuditLogManager.LogOneAuditLog(item, performCascadeOperation);
				results.Results.Add(item);
			}
			catch (System.Exception ex)
			{
