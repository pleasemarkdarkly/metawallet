...
{
	// ------------------------------------------------------------------------------
	/// <summary>Search Audit Log Data is used to help manage AuditLog information.</summary>
	///
	/// File History:
	...
	/// <remarks></remarks>
	// ------------------------------------------------------------------------------
	public partial class SearchAuditLogData  : Components.Desktop.BaseSearchUserControl
	{
		#region Fields
		#endregion Fields
		#region Properties
		...
		public bool UseValidation
		{
			...
			set
			{
				...
				editAuditLog.UseValidation = value;
			}
		}
		...
		// ------------------------------------------------------------------------------
		/// <summary>Load page for Audit Log search.</summary>
		///
		// ------------------------------------------------------------------------------
		private void Page_Load(object sender, System.EventArgs e)
		{
			try
			{
				workingAuditLogData.Visible = ProvideWorkingSet;
				base.OnLoad(); // base list control manages loading data
				if (WorkflowMode != WorkflowMode.Internal)
				{
					if (Session["SearchEvents/AuditLogData"] != null)
					{
						_collection = (BLL.SortableList<BLL.Events.AuditLog>)Session["SearchEvents/AuditLogData"];
					}
				}
				// Set current item into child control
				if (_selectedIndex > -1)
				{
					this.editAuditLog.AuditLogItem = (BLL.Events.AuditLog) Collection[_selectedIndex];
				}
				this.editAuditLog.AuditLogAdded += new EntityEditEventHandler(editAuditLog_AuditLogAdded);
				this.editAuditLog.AuditLogRemoved += new EntityEditEventHandler(editAuditLog_AuditLogRemoved);
			}
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "SearchAuditLogData.Page_Load"));
			}
			finally
			{
			}
		...
		// ------------------------------------------------------------------------------
		/// <summary>Item added event handler for Audit Log.</summary>
		///
		// ------------------------------------------------------------------------------
		private void editAuditLog_AuditLogAdded(BaseFormlUserControl sender, EntityEditEventArgs e)
		{
			if (_collection.Contains(e.Entity))
			{
				Globals.AddErrorMessage(this.Page, "Collection already contains this Audit Log", new object[0]);
			}
			else
			{
		...
		// ------------------------------------------------------------------------------
		/// <summary>Item removed event handler for Audit Log.</summary>
		///
		// ------------------------------------------------------------------------------
		private void editAuditLog_AuditLogRemoved(BaseFormlUserControl sender, EntityEditEventArgs e)
		{
			_collection.Add(e.Entity);
		}
		// ------------------------------------------------------------------------------
		/// <summary>Binding for a list of Audit Log.</summary>
		///
		// ------------------------------------------------------------------------------
		private void dgPrimaryList_ItemDataBound(object sender, DataGridItemEventArgs e)
		{
			try
			{
				...
				{
					BLL.Events.AuditLog vAuditLog = (BLL.Events.AuditLog)e.Item.DataItem;
					SetControlText(e.Item,"lblPrimaryName", MOD.Data.DataHelper.GetString(vAuditLog.PrimaryName, ""));
					SetControlText(e.Item,"lblEventCode", MOD.Data.DataHelper.GetString(vAuditLog.EventCode, ""));
					SetControlText(e.Item,"lblCreatedDate", MOD.Data.DataHelper.GetString(vAuditLog.CreatedDate, ""));
					SetControlText(e.Item,"lblLastModifiedDate", MOD.Data.DataHelper.GetString(vAuditLog.LastModifiedDate, ""));
					SetControlText(e.Item,"lblEventName", MOD.Data.DataHelper.GetString(vAuditLog.EventName, MOD.Data.DefaultValue.String));
					...
					foreach (DataGridColumn loopColumn in dgPrimaryList.Columns)
					{
						...
						{
							loopColumn.SortExpression = vAuditLog.PrimarySortColumn;
							break;
						}
					}
					...
					else
					{
						...
						lnkEditOut.NavigateUrl = Page.ResolveUrl(string.Format("EditAuditLog.aspx?mode=" + modeValue + "&message=&AuditLogID={0}", vAuditLog.AuditLogID));
					}
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "SearchAuditLogData.dgPrimaryList_ItemDataBound"));
			}
			finally
			{
			}
		...
		// ------------------------------------------------------------------------------
		/// <summary>Set Audit Log information from the ui.</summary>
		///
		// ------------------------------------------------------------------------------
		...
		// ------------------------------------------------------------------------------
		/// <summary>Get Audit Log information from the database.</summary>
		///
		// ------------------------------------------------------------------------------
		protected override void LoadCollectionFromDatabase()
		{
			try
			{
				...
				{
					_collection = BLL.Events.AuditLogManager.GetManyAuditLogDataByCriteria(eventCode, lastModifiedDateStart, lastModifiedDateEnd, out totalRecords, out maximumListSizeExceeded, this.DataOptions);
					Session["SearchEvents/AuditLogData"] = _collection;
		...
		// ------------------------------------------------------------------------------
		/// <summary>Item command event handler for Audit Log list.</summary>
		///
		// ------------------------------------------------------------------------------
		public void dgPrimaryList_ItemCommand(object sender, EventArgs e)
		{
			...
			if (args.CommandName == "Add" || args.CommandName == "Edit" || args.CommandName == "Delete")
			{
				...
				BLL.Events.AuditLog item = ((BLL.SortableList<BLL.Events.AuditLog>)_collection).Find(hashCode);
				if (args.CommandName == "Add")
				{
					workingAuditLogData.AddItem(item);
				}
				else if (args.CommandName == "Edit")
				{
					this.editAuditLog.AuditLogItem = item;
				}
		...
		// ------------------------------------------------------------------------------
		/// <summary>Page pre render event handler for Audit Log.</summary>
		///
		// ------------------------------------------------------------------------------
		private void Page_PreRender(object sender, EventArgs e)
		{
			try
			{
				...
				dgPrimaryList.DataKeyField = "AuditLogID";
				dgPrimaryList.DataBind();
				if (this.WorkflowMode == MOD.Data.WorkflowMode.Internal)
				{
					editAuditLog.Visible = true;
					btnNew.Visible = false;
				}
				else
				{
					editAuditLog.Visible = false;
					btnNew.Visible = true;
				}
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "SearchAuditLogData.Page_PreRender"));
			}
			finally
			{
			}
		...
		private void btnNew_Click(object sender, EventArgs e)
		{
			...
			Response.Redirect(Page.ResolveUrl(string.Format("EditAuditLog.aspx?mode=" + MOD.Data.AccessMode.Add.ToString() + "&AuditLogID=" + MOD.Data.DataHelper.GetString(MOD.Data.DefaultValue.Guid, "") + "")));
		}
		
		// ------------------------------------------------------------------------------
		/// <summary>Clear search form for Audit Log.</summary>
		///
		// ------------------------------------------------------------------------------
		...
		// ------------------------------------------------------------------------------
		/// <summary>Perform search for Audit Log items.</summary>
		///
		// ------------------------------------------------------------------------------
		private void btnSearch_Click(object sender, EventArgs e)
		{
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "SearchAuditLogData.btnSearch_Click"));
			}
			finally
			{
			}
