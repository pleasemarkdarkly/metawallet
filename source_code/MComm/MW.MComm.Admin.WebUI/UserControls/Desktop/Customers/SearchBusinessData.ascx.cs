...
{
	// ------------------------------------------------------------------------------
	/// <summary>Search Business Data is used to help manage Business information.</summary>
	///
	/// File History:
	...
	/// <remarks></remarks>
	// ------------------------------------------------------------------------------
	public partial class SearchBusinessData  : Components.Desktop.BaseSearchUserControl
	{
		#region Fields
		#endregion Fields
		#region Properties
		...
		public bool UseValidation
		{
			...
			set
			{
				...
				editBusiness.UseValidation = value;
			}
		}
		...
		// ------------------------------------------------------------------------------
		/// <summary>Load page for Business search.</summary>
		///
		// ------------------------------------------------------------------------------
		private void Page_Load(object sender, System.EventArgs e)
		{
			try
			{
				workingBusinessData.Visible = ProvideWorkingSet;
				base.OnLoad(); // base list control manages loading data
				if (WorkflowMode != WorkflowMode.Internal)
				{
					if (Session["SearchCustomers/BusinessData"] != null)
					{
						_collection = (BLL.SortableList<BLL.Customers.Business>)Session["SearchCustomers/BusinessData"];
					}
				}
				// Set current item into child control
				if (_selectedIndex > -1)
				{
					this.editBusiness.BusinessItem = (BLL.Customers.Business) Collection[_selectedIndex];
				}
				this.editBusiness.BusinessAdded += new EntityEditEventHandler(editBusiness_BusinessAdded);
				this.editBusiness.BusinessRemoved += new EntityEditEventHandler(editBusiness_BusinessRemoved);
			}
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "SearchBusinessData.Page_Load"));
			}
			finally
			{
			}
		...
		// ------------------------------------------------------------------------------
		/// <summary>Item added event handler for Business.</summary>
		///
		// ------------------------------------------------------------------------------
		private void editBusiness_BusinessAdded(BaseFormlUserControl sender, EntityEditEventArgs e)
		{
			if (_collection.Contains(e.Entity))
			{
				Globals.AddErrorMessage(this.Page, "Collection already contains this Business", new object[0]);
			}
			else
			{
		...
		// ------------------------------------------------------------------------------
		/// <summary>Item removed event handler for Business.</summary>
		///
		// ------------------------------------------------------------------------------
		private void editBusiness_BusinessRemoved(BaseFormlUserControl sender, EntityEditEventArgs e)
		{
			_collection.Add(e.Entity);
		}
		// ------------------------------------------------------------------------------
		/// <summary>Binding for a list of Business.</summary>
		///
		// ------------------------------------------------------------------------------
		private void dgPrimaryList_ItemDataBound(object sender, DataGridItemEventArgs e)
		{
			try
			{
				...
				{
					BLL.Customers.Business vBusiness = (BLL.Customers.Business)e.Item.DataItem;
					SetControlText(e.Item,"lblPrimaryName", MOD.Data.DataHelper.GetString(vBusiness.PrimaryName, ""));
					SetControlText(e.Item,"lblServiceCharges", MOD.Data.DataHelper.GetString(vBusiness.ServiceCharges, ""));
					SetControlText(e.Item,"lblCreatedDate", MOD.Data.DataHelper.GetString(vBusiness.CreatedDate, ""));
					SetControlText(e.Item,"lblLastModifiedDate", MOD.Data.DataHelper.GetString(vBusiness.LastModifiedDate, ""));
					SetControlText(e.Item,"lblMetaPartnerSubTypeCode", MOD.Data.DataHelper.GetString(vBusiness.MetaPartnerSubTypeCode, ""));
					SetControlText(e.Item,"lblLocaleCode", MOD.Data.DataHelper.GetString(vBusiness.LocaleCode, ""));
					SetControlText(e.Item,"lblCurrencyCode", MOD.Data.DataHelper.GetString(vBusiness.CurrencyCode, ""));
					SetControlText(e.Item,"lblDateFormatCode", MOD.Data.DataHelper.GetString(vBusiness.DateFormatCode, ""));
					SetControlText(e.Item,"lblTaxCode", MOD.Data.DataHelper.GetString(vBusiness.TaxCode, ""));
					SetControlText(e.Item,"lblIsActive", MOD.Data.DataHelper.GetString(vBusiness.IsActive, ""));
					SetControlText(e.Item,"lblMetaPartnerName", MOD.Data.DataHelper.GetString(vBusiness.MetaPartnerName, ""));
					System.Web.UI.WebControls.Image lblPictureImageURL = (System.Web.UI.WebControls.Image)e.Item.FindControl("lblPictureImageURL");
					lblPictureImageURL.ImageUrl = CoreUtility.FileManager.GetAbsoluteHttpUrl(CoreUtility.FileGroupCode.Customers, vBusiness.PictureImageURL, Page.ResolveUrl("~/images/missing_image-75.jpg"));
					HyperLink lnkPictureImageURL = (HyperLink)e.Item.FindControl("lnkPictureImageURL");
					lnkPictureImageURL.NavigateUrl = Page.ResolveUrl(CoreUtility.FileManager.GetAbsoluteHttpUrl(CoreUtility.FileGroupCode.Customers, vBusiness.PictureImageURL, Page.ResolveUrl("~/images/missing_image-75.jpg")));
					...
					foreach (DataGridColumn loopColumn in dgPrimaryList.Columns)
					{
						...
						{
							loopColumn.SortExpression = vBusiness.PrimarySortColumn;
							break;
						}
					}
					...
					else
					{
						...
						lnkEditOut.NavigateUrl = Page.ResolveUrl(string.Format("EditBusiness.aspx?mode=" + modeValue + "&message=&MetaPartnerID={0}&BusinessMetaPartnerID={1}", vBusiness.MetaPartnerID, vBusiness.MetaPartnerID));
					}
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "SearchBusinessData.dgPrimaryList_ItemDataBound"));
			}
			finally
			{
			}
		...
		// ------------------------------------------------------------------------------
		/// <summary>Set Business information from the ui.</summary>
		///
		// ------------------------------------------------------------------------------
		...
		// ------------------------------------------------------------------------------
		/// <summary>Get Business information from the database.</summary>
		///
		// ------------------------------------------------------------------------------
		protected override void LoadCollectionFromDatabase()
		{
			try
			{
				...
				{
					_collection = BLL.Customers.BusinessManager.GetManyBusinessDataByCriteria(localeCode, currencyCode, dateFormatCode, lastModifiedDateStart, lastModifiedDateEnd, metaPartnerName, out totalRecords, out maximumListSizeExceeded, this.DataOptions);
					Session["SearchCustomers/BusinessData"] = _collection;
		...
		// ------------------------------------------------------------------------------
		/// <summary>Item command event handler for Business list.</summary>
		///
		// ------------------------------------------------------------------------------
		public void dgPrimaryList_ItemCommand(object sender, EventArgs e)
		{
			...
			if (args.CommandName == "Add" || args.CommandName == "Edit" || args.CommandName == "Delete")
			{
				...
				BLL.Customers.Business item = ((BLL.SortableList<BLL.Customers.Business>)_collection).Find(hashCode);
				if (args.CommandName == "Add")
				{
					workingBusinessData.AddItem(item);
				}
				else if (args.CommandName == "Edit")
				{
					this.editBusiness.BusinessItem = item;
				}
		...
		// ------------------------------------------------------------------------------
		/// <summary>Page pre render event handler for Business.</summary>
		///
		// ------------------------------------------------------------------------------
		private void Page_PreRender(object sender, EventArgs e)
		{
			try
			{
				...
				if (this.WorkflowMode == MOD.Data.WorkflowMode.Internal)
				{
					editBusiness.Visible = true;
					btnNew.Visible = false;
				}
				else
				{
					editBusiness.Visible = false;
					btnNew.Visible = true;
				}
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "SearchBusinessData.Page_PreRender"));
			}
			finally
			{
			}
		...
		private void btnNew_Click(object sender, EventArgs e)
		{
			...
			Response.Redirect(Page.ResolveUrl(string.Format("EditBusiness.aspx?mode=" + MOD.Data.AccessMode.Add.ToString() + "&MetaPartnerID=" + MOD.Data.DataHelper.GetString(MOD.Data.DefaultValue.Guid, "") + "&BusinessMetaPartnerID=" + MOD.Data.DataHelper.GetString(MOD.Data.DefaultValue.Guid, "") + "")));
		}
		
		// ------------------------------------------------------------------------------
		/// <summary>Clear search form for Business.</summary>
		///
		// ------------------------------------------------------------------------------
		...
		// ------------------------------------------------------------------------------
		/// <summary>Perform search for Business items.</summary>
		///
		// ------------------------------------------------------------------------------
		private void btnSearch_Click(object sender, EventArgs e)
		{
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "SearchBusinessData.btnSearch_Click"));
			}
			finally
			{
			}
