...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for SiteLabel instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.UserExperience"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.UserExperience.SiteLabel))]
    public partial class SiteLabel_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public SiteLabel_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for SiteLabel.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void UserExperience_SiteLabel_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** UserExperience_SiteLabel_PerformanceTest START ********************");
                ...
                TestHelper.ReportValueToConsole("UserExperience_SiteLabel_PerformanceTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                {
                    TestHelper.ReportValueToConsole("UserExperience_SiteLabel_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString((UnitTestPerformanceLevel)(i + 1)) + " time (in seconds)", perfTimeLevels[i]);
                }
                TestHelper.ReportValueToConsole("UserExperience_SiteLabel_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString(UnitTestPerformanceLevel.Unacceptable) + " time (in seconds) is beyond", perfTimeLevels[(((int)UnitTestPerformanceLevel.Unacceptable - 1) - 1)]);
                ...
                TestHelper.ReportValueToConsole("UserExperience_SiteLabel_PerformanceTest: page size", pageSize);
                TestHelper.ReportValueToConsole("UserExperience_SiteLabel_PerformanceTest: maxListSize", maxListSize);
                ...
                TestHelper.ReportValueToConsole("UserExperience_SiteLabel_PerformanceTest: GetManySiteLabelDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    long startTicks = 0;
                    long endTicks = 0;
                    double secondsInterval = 0.00;
                    BLL.SortableList<BLL.UserExperience.SiteLabel> items = null;

                    TestHelper.ReportValueToConsole("UserExperience_SiteLabel_PerformanceTest: Loop Iteration", i);
                    ...
                    TestHelper.ReportValueToConsole("UserExperience_SiteLabel_PerformanceTest: GetManySiteLabelDataByCriteria acceptable search time (in seconds)", acceptableTime);
                    try
                    {
                        // get many
                        Results.ResetCurrentPerfMetric();
                        Results.SetPerfMetricName("UserExperience.SiteLabel.GetMany." + i.ToString());
                        items = SiteLabel_Manager.GetManySiteLabelDataByCriteriaTest(dbOptions, Results);
                        TestHelper.ReportValueToConsole("UserExperience_SiteLabel_PerformanceTest: GetManySiteLabelDataByCriteria succeeded for SiteLabel. The count", items.Count.ToString());
                        ...
                        perfStr = "GetManySiteLabelDataByCriteria search time (in seconds) for SiteLabel count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional,"GetManySiteLabelDataByCriteria failed for SiteLabel", itemStr, ex, strExtraInfo);
                        }
                    }

                    if (items != null)
                    {
                        TestHelper.ReportValueToConsole("UserExperience_SiteLabel_PerformanceTest: GetManySiteLabelDataByCriteria acceptable serialization time (in seconds)", acceptableTime);
                        try
                        {
                            // serialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("UserExperience.SiteLabel.Serialize." + i.ToString());
                            ...
                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManySiteLabelDataByCriteria serialization time (in seconds) for SiteLabel count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                        ...
                        TestHelper.ReportValueToConsole("UserExperience_SiteLabel_PerformanceTest: GetManySiteLabelDataByCriteria acceptable deserialization time (in seconds)", acceptableTime);
                        try
                        {
                            // deserialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("UserExperience.SiteLabel.Deserialize." + i.ToString());
                            ...
                            items = (BLL.SortableList<BLL.UserExperience.SiteLabel>)oItems;
                            int itemCount = items.Count;
                            oItems = null;
                            items = null;

                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManySiteLabelDataByCriteria deserialization time (in seconds) for SiteLabel count " + itemCount + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                ...
                if (numGetDataSearches > 0)
                {
                    ...
                    TestHelper.ReportValueToConsole("UserExperience_SiteLabel_PerformanceTest: GetManySiteLabelDataByCriteria AVERAGE SEARCH TIME (in seconds)", avgGetDataSearchSecs);
                }
                else
                {
                    Console.WriteLine("UserExperience_SiteLabel_PerformanceTest: GetManySiteLabelDataByCriteria -- Could not perform search. NO AVERAGE SEARCH TIME AVAILABLE.");
                }
                if (numSerializations > 0)
                {
                    avgSerializeDataSecs /= numSerializations;
                    TestHelper.ReportValueToConsole("UserExperience_SiteLabel_PerformanceTest: GetManySiteLabelDataByCriteria AVERAGE SERIALIZATION TIME (in seconds)", avgSerializeDataSecs);
                }
                else
                {
                    Console.WriteLine("UserExperience_SiteLabel_PerformanceTest: GetManySiteLabelDataByCriteria -- Could not serialize. NO AVERAGE SERIALIZATION TIME AVAILABLE.");
                }
                if (numDeserializations > 0)
                {
                    avgDeserializeDataSecs /= numDeserializations;
                    TestHelper.ReportValueToConsole("UserExperience_SiteLabel_PerformanceTest: GetManySiteLabelDataByCriteria AVERAGE DESERIALIZATION TIME (in seconds)", avgDeserializeDataSecs);
                }
                else
                {
                    Console.WriteLine("UserExperience_SiteLabel_PerformanceTest: GetManySiteLabelDataByCriteria -- Could not deserialize. NO AVERAGE DESERIALIZATION TIME AVAILABLE.");
                }
                ...
                Console.WriteLine("******************** UserExperience_SiteLabel_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== UserExperience_SiteLabel_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
