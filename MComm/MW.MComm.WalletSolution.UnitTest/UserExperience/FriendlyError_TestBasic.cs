...
namespace MW.MComm.WalletSolution.UnitTest.UserExperience
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.UserExperience"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.UserExperience.FriendlyError))]
    public partial class FriendlyError_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public FriendlyError_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for FriendlyError.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void UserExperience_FriendlyError_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** UserExperience_FriendlyError_BasicTest START ********************");
                ...
                BLL.UserExperience.FriendlyError item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("UserExperience_FriendlyError_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("UserExperience_FriendlyError_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("UserExperience_FriendlyError_BasicTest: Add FriendlyError count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = FriendlyError_Manager.AddOneFriendlyErrorTest(true, Results);
                        ...
                        else
                        {
                            ...
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("UserExperience_FriendlyError_BasicTest: Add succeeded for FriendlyError", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.UserExperience.FriendlyError)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for FriendlyError", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for FriendlyError", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if FriendlyError Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("UserExperience_FriendlyError_BasicTest: Update FriendlyError count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = FriendlyError_Manager.UpdateOneFriendlyErrorTest((int)loopID["ErrorNumber"], (int)loopID["LocaleCode"], true, Results);
                        TestHelper.ReportValueToConsole("UserExperience_FriendlyError_BasicTest: Update succeeded for FriendlyError", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for FriendlyError", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("UserExperience_FriendlyError_BasicTest: Delete FriendlyError count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = FriendlyError_Manager.DeleteOneFriendlyErrorTest((int)loopID["ErrorNumber"], (int)loopID["LocaleCode"], true, Results);
                            TestHelper.ReportValueToConsole("UserExperience_FriendlyError_BasicTest: Delete succeeded for FriendlyError", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for FriendlyError", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** UserExperience_FriendlyError_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== UserExperience_FriendlyError_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
