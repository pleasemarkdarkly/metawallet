...
namespace MW.MComm.WalletSolution.UnitTest.UserExperience
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.UserExperience"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.UserExperience.AdminHelp))]
    public partial class AdminHelp_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public AdminHelp_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for AdminHelp.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void UserExperience_AdminHelp_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** UserExperience_AdminHelp_BasicTest START ********************");
                ...
                BLL.UserExperience.AdminHelp item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("UserExperience_AdminHelp_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("UserExperience_AdminHelp_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("UserExperience_AdminHelp_BasicTest: Add AdminHelp count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = AdminHelp_Manager.AddOneAdminHelpTest(true, Results);
                        ...
                        else
                        {
                            ...
                            entityID["ActivityCode"] = item.ActivityCode;
                            entityID["LocaleCode"] = item.LocaleCode;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("UserExperience_AdminHelp_BasicTest: Add succeeded for AdminHelp", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.UserExperience.AdminHelp)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for AdminHelp", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for AdminHelp", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if AdminHelp Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("UserExperience_AdminHelp_BasicTest: Update AdminHelp count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = AdminHelp_Manager.UpdateOneAdminHelpTest((int)loopID["ActivityCode"], (int)loopID["LocaleCode"], true, Results);
                        TestHelper.ReportValueToConsole("UserExperience_AdminHelp_BasicTest: Update succeeded for AdminHelp", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for AdminHelp", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("UserExperience_AdminHelp_BasicTest: Delete AdminHelp count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = AdminHelp_Manager.DeleteOneAdminHelpTest((int)loopID["ActivityCode"], (int)loopID["LocaleCode"], true, Results);
                            TestHelper.ReportValueToConsole("UserExperience_AdminHelp_BasicTest: Delete succeeded for AdminHelp", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for AdminHelp", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** UserExperience_AdminHelp_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== UserExperience_AdminHelp_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
