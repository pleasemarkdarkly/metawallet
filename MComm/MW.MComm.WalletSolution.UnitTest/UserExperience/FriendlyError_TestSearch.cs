...
namespace MW.MComm.WalletSolution.UnitTest.UserExperience
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Search.UserExperience"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.UserExperience.FriendlyError))]
    public partial class FriendlyError_TestSearch : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public FriendlyError_TestSearch() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the search entity test for FriendlyError.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void UserExperience_FriendlyError_SearchTest()
        {
            try
            {
                Console.WriteLine("******************** UserExperience_FriendlyError_SearchTest START ********************");
                ...
                TestHelper.ReportValueToConsole("UserExperience_FriendlyError_SearchTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("UserExperience_FriendlyError_SearchTest: GetAllFriendlyErrorData test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.UserExperience.FriendlyError> items = FriendlyError_Manager.GetAllFriendlyErrorDataTest(Results);
                        TestHelper.ReportValueToConsole("UserExperience_FriendlyError_SearchTest: GetAllFriendlyErrorData succeeded for FriendlyError. The count", items.Count.ToString());
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllFriendlyErrorData failed for FriendlyError", itemStr, ex, strExtraInfo);
                        }
                    }
                }
                loadCount = TestHelper.GetInt(Globals.MinimumGetAllCountPerEntity, Globals.MaximumGetAllCountPerEntity);
                TestHelper.ReportValueToConsole("UserExperience_FriendlyError_SearchTest: GetManyFriendlyErrorDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.UserExperience.FriendlyError> items = FriendlyError_Manager.GetManyFriendlyErrorDataByCriteriaTest(Globals.getDataOptions("","Random",1,200,2000), Results);
                        TestHelper.ReportValueToConsole("UserExperience_FriendlyError_SearchTest: GetManyFriendlyErrorDataByCriteria succeeded for FriendlyError. The count", items.Count.ToString());
                ...
                Console.WriteLine("******************** UserExperience_FriendlyError_SearchTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== UserExperience_FriendlyError_SearchTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
