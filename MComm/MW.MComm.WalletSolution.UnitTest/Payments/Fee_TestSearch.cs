...
namespace MW.MComm.WalletSolution.UnitTest.Payments
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Search.Payments"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Payments.Fee))]
    public partial class Fee_TestSearch : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the search entity test for Fee.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Payments_Fee_SearchTest()
        {
            try
            {
                Console.WriteLine("******************** Payments_Fee_SearchTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Payments_Fee_SearchTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Payments_Fee_SearchTest: GetAllFeeData test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Payments.Fee> items = Fee_Manager.GetAllFeeDataTest(Results);
                        TestHelper.ReportValueToConsole("Payments_Fee_SearchTest: GetAllFeeData succeeded for Fee. The count", items.Count.ToString());
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllFeeData failed for Fee", itemStr, ex, strExtraInfo);
                        }
                    }
                }
                loadCount = TestHelper.GetInt(Globals.MinimumGetAllCountPerEntity, Globals.MaximumGetAllCountPerEntity);
                TestHelper.ReportValueToConsole("Payments_Fee_SearchTest: GetManyFeeDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Payments.Fee> items = Fee_Manager.GetManyFeeDataByCriteriaTest(Globals.getDataOptions("","Random",1,200,2000), Results);
                        TestHelper.ReportValueToConsole("Payments_Fee_SearchTest: GetManyFeeDataByCriteria succeeded for Fee. The count", items.Count.ToString());
                ...
                Console.WriteLine("******************** Payments_Fee_SearchTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Payments_Fee_SearchTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
