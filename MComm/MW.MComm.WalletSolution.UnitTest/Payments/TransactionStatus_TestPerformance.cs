...
namespace MW.MComm.WalletSolution.UnitTest.Payments
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Payments"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Payments.TransactionStatus))]
    public partial class TransactionStatus_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public TransactionStatus_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for TransactionStatus.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Payments_TransactionStatus_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Payments_TransactionStatus_PerformanceTest START ********************");
                ...
                Console.WriteLine("GetAllTransactionStatusData test count is {0}.", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Payments.TransactionStatus> items = TransactionStatus_Manager.GetAllTransactionStatusDataTest(Results);
                        Console.WriteLine("GetAllTransactionStatusData succeeded for TransactionStatus.");
                        ...
                        items = (BLL.SortableList<BLL.Payments.TransactionStatus>)oItems;
                        oItems = null;
                        items = null;
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllTransactionStatusData failed for TransactionStatus", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                Console.WriteLine("******************** Payments_TransactionStatus_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Payments_TransactionStatus_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
