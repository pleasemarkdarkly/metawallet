...
namespace MW.MComm.WalletSolution.UnitTest.Payments
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Payments"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Payments.PaymentTransactionLog))]
    public partial class PaymentTransactionLog_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public PaymentTransactionLog_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for PaymentTransactionLog.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Payments_PaymentTransactionLog_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Payments_PaymentTransactionLog_BasicTest START ********************");
                ...
                BLL.Payments.PaymentTransactionLog item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Payments_PaymentTransactionLog_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Payments_PaymentTransactionLog_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Payments_PaymentTransactionLog_BasicTest: Add PaymentTransactionLog count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = PaymentTransactionLog_Manager.AddOnePaymentTransactionLogTest(true, Results);
                        ...
                        else
                        {
                            ...
                            entityID["PaymentTransactionLogID"] = item.PaymentTransactionLogID;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Payments_PaymentTransactionLog_BasicTest: Add succeeded for PaymentTransactionLog", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Payments.PaymentTransactionLog)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for PaymentTransactionLog", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for PaymentTransactionLog", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if PaymentTransactionLog Add count is not the same as Update count
                ...
                Console.WriteLine("******************** Payments_PaymentTransactionLog_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Payments_PaymentTransactionLog_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
