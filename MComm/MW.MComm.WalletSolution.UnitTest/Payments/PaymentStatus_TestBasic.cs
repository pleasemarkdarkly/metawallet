...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for PaymentStatus instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Payments"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Payments.PaymentStatus))]
    public partial class PaymentStatus_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public PaymentStatus_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for PaymentStatus.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Payments_PaymentStatus_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Payments_PaymentStatus_BasicTest START ********************");
                ...
                BLL.Payments.PaymentStatus item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Payments_PaymentStatus_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Payments_PaymentStatus_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Payments_PaymentStatus_BasicTest: Add PaymentStatus count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = PaymentStatus_Manager.AddOnePaymentStatusTest(true, Results);
                        if (Results.IsCollision == true)
                        {
                            throw new UnitTestException(UnitTestErrorType.Warning, "Add collision encountered for PaymentStatus: " + item.PrimaryKey.ToString(), null);
                        }
                        else
                        {
                            MOD.Data.NamedObjectCollection entityID = new MOD.Data.NamedObjectCollection();
                            entityID["PaymentStatusCode"] = item.PaymentStatusCode;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Payments_PaymentStatus_BasicTest: Add succeeded for PaymentStatus", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Payments.PaymentStatus)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for PaymentStatus", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for PaymentStatus", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if PaymentStatus Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Payments_PaymentStatus_BasicTest: Update PaymentStatus count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = PaymentStatus_Manager.UpdateOnePaymentStatusTest((int)loopID["PaymentStatusCode"], true, Results);
                        TestHelper.ReportValueToConsole("Payments_PaymentStatus_BasicTest: Update succeeded for PaymentStatus", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for PaymentStatus", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Payments_PaymentStatus_BasicTest: Delete PaymentStatus count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = PaymentStatus_Manager.DeleteOnePaymentStatusTest((int)loopID["PaymentStatusCode"], true, Results);
                            TestHelper.ReportValueToConsole("Payments_PaymentStatus_BasicTest: Delete succeeded for PaymentStatus", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for PaymentStatus", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Payments_PaymentStatus_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Payments_PaymentStatus_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
