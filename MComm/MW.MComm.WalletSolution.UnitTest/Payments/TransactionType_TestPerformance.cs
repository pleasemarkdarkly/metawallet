...
namespace MW.MComm.WalletSolution.UnitTest.Payments
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Payments"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Payments.TransactionType))]
    public partial class TransactionType_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public TransactionType_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for TransactionType.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Payments_TransactionType_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Payments_TransactionType_PerformanceTest START ********************");
                ...
                Console.WriteLine("GetAllTransactionTypeData test count is {0}.", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Payments.TransactionType> items = TransactionType_Manager.GetAllTransactionTypeDataTest(Results);
                        Console.WriteLine("GetAllTransactionTypeData succeeded for TransactionType.");
                        ...
                        items = (BLL.SortableList<BLL.Payments.TransactionType>)oItems;
                        oItems = null;
                        items = null;
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllTransactionTypeData failed for TransactionType", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                Console.WriteLine("******************** Payments_TransactionType_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Payments_TransactionType_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
