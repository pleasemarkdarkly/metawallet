...
namespace MW.MComm.WalletSolution.UnitTest.Events
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Events"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Events.Event))]
    public partial class Event_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for Event.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Events_Event_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Events_Event_BasicTest START ********************");
                ...
                BLL.Events.Event item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Events_Event_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Events_Event_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Events_Event_BasicTest: Add Event count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = Event_Manager.AddOneEventTest(true, Results);
                        ...
                        else
                        {
                            ...
                            entityID["EventCode"] = item.EventCode;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Events_Event_BasicTest: Add succeeded for Event", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for Event", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for Event", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if Event Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Events_Event_BasicTest: Update Event count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = Event_Manager.UpdateOneEventTest((int)loopID["EventCode"], true, Results);
                        TestHelper.ReportValueToConsole("Events_Event_BasicTest: Update succeeded for Event", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for Event", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Events_Event_BasicTest: Delete Event count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = Event_Manager.DeleteOneEventTest((int)loopID["EventCode"], true, Results);
                            TestHelper.ReportValueToConsole("Events_Event_BasicTest: Delete succeeded for Event", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for Event", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Events_Event_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Events_Event_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
