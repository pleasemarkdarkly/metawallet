...
namespace MW.MComm.WalletSolution.UnitTest.Events
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Events"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Events.EventType))]
    public partial class EventType_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public EventType_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for EventType.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Events_EventType_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Events_EventType_PerformanceTest START ********************");
                ...
                Console.WriteLine("GetAllEventTypeData test count is {0}.", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Events.EventType> items = EventType_Manager.GetAllEventTypeDataTest(Results);
                        Console.WriteLine("GetAllEventTypeData succeeded for EventType.");
                        ...
                        items = (BLL.SortableList<BLL.Events.EventType>)oItems;
                        oItems = null;
                        items = null;
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllEventTypeData failed for EventType", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                Console.WriteLine("******************** Events_EventType_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Events_EventType_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
