...
namespace MW.MComm.WalletSolution.UnitTest.Events
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Events"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Events.AuditLog))]
    public partial class AuditLog_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public AuditLog_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for AuditLog.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Events_AuditLog_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Events_AuditLog_BasicTest START ********************");
                ...
                BLL.Events.AuditLog item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Events_AuditLog_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Events_AuditLog_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Events_AuditLog_BasicTest: Add AuditLog count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = AuditLog_Manager.AddOneAuditLogTest(true, Results);
                        ...
                        else
                        {
                            ...
                            entityID["AuditLogID"] = item.AuditLogID;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Events_AuditLog_BasicTest: Add succeeded for AuditLog", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Events.AuditLog)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for AuditLog", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for AuditLog", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if AuditLog Add count is not the same as Update count
                ...
                Console.WriteLine("******************** Events_AuditLog_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Events_AuditLog_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
