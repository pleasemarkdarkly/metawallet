...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for SeverityLevel instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Core"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Core.SeverityLevel))]
    public partial class SeverityLevel_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public SeverityLevel_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for SeverityLevel.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Core_SeverityLevel_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Core_SeverityLevel_BasicTest START ********************");
                ...
                BLL.Core.SeverityLevel item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Core_SeverityLevel_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Core_SeverityLevel_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Core_SeverityLevel_BasicTest: Add SeverityLevel count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = SeverityLevel_Manager.AddOneSeverityLevelTest(true, Results);
                        if (Results.IsCollision == true)
                        {
                            throw new UnitTestException(UnitTestErrorType.Warning, "Add collision encountered for SeverityLevel: " + item.PrimaryKey.ToString(), null);
                        }
                        else
                        {
                            MOD.Data.NamedObjectCollection entityID = new MOD.Data.NamedObjectCollection();
                            entityID["SeverityLevelCode"] = item.SeverityLevelCode;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Core_SeverityLevel_BasicTest: Add succeeded for SeverityLevel", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Core.SeverityLevel)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for SeverityLevel", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for SeverityLevel", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if SeverityLevel Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Core_SeverityLevel_BasicTest: Update SeverityLevel count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = SeverityLevel_Manager.UpdateOneSeverityLevelTest((int)loopID["SeverityLevelCode"], true, Results);
                        TestHelper.ReportValueToConsole("Core_SeverityLevel_BasicTest: Update succeeded for SeverityLevel", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for SeverityLevel", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Core_SeverityLevel_BasicTest: Delete SeverityLevel count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = SeverityLevel_Manager.DeleteOneSeverityLevelTest((int)loopID["SeverityLevelCode"], true, Results);
                            TestHelper.ReportValueToConsole("Core_SeverityLevel_BasicTest: Delete succeeded for SeverityLevel", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for SeverityLevel", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Core_SeverityLevel_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Core_SeverityLevel_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
