...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for AttributeType instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Core"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Core.AttributeType))]
    public partial class AttributeType_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public AttributeType_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for AttributeType.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Core_AttributeType_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Core_AttributeType_BasicTest START ********************");
                ...
                BLL.Core.AttributeType item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Core_AttributeType_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Core_AttributeType_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Core_AttributeType_BasicTest: Add AttributeType count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = AttributeType_Manager.AddOneAttributeTypeTest(true, Results);
                        if (Results.IsCollision == true)
                        {
                            throw new UnitTestException(UnitTestErrorType.Warning, "Add collision encountered for AttributeType: " + item.PrimaryKey.ToString(), null);
                        }
                        else
                        {
                            MOD.Data.NamedObjectCollection entityID = new MOD.Data.NamedObjectCollection();
                            entityID["AttributeTypeCode"] = item.AttributeTypeCode;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Core_AttributeType_BasicTest: Add succeeded for AttributeType", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Core.AttributeType)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for AttributeType", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for AttributeType", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if AttributeType Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Core_AttributeType_BasicTest: Update AttributeType count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = AttributeType_Manager.UpdateOneAttributeTypeTest((int)loopID["AttributeTypeCode"], true, Results);
                        TestHelper.ReportValueToConsole("Core_AttributeType_BasicTest: Update succeeded for AttributeType", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for AttributeType", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Core_AttributeType_BasicTest: Delete AttributeType count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = AttributeType_Manager.DeleteOneAttributeTypeTest((int)loopID["AttributeTypeCode"], true, Results);
                            TestHelper.ReportValueToConsole("Core_AttributeType_BasicTest: Delete succeeded for AttributeType", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for AttributeType", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Core_AttributeType_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Core_AttributeType_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
