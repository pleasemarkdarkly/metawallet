...
namespace MW.MComm.WalletSolution.UnitTest.Core
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Search.Core"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Core.BaseAttribute))]
    public partial class BaseAttribute_TestSearch : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public BaseAttribute_TestSearch() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the search entity test for BaseAttribute.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Core_BaseAttribute_SearchTest()
        {
            try
            {
                Console.WriteLine("******************** Core_BaseAttribute_SearchTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Core_BaseAttribute_SearchTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Core_BaseAttribute_SearchTest: GetAllBaseAttributeData test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Core.BaseAttribute> items = BaseAttribute_Manager.GetAllBaseAttributeDataTest(Results);
                        TestHelper.ReportValueToConsole("Core_BaseAttribute_SearchTest: GetAllBaseAttributeData succeeded for BaseAttribute. The count", items.Count.ToString());
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllBaseAttributeData failed for BaseAttribute", itemStr, ex, strExtraInfo);
                        }
                    }
                }
                loadCount = TestHelper.GetInt(Globals.MinimumGetAllCountPerEntity, Globals.MaximumGetAllCountPerEntity);
                TestHelper.ReportValueToConsole("Core_BaseAttribute_SearchTest: GetManyBaseAttributeDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Core.BaseAttribute> items = BaseAttribute_Manager.GetManyBaseAttributeDataByCriteriaTest(Globals.getDataOptions("","Random",1,200,2000), Results);
                        TestHelper.ReportValueToConsole("Core_BaseAttribute_SearchTest: GetManyBaseAttributeDataByCriteria succeeded for BaseAttribute. The count", items.Count.ToString());
                ...
                Console.WriteLine("******************** Core_BaseAttribute_SearchTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Core_BaseAttribute_SearchTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
