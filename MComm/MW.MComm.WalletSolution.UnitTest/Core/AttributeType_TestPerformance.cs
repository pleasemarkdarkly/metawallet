...
namespace MW.MComm.WalletSolution.UnitTest.Core
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Core"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Core.AttributeType))]
    public partial class AttributeType_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public AttributeType_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for AttributeType.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Core_AttributeType_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Core_AttributeType_PerformanceTest START ********************");
                ...
                Console.WriteLine("GetAllAttributeTypeData test count is {0}.", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Core.AttributeType> items = AttributeType_Manager.GetAllAttributeTypeDataTest(Results);
                        Console.WriteLine("GetAllAttributeTypeData succeeded for AttributeType.");
                        ...
                        items = (BLL.SortableList<BLL.Core.AttributeType>)oItems;
                        oItems = null;
                        items = null;
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllAttributeTypeData failed for AttributeType", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                Console.WriteLine("******************** Core_AttributeType_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Core_AttributeType_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
