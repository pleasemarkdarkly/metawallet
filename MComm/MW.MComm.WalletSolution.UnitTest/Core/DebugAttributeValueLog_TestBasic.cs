...
namespace MW.MComm.WalletSolution.UnitTest.Core
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Core"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Core.DebugAttributeValueLog))]
    public partial class DebugAttributeValueLog_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public DebugAttributeValueLog_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for DebugAttributeValueLog.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Core_DebugAttributeValueLog_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Core_DebugAttributeValueLog_BasicTest START ********************");
                ...
                BLL.Core.DebugAttributeValueLog item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Core_DebugAttributeValueLog_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Core_DebugAttributeValueLog_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Core_DebugAttributeValueLog_BasicTest: Add DebugAttributeValueLog count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = DebugAttributeValueLog_Manager.AddOneDebugAttributeValueLogTest(true, Results);
                        ...
                        else
                        {
                            ...
                            entityID["DebugAttributeValueLogID"] = item.DebugAttributeValueLogID;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Core_DebugAttributeValueLog_BasicTest: Add succeeded for DebugAttributeValueLog", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Core.DebugAttributeValueLog)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for DebugAttributeValueLog", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for DebugAttributeValueLog", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if DebugAttributeValueLog Add count is not the same as Update count
                ...
                Console.WriteLine("******************** Core_DebugAttributeValueLog_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Core_DebugAttributeValueLog_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
