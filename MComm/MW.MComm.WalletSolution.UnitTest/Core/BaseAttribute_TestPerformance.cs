...
namespace MW.MComm.WalletSolution.UnitTest.Core
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Core"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Core.BaseAttribute))]
    public partial class BaseAttribute_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public BaseAttribute_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for BaseAttribute.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Core_BaseAttribute_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Core_BaseAttribute_PerformanceTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Core_BaseAttribute_PerformanceTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                {
                    TestHelper.ReportValueToConsole("Core_BaseAttribute_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString((UnitTestPerformanceLevel)(i + 1)) + " time (in seconds)", perfTimeLevels[i]);
                }
                TestHelper.ReportValueToConsole("Core_BaseAttribute_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString(UnitTestPerformanceLevel.Unacceptable) + " time (in seconds) is beyond", perfTimeLevels[(((int)UnitTestPerformanceLevel.Unacceptable - 1) - 1)]);
                ...
                TestHelper.ReportValueToConsole("Core_BaseAttribute_PerformanceTest: page size", pageSize);
                TestHelper.ReportValueToConsole("Core_BaseAttribute_PerformanceTest: maxListSize", maxListSize);
                ...
                TestHelper.ReportValueToConsole("Core_BaseAttribute_PerformanceTest: GetManyBaseAttributeDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    long startTicks = 0;
                    long endTicks = 0;
                    double secondsInterval = 0.00;
                    BLL.SortableList<BLL.Core.BaseAttribute> items = null;

                    TestHelper.ReportValueToConsole("Core_BaseAttribute_PerformanceTest: Loop Iteration", i);
                    ...
                    TestHelper.ReportValueToConsole("Core_BaseAttribute_PerformanceTest: GetManyBaseAttributeDataByCriteria acceptable search time (in seconds)", acceptableTime);
                    try
                    {
                        // get many
                        Results.ResetCurrentPerfMetric();
                        Results.SetPerfMetricName("Core.BaseAttribute.GetMany." + i.ToString());
                        items = BaseAttribute_Manager.GetManyBaseAttributeDataByCriteriaTest(dbOptions, Results);
                        TestHelper.ReportValueToConsole("Core_BaseAttribute_PerformanceTest: GetManyBaseAttributeDataByCriteria succeeded for BaseAttribute. The count", items.Count.ToString());
                        ...
                        perfStr = "GetManyBaseAttributeDataByCriteria search time (in seconds) for BaseAttribute count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional,"GetManyBaseAttributeDataByCriteria failed for BaseAttribute", itemStr, ex, strExtraInfo);
                        }
                    }

                    if (items != null)
                    {
                        TestHelper.ReportValueToConsole("Core_BaseAttribute_PerformanceTest: GetManyBaseAttributeDataByCriteria acceptable serialization time (in seconds)", acceptableTime);
                        try
                        {
                            // serialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Core.BaseAttribute.Serialize." + i.ToString());
                            ...
                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyBaseAttributeDataByCriteria serialization time (in seconds) for BaseAttribute count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                        ...
                        TestHelper.ReportValueToConsole("Core_BaseAttribute_PerformanceTest: GetManyBaseAttributeDataByCriteria acceptable deserialization time (in seconds)", acceptableTime);
                        try
                        {
                            // deserialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Core.BaseAttribute.Deserialize." + i.ToString());
                            ...
                            items = (BLL.SortableList<BLL.Core.BaseAttribute>)oItems;
                            int itemCount = items.Count;
                            oItems = null;
                            items = null;

                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyBaseAttributeDataByCriteria deserialization time (in seconds) for BaseAttribute count " + itemCount + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                ...
                if (numGetDataSearches > 0)
                {
                    ...
                    TestHelper.ReportValueToConsole("Core_BaseAttribute_PerformanceTest: GetManyBaseAttributeDataByCriteria AVERAGE SEARCH TIME (in seconds)", avgGetDataSearchSecs);
                }
                else
                {
                    Console.WriteLine("Core_BaseAttribute_PerformanceTest: GetManyBaseAttributeDataByCriteria -- Could not perform search. NO AVERAGE SEARCH TIME AVAILABLE.");
                }
                if (numSerializations > 0)
                {
                    avgSerializeDataSecs /= numSerializations;
                    TestHelper.ReportValueToConsole("Core_BaseAttribute_PerformanceTest: GetManyBaseAttributeDataByCriteria AVERAGE SERIALIZATION TIME (in seconds)", avgSerializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Core_BaseAttribute_PerformanceTest: GetManyBaseAttributeDataByCriteria -- Could not serialize. NO AVERAGE SERIALIZATION TIME AVAILABLE.");
                }
                if (numDeserializations > 0)
                {
                    avgDeserializeDataSecs /= numDeserializations;
                    TestHelper.ReportValueToConsole("Core_BaseAttribute_PerformanceTest: GetManyBaseAttributeDataByCriteria AVERAGE DESERIALIZATION TIME (in seconds)", avgDeserializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Core_BaseAttribute_PerformanceTest: GetManyBaseAttributeDataByCriteria -- Could not deserialize. NO AVERAGE DESERIALIZATION TIME AVAILABLE.");
                }
                ...
                Console.WriteLine("******************** Core_BaseAttribute_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Core_BaseAttribute_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
