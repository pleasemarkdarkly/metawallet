...
namespace MW.MComm.WalletSolution.UnitTest.Core
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Core"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Core.DebugLog))]
    public partial class DebugLog_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public DebugLog_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for DebugLog.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Core_DebugLog_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Core_DebugLog_BasicTest START ********************");
                ...
                BLL.Core.DebugLog item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Core_DebugLog_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Core_DebugLog_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Core_DebugLog_BasicTest: Add DebugLog count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = DebugLog_Manager.AddOneDebugLogTest(true, Results);
                        ...
                        else
                        {
                            ...
                            entityID["DebugLogID"] = item.DebugLogID;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Core_DebugLog_BasicTest: Add succeeded for DebugLog", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for DebugLog", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for DebugLog", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if DebugLog Add count is not the same as Update count
                ...
                Console.WriteLine("******************** Core_DebugLog_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Core_DebugLog_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
