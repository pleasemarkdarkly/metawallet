...
namespace MW.MComm.WalletSolution.UnitTest.Core
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Core"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Core.EventType))]
    public partial class EventType_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        [Test]
        public void Core_EventType_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Core_EventType_BasicTest START ********************");
                ...
                BLL.Core.EventType item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Core_EventType_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Core_EventType_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Core_EventType_BasicTest: Add EventType count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Core_EventType_BasicTest: Add succeeded for EventType", item.PrimaryKey.ToString());
                ...
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Core_EventType_BasicTest: Update EventType count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        ...
                        TestHelper.ReportValueToConsole("Core_EventType_BasicTest: Update succeeded for EventType", item.PrimaryKey.ToString());
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Core_EventType_BasicTest: Delete EventType count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            ...
                            TestHelper.ReportValueToConsole("Core_EventType_BasicTest: Delete succeeded for EventType", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                ...
                Console.WriteLine("******************** Core_EventType_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Core_EventType_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
