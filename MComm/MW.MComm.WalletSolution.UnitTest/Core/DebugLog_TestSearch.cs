...
namespace MW.MComm.WalletSolution.UnitTest.Core
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Search.Core"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Core.DebugLog))]
    public partial class DebugLog_TestSearch : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public DebugLog_TestSearch() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the search entity test for DebugLog.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Core_DebugLog_SearchTest()
        {
            try
            {
                Console.WriteLine("******************** Core_DebugLog_SearchTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Core_DebugLog_SearchTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Core_DebugLog_SearchTest: GetAllDebugLogData test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Core.DebugLog> items = DebugLog_Manager.GetAllDebugLogDataTest(Results);
                        TestHelper.ReportValueToConsole("Core_DebugLog_SearchTest: GetAllDebugLogData succeeded for DebugLog. The count", items.Count.ToString());
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllDebugLogData failed for DebugLog", itemStr, ex, strExtraInfo);
                        }
                    }
                }
                loadCount = TestHelper.GetInt(Globals.MinimumGetAllCountPerEntity, Globals.MaximumGetAllCountPerEntity);
                TestHelper.ReportValueToConsole("Core_DebugLog_SearchTest: GetManyDebugLogDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Core.DebugLog> items = DebugLog_Manager.GetManyDebugLogDataByCriteriaTest(Globals.getDataOptions("","Random",1,200,2000), Results);
                        TestHelper.ReportValueToConsole("Core_DebugLog_SearchTest: GetManyDebugLogDataByCriteria succeeded for DebugLog. The count", items.Count.ToString());
                ...
                Console.WriteLine("******************** Core_DebugLog_SearchTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Core_DebugLog_SearchTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
