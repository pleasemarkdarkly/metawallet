...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for DebugLog instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Core"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Core.DebugLog))]
    public partial class DebugLog_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public DebugLog_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for DebugLog.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Core_DebugLog_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Core_DebugLog_PerformanceTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Core_DebugLog_PerformanceTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                {
                    TestHelper.ReportValueToConsole("Core_DebugLog_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString((UnitTestPerformanceLevel)(i + 1)) + " time (in seconds)", perfTimeLevels[i]);
                }
                TestHelper.ReportValueToConsole("Core_DebugLog_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString(UnitTestPerformanceLevel.Unacceptable) + " time (in seconds) is beyond", perfTimeLevels[(((int)UnitTestPerformanceLevel.Unacceptable - 1) - 1)]);
                ...
                TestHelper.ReportValueToConsole("Core_DebugLog_PerformanceTest: page size", pageSize);
                TestHelper.ReportValueToConsole("Core_DebugLog_PerformanceTest: maxListSize", maxListSize);
                ...
                TestHelper.ReportValueToConsole("Core_DebugLog_PerformanceTest: GetManyDebugLogDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    long startTicks = 0;
                    long endTicks = 0;
                    double secondsInterval = 0.00;
                    BLL.SortableList<BLL.Core.DebugLog> items = null;

                    TestHelper.ReportValueToConsole("Core_DebugLog_PerformanceTest: Loop Iteration", i);
                    ...
                    TestHelper.ReportValueToConsole("Core_DebugLog_PerformanceTest: GetManyDebugLogDataByCriteria acceptable search time (in seconds)", acceptableTime);
                    try
                    {
                        // get many
                        Results.ResetCurrentPerfMetric();
                        Results.SetPerfMetricName("Core.DebugLog.GetMany." + i.ToString());
                        items = DebugLog_Manager.GetManyDebugLogDataByCriteriaTest(dbOptions, Results);
                        TestHelper.ReportValueToConsole("Core_DebugLog_PerformanceTest: GetManyDebugLogDataByCriteria succeeded for DebugLog. The count", items.Count.ToString());
                        ...
                        perfStr = "GetManyDebugLogDataByCriteria search time (in seconds) for DebugLog count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional,"GetManyDebugLogDataByCriteria failed for DebugLog", itemStr, ex, strExtraInfo);
                        }
                    }

                    if (items != null)
                    {
                        TestHelper.ReportValueToConsole("Core_DebugLog_PerformanceTest: GetManyDebugLogDataByCriteria acceptable serialization time (in seconds)", acceptableTime);
                        try
                        {
                            // serialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Core.DebugLog.Serialize." + i.ToString());
                            ...
                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyDebugLogDataByCriteria serialization time (in seconds) for DebugLog count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                        ...
                        TestHelper.ReportValueToConsole("Core_DebugLog_PerformanceTest: GetManyDebugLogDataByCriteria acceptable deserialization time (in seconds)", acceptableTime);
                        try
                        {
                            // deserialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Core.DebugLog.Deserialize." + i.ToString());
                            ...
                            items = (BLL.SortableList<BLL.Core.DebugLog>)oItems;
                            int itemCount = items.Count;
                            oItems = null;
                            items = null;

                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyDebugLogDataByCriteria deserialization time (in seconds) for DebugLog count " + itemCount + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                ...
                if (numGetDataSearches > 0)
                {
                    ...
                    TestHelper.ReportValueToConsole("Core_DebugLog_PerformanceTest: GetManyDebugLogDataByCriteria AVERAGE SEARCH TIME (in seconds)", avgGetDataSearchSecs);
                }
                else
                {
                    Console.WriteLine("Core_DebugLog_PerformanceTest: GetManyDebugLogDataByCriteria -- Could not perform search. NO AVERAGE SEARCH TIME AVAILABLE.");
                }
                if (numSerializations > 0)
                {
                    avgSerializeDataSecs /= numSerializations;
                    TestHelper.ReportValueToConsole("Core_DebugLog_PerformanceTest: GetManyDebugLogDataByCriteria AVERAGE SERIALIZATION TIME (in seconds)", avgSerializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Core_DebugLog_PerformanceTest: GetManyDebugLogDataByCriteria -- Could not serialize. NO AVERAGE SERIALIZATION TIME AVAILABLE.");
                }
                if (numDeserializations > 0)
                {
                    avgDeserializeDataSecs /= numDeserializations;
                    TestHelper.ReportValueToConsole("Core_DebugLog_PerformanceTest: GetManyDebugLogDataByCriteria AVERAGE DESERIALIZATION TIME (in seconds)", avgDeserializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Core_DebugLog_PerformanceTest: GetManyDebugLogDataByCriteria -- Could not deserialize. NO AVERAGE DESERIALIZATION TIME AVAILABLE.");
                }
                ...
                Console.WriteLine("******************** Core_DebugLog_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Core_DebugLog_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
