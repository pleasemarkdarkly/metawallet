...
namespace MW.MComm.WalletSolution.UnitTest.Accounts
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Accounts"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Accounts.BankAccount))]
    public partial class BankAccount_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public BankAccount_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for BankAccount.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Accounts_BankAccount_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Accounts_BankAccount_PerformanceTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Accounts_BankAccount_PerformanceTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                {
                    TestHelper.ReportValueToConsole("Accounts_BankAccount_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString((UnitTestPerformanceLevel)(i + 1)) + " time (in seconds)", perfTimeLevels[i]);
                }
                TestHelper.ReportValueToConsole("Accounts_BankAccount_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString(UnitTestPerformanceLevel.Unacceptable) + " time (in seconds) is beyond", perfTimeLevels[(((int)UnitTestPerformanceLevel.Unacceptable - 1) - 1)]);
                ...
                TestHelper.ReportValueToConsole("Accounts_BankAccount_PerformanceTest: page size", pageSize);
                TestHelper.ReportValueToConsole("Accounts_BankAccount_PerformanceTest: maxListSize", maxListSize);
                ...
                TestHelper.ReportValueToConsole("Accounts_BankAccount_PerformanceTest: GetManyBankAccountDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    long startTicks = 0;
                    long endTicks = 0;
                    double secondsInterval = 0.00;
                    BLL.SortableList<BLL.Accounts.BankAccount> items = null;

                    TestHelper.ReportValueToConsole("Accounts_BankAccount_PerformanceTest: Loop Iteration", i);
                    ...
                    TestHelper.ReportValueToConsole("Accounts_BankAccount_PerformanceTest: GetManyBankAccountDataByCriteria acceptable search time (in seconds)", acceptableTime);
                    try
                    {
                        // get many
                        Results.ResetCurrentPerfMetric();
                        Results.SetPerfMetricName("Accounts.BankAccount.GetMany." + i.ToString());
                        items = BankAccount_Manager.GetManyBankAccountDataByCriteriaTest(dbOptions, Results);
                        TestHelper.ReportValueToConsole("Accounts_BankAccount_PerformanceTest: GetManyBankAccountDataByCriteria succeeded for BankAccount. The count", items.Count.ToString());
                        ...
                        perfStr = "GetManyBankAccountDataByCriteria search time (in seconds) for BankAccount count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional,"GetManyBankAccountDataByCriteria failed for BankAccount", itemStr, ex, strExtraInfo);
                        }
                    }

                    if (items != null)
                    {
                        TestHelper.ReportValueToConsole("Accounts_BankAccount_PerformanceTest: GetManyBankAccountDataByCriteria acceptable serialization time (in seconds)", acceptableTime);
                        try
                        {
                            // serialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Accounts.BankAccount.Serialize." + i.ToString());
                            ...
                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyBankAccountDataByCriteria serialization time (in seconds) for BankAccount count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                        ...
                        TestHelper.ReportValueToConsole("Accounts_BankAccount_PerformanceTest: GetManyBankAccountDataByCriteria acceptable deserialization time (in seconds)", acceptableTime);
                        try
                        {
                            // deserialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Accounts.BankAccount.Deserialize." + i.ToString());
                            ...
                            items = (BLL.SortableList<BLL.Accounts.BankAccount>)oItems;
                            int itemCount = items.Count;
                            oItems = null;
                            items = null;

                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyBankAccountDataByCriteria deserialization time (in seconds) for BankAccount count " + itemCount + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                ...
                if (numGetDataSearches > 0)
                {
                    ...
                    TestHelper.ReportValueToConsole("Accounts_BankAccount_PerformanceTest: GetManyBankAccountDataByCriteria AVERAGE SEARCH TIME (in seconds)", avgGetDataSearchSecs);
                }
                else
                {
                    Console.WriteLine("Accounts_BankAccount_PerformanceTest: GetManyBankAccountDataByCriteria -- Could not perform search. NO AVERAGE SEARCH TIME AVAILABLE.");
                }
                if (numSerializations > 0)
                {
                    avgSerializeDataSecs /= numSerializations;
                    TestHelper.ReportValueToConsole("Accounts_BankAccount_PerformanceTest: GetManyBankAccountDataByCriteria AVERAGE SERIALIZATION TIME (in seconds)", avgSerializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Accounts_BankAccount_PerformanceTest: GetManyBankAccountDataByCriteria -- Could not serialize. NO AVERAGE SERIALIZATION TIME AVAILABLE.");
                }
                if (numDeserializations > 0)
                {
                    avgDeserializeDataSecs /= numDeserializations;
                    TestHelper.ReportValueToConsole("Accounts_BankAccount_PerformanceTest: GetManyBankAccountDataByCriteria AVERAGE DESERIALIZATION TIME (in seconds)", avgDeserializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Accounts_BankAccount_PerformanceTest: GetManyBankAccountDataByCriteria -- Could not deserialize. NO AVERAGE DESERIALIZATION TIME AVAILABLE.");
                }
                ...
                Console.WriteLine("******************** Accounts_BankAccount_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Accounts_BankAccount_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
