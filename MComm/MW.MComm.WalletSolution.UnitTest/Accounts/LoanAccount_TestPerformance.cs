...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for LoanAccount instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Accounts"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Accounts.LoanAccount))]
    public partial class LoanAccount_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public LoanAccount_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for LoanAccount.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Accounts_LoanAccount_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Accounts_LoanAccount_PerformanceTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Accounts_LoanAccount_PerformanceTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                {
                    TestHelper.ReportValueToConsole("Accounts_LoanAccount_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString((UnitTestPerformanceLevel)(i + 1)) + " time (in seconds)", perfTimeLevels[i]);
                }
                TestHelper.ReportValueToConsole("Accounts_LoanAccount_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString(UnitTestPerformanceLevel.Unacceptable) + " time (in seconds) is beyond", perfTimeLevels[(((int)UnitTestPerformanceLevel.Unacceptable - 1) - 1)]);
                ...
                TestHelper.ReportValueToConsole("Accounts_LoanAccount_PerformanceTest: page size", pageSize);
                TestHelper.ReportValueToConsole("Accounts_LoanAccount_PerformanceTest: maxListSize", maxListSize);
                ...
                TestHelper.ReportValueToConsole("Accounts_LoanAccount_PerformanceTest: GetManyLoanAccountDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    long startTicks = 0;
                    long endTicks = 0;
                    double secondsInterval = 0.00;
                    BLL.SortableList<BLL.Accounts.LoanAccount> items = null;

                    TestHelper.ReportValueToConsole("Accounts_LoanAccount_PerformanceTest: Loop Iteration", i);
                    ...
                    TestHelper.ReportValueToConsole("Accounts_LoanAccount_PerformanceTest: GetManyLoanAccountDataByCriteria acceptable search time (in seconds)", acceptableTime);
                    try
                    {
                        // get many
                        Results.ResetCurrentPerfMetric();
                        Results.SetPerfMetricName("Accounts.LoanAccount.GetMany." + i.ToString());
                        items = LoanAccount_Manager.GetManyLoanAccountDataByCriteriaTest(dbOptions, Results);
                        TestHelper.ReportValueToConsole("Accounts_LoanAccount_PerformanceTest: GetManyLoanAccountDataByCriteria succeeded for LoanAccount. The count", items.Count.ToString());
                        ...
                        perfStr = "GetManyLoanAccountDataByCriteria search time (in seconds) for LoanAccount count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional,"GetManyLoanAccountDataByCriteria failed for LoanAccount", itemStr, ex, strExtraInfo);
                        }
                    }

                    if (items != null)
                    {
                        TestHelper.ReportValueToConsole("Accounts_LoanAccount_PerformanceTest: GetManyLoanAccountDataByCriteria acceptable serialization time (in seconds)", acceptableTime);
                        try
                        {
                            // serialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Accounts.LoanAccount.Serialize." + i.ToString());
                            ...
                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyLoanAccountDataByCriteria serialization time (in seconds) for LoanAccount count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                        ...
                        TestHelper.ReportValueToConsole("Accounts_LoanAccount_PerformanceTest: GetManyLoanAccountDataByCriteria acceptable deserialization time (in seconds)", acceptableTime);
                        try
                        {
                            // deserialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Accounts.LoanAccount.Deserialize." + i.ToString());
                            ...
                            items = (BLL.SortableList<BLL.Accounts.LoanAccount>)oItems;
                            int itemCount = items.Count;
                            oItems = null;
                            items = null;

                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyLoanAccountDataByCriteria deserialization time (in seconds) for LoanAccount count " + itemCount + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                ...
                if (numGetDataSearches > 0)
                {
                    ...
                    TestHelper.ReportValueToConsole("Accounts_LoanAccount_PerformanceTest: GetManyLoanAccountDataByCriteria AVERAGE SEARCH TIME (in seconds)", avgGetDataSearchSecs);
                }
                else
                {
                    Console.WriteLine("Accounts_LoanAccount_PerformanceTest: GetManyLoanAccountDataByCriteria -- Could not perform search. NO AVERAGE SEARCH TIME AVAILABLE.");
                }
                if (numSerializations > 0)
                {
                    avgSerializeDataSecs /= numSerializations;
                    TestHelper.ReportValueToConsole("Accounts_LoanAccount_PerformanceTest: GetManyLoanAccountDataByCriteria AVERAGE SERIALIZATION TIME (in seconds)", avgSerializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Accounts_LoanAccount_PerformanceTest: GetManyLoanAccountDataByCriteria -- Could not serialize. NO AVERAGE SERIALIZATION TIME AVAILABLE.");
                }
                if (numDeserializations > 0)
                {
                    avgDeserializeDataSecs /= numDeserializations;
                    TestHelper.ReportValueToConsole("Accounts_LoanAccount_PerformanceTest: GetManyLoanAccountDataByCriteria AVERAGE DESERIALIZATION TIME (in seconds)", avgDeserializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Accounts_LoanAccount_PerformanceTest: GetManyLoanAccountDataByCriteria -- Could not deserialize. NO AVERAGE DESERIALIZATION TIME AVAILABLE.");
                }
                ...
                Console.WriteLine("******************** Accounts_LoanAccount_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Accounts_LoanAccount_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
