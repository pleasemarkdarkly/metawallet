...
namespace MW.MComm.WalletSolution.UnitTest.Accounts
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Accounts"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Accounts.StoredValueAccount))]
    public partial class StoredValueAccount_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public StoredValueAccount_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for StoredValueAccount.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Accounts_StoredValueAccount_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Accounts_StoredValueAccount_BasicTest START ********************");
                ...
                BLL.Accounts.StoredValueAccount item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Accounts_StoredValueAccount_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Accounts_StoredValueAccount_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Accounts_StoredValueAccount_BasicTest: Add StoredValueAccount count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = StoredValueAccount_Manager.AddOneStoredValueAccountTest(true, Results);
                        ...
                        else
                        {
                            ...
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Accounts_StoredValueAccount_BasicTest: Add succeeded for StoredValueAccount", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Accounts.StoredValueAccount)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for StoredValueAccount", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for StoredValueAccount", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if StoredValueAccount Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Accounts_StoredValueAccount_BasicTest: Update StoredValueAccount count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = StoredValueAccount_Manager.UpdateOneStoredValueAccountTest((Guid)loopID["AccountID"], true, Results);
                        TestHelper.ReportValueToConsole("Accounts_StoredValueAccount_BasicTest: Update succeeded for StoredValueAccount", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for StoredValueAccount", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Accounts_StoredValueAccount_BasicTest: Delete StoredValueAccount count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = StoredValueAccount_Manager.DeleteOneStoredValueAccountTest((Guid)loopID["AccountID"], true, Results);
                            TestHelper.ReportValueToConsole("Accounts_StoredValueAccount_BasicTest: Delete succeeded for StoredValueAccount", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for StoredValueAccount", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Accounts_StoredValueAccount_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Accounts_StoredValueAccount_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
