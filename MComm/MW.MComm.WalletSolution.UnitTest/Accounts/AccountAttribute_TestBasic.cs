...
namespace MW.MComm.WalletSolution.UnitTest.Accounts
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Accounts"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Accounts.AccountAttribute))]
    public partial class AccountAttribute_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public AccountAttribute_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for AccountAttribute.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Accounts_AccountAttribute_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Accounts_AccountAttribute_BasicTest START ********************");
                ...
                BLL.Accounts.AccountAttribute item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Accounts_AccountAttribute_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Accounts_AccountAttribute_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Accounts_AccountAttribute_BasicTest: Add AccountAttribute count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = AccountAttribute_Manager.AddOneAccountAttributeTest(true, Results);
                        ...
                        else
                        {
                            ...
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Accounts_AccountAttribute_BasicTest: Add succeeded for AccountAttribute", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Accounts.AccountAttribute)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for AccountAttribute", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for AccountAttribute", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if AccountAttribute Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Accounts_AccountAttribute_BasicTest: Update AccountAttribute count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = AccountAttribute_Manager.UpdateOneAccountAttributeTest((Guid)loopID["BaseAttributeID"], true, Results);
                        TestHelper.ReportValueToConsole("Accounts_AccountAttribute_BasicTest: Update succeeded for AccountAttribute", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for AccountAttribute", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Accounts_AccountAttribute_BasicTest: Delete AccountAttribute count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = AccountAttribute_Manager.DeleteOneAccountAttributeTest((Guid)loopID["BaseAttributeID"], true, Results);
                            TestHelper.ReportValueToConsole("Accounts_AccountAttribute_BasicTest: Delete succeeded for AccountAttribute", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for AccountAttribute", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Accounts_AccountAttribute_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Accounts_AccountAttribute_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
