...
namespace MW.MComm.WalletSolution.UnitTest.Accounts
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Accounts"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Accounts.CreditCardType))]
    public partial class CreditCardType_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public CreditCardType_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for CreditCardType.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Accounts_CreditCardType_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Accounts_CreditCardType_PerformanceTest START ********************");
                ...
                Console.WriteLine("GetAllCreditCardTypeData test count is {0}.", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Accounts.CreditCardType> items = CreditCardType_Manager.GetAllCreditCardTypeDataTest(Results);
                        Console.WriteLine("GetAllCreditCardTypeData succeeded for CreditCardType.");
                        ...
                        items = (BLL.SortableList<BLL.Accounts.CreditCardType>)oItems;
                        oItems = null;
                        items = null;
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllCreditCardTypeData failed for CreditCardType", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                Console.WriteLine("******************** Accounts_CreditCardType_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Accounts_CreditCardType_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
