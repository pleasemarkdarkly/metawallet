...
namespace MW.MComm.WalletSolution.UnitTest.Accounts
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Accounts"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Accounts.PaymentInstitution))]
    public partial class PaymentInstitution_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public PaymentInstitution_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for PaymentInstitution.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Accounts_PaymentInstitution_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Accounts_PaymentInstitution_PerformanceTest START ********************");
                ...
                Console.WriteLine("GetAllPaymentInstitutionData test count is {0}.", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Accounts.PaymentInstitution> items = PaymentInstitution_Manager.GetAllPaymentInstitutionDataTest(Results);
                        Console.WriteLine("GetAllPaymentInstitutionData succeeded for PaymentInstitution.");
                        ...
                        items = (BLL.SortableList<BLL.Accounts.PaymentInstitution>)oItems;
                        oItems = null;
                        items = null;
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllPaymentInstitutionData failed for PaymentInstitution", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                Console.WriteLine("******************** Accounts_PaymentInstitution_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Accounts_PaymentInstitution_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
