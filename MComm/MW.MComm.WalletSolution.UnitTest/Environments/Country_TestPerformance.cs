...
namespace MW.MComm.WalletSolution.UnitTest.Environments
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Environments"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Environments.Country))]
    public partial class Country_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public Country_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for Country.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Environments_Country_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Environments_Country_PerformanceTest START ********************");
                ...
                Console.WriteLine("GetAllCountryData test count is {0}.", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Environments.Country> items = Country_Manager.GetAllCountryDataTest(Results);
                        Console.WriteLine("GetAllCountryData succeeded for Country.");
                        ...
                        items = (BLL.SortableList<BLL.Environments.Country>)oItems;
                        oItems = null;
                        items = null;
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllCountryData failed for Country", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                Console.WriteLine("******************** Environments_Country_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Environments_Country_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
