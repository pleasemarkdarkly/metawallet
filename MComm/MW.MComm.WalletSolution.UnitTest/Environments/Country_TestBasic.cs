...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for Country instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Environments"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Environments.Country))]
    public partial class Country_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for Country.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Environments_Country_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Environments_Country_BasicTest START ********************");
                ...
                BLL.Environments.Country item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Environments_Country_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Environments_Country_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Environments_Country_BasicTest: Add Country count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = Country_Manager.AddOneCountryTest(true, Results);
                        if (Results.IsCollision == true)
                        {
                            throw new UnitTestException(UnitTestErrorType.Warning, "Add collision encountered for Country: " + item.PrimaryKey.ToString(), null);
                        }
                        else
                        {
                            MOD.Data.NamedObjectCollection entityID = new MOD.Data.NamedObjectCollection();
                            entityID["CountryCode"] = item.CountryCode;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Environments_Country_BasicTest: Add succeeded for Country", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Environments.Country)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for Country", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for Country", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if Country Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Environments_Country_BasicTest: Update Country count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = Country_Manager.UpdateOneCountryTest((int)loopID["CountryCode"], true, Results);
                        TestHelper.ReportValueToConsole("Environments_Country_BasicTest: Update succeeded for Country", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for Country", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Environments_Country_BasicTest: Delete Country count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = Country_Manager.DeleteOneCountryTest((int)loopID["CountryCode"], true, Results);
                            TestHelper.ReportValueToConsole("Environments_Country_BasicTest: Delete succeeded for Country", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for Country", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Environments_Country_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Environments_Country_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
