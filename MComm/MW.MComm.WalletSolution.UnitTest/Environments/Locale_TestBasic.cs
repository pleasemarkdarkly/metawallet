...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for Locale instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Environments"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Environments.Locale))]
    public partial class Locale_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for Locale.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Environments_Locale_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Environments_Locale_BasicTest START ********************");
                ...
                BLL.Environments.Locale item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Environments_Locale_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Environments_Locale_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Environments_Locale_BasicTest: Add Locale count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = Locale_Manager.AddOneLocaleTest(true, Results);
                        if (Results.IsCollision == true)
                        {
                            throw new UnitTestException(UnitTestErrorType.Warning, "Add collision encountered for Locale: " + item.PrimaryKey.ToString(), null);
                        }
                        else
                        {
                            ...
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Environments_Locale_BasicTest: Add succeeded for Locale", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Environments.Locale)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for Locale", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for Locale", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if Locale Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Environments_Locale_BasicTest: Update Locale count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = Locale_Manager.UpdateOneLocaleTest((int)loopID["LocaleCode"], true, Results);
                        TestHelper.ReportValueToConsole("Environments_Locale_BasicTest: Update succeeded for Locale", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for Locale", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Environments_Locale_BasicTest: Delete Locale count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = Locale_Manager.DeleteOneLocaleTest((int)loopID["LocaleCode"], true, Results);
                            TestHelper.ReportValueToConsole("Environments_Locale_BasicTest: Delete succeeded for Locale", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for Locale", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Environments_Locale_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Environments_Locale_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
