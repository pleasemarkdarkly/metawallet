...
namespace MW.MComm.WalletSolution.UnitTest.Environments
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Environments"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Environments.Locale))]
    public partial class Locale_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public Locale_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for Locale.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Environments_Locale_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Environments_Locale_PerformanceTest START ********************");
                ...
                Console.WriteLine("GetAllLocaleData test count is {0}.", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Environments.Locale> items = Locale_Manager.GetAllLocaleDataTest(Results);
                        Console.WriteLine("GetAllLocaleData succeeded for Locale.");
                        ...
                        items = (BLL.SortableList<BLL.Environments.Locale>)oItems;
                        oItems = null;
                        items = null;
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllLocaleData failed for Locale", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                Console.WriteLine("******************** Environments_Locale_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Environments_Locale_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
