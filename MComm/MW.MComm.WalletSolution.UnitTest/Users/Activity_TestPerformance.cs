...
namespace MW.MComm.WalletSolution.UnitTest.Users
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Users"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Users.Activity))]
    public partial class Activity_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public Activity_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for Activity.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Users_Activity_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Users_Activity_PerformanceTest START ********************");
                ...
                Console.WriteLine("GetAllActivityData test count is {0}.", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Users.Activity> items = Activity_Manager.GetAllActivityDataTest(Results);
                        Console.WriteLine("GetAllActivityData succeeded for Activity.");
                        ...
                        items = (BLL.SortableList<BLL.Users.Activity>)oItems;
                        oItems = null;
                        items = null;
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllActivityData failed for Activity", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                Console.WriteLine("******************** Users_Activity_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Users_Activity_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
