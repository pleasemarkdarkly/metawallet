...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for Activity instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Users"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Users.Activity))]
    public partial class Activity_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public Activity_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for Activity.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Users_Activity_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Users_Activity_BasicTest START ********************");
                ...
                BLL.Users.Activity item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Users_Activity_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Users_Activity_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Users_Activity_BasicTest: Add Activity count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = Activity_Manager.AddOneActivityTest(true, Results);
                        if (Results.IsCollision == true)
                        {
                            throw new UnitTestException(UnitTestErrorType.Warning, "Add collision encountered for Activity: " + item.PrimaryKey.ToString(), null);
                        }
                        else
                        {
                            ...
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Users_Activity_BasicTest: Add succeeded for Activity", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for Activity", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for Activity", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if Activity Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Users_Activity_BasicTest: Update Activity count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = Activity_Manager.UpdateOneActivityTest((int)loopID["ActivityCode"], true, Results);
                        TestHelper.ReportValueToConsole("Users_Activity_BasicTest: Update succeeded for Activity", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for Activity", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Users_Activity_BasicTest: Delete Activity count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = Activity_Manager.DeleteOneActivityTest((int)loopID["ActivityCode"], true, Results);
                            TestHelper.ReportValueToConsole("Users_Activity_BasicTest: Delete succeeded for Activity", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for Activity", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Users_Activity_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Users_Activity_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
