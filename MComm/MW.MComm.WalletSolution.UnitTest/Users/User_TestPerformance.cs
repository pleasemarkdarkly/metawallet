...
namespace MW.MComm.WalletSolution.UnitTest.Users
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Users"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Users.User))]
    public partial class User_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public User_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for User.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Users_User_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Users_User_PerformanceTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Users_User_PerformanceTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                {
                    TestHelper.ReportValueToConsole("Users_User_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString((UnitTestPerformanceLevel)(i + 1)) + " time (in seconds)", perfTimeLevels[i]);
                }
                TestHelper.ReportValueToConsole("Users_User_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString(UnitTestPerformanceLevel.Unacceptable) + " time (in seconds) is beyond", perfTimeLevels[(((int)UnitTestPerformanceLevel.Unacceptable - 1) - 1)]);
                ...
                TestHelper.ReportValueToConsole("Users_User_PerformanceTest: page size", pageSize);
                TestHelper.ReportValueToConsole("Users_User_PerformanceTest: maxListSize", maxListSize);
                ...
                TestHelper.ReportValueToConsole("Users_User_PerformanceTest: GetManyUserDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    long startTicks = 0;
                    long endTicks = 0;
                    double secondsInterval = 0.00;
                    BLL.SortableList<BLL.Users.User> items = null;

                    TestHelper.ReportValueToConsole("Users_User_PerformanceTest: Loop Iteration", i);
                    ...
                    TestHelper.ReportValueToConsole("Users_User_PerformanceTest: GetManyUserDataByCriteria acceptable search time (in seconds)", acceptableTime);
                    try
                    {
                        // get many
                        Results.ResetCurrentPerfMetric();
                        Results.SetPerfMetricName("Users.User.GetMany." + i.ToString());
                        items = User_Manager.GetManyUserDataByCriteriaTest(dbOptions, Results);
                        TestHelper.ReportValueToConsole("Users_User_PerformanceTest: GetManyUserDataByCriteria succeeded for User. The count", items.Count.ToString());
                        ...
                        perfStr = "GetManyUserDataByCriteria search time (in seconds) for User count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                    ...
                    if (items != null)
                    {
                        TestHelper.ReportValueToConsole("Users_User_PerformanceTest: GetManyUserDataByCriteria acceptable serialization time (in seconds)", acceptableTime);
                        try
                        {
                            // serialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Users.User.Serialize." + i.ToString());
                            ...
                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyUserDataByCriteria serialization time (in seconds) for User count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                        ...
                        TestHelper.ReportValueToConsole("Users_User_PerformanceTest: GetManyUserDataByCriteria acceptable deserialization time (in seconds)", acceptableTime);
                        try
                        {
                            // deserialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Users.User.Deserialize." + i.ToString());
                            ...
                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyUserDataByCriteria deserialization time (in seconds) for User count " + itemCount + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                ...
                if (numGetDataSearches > 0)
                {
                    ...
                    TestHelper.ReportValueToConsole("Users_User_PerformanceTest: GetManyUserDataByCriteria AVERAGE SEARCH TIME (in seconds)", avgGetDataSearchSecs);
                }
                else
                {
                    Console.WriteLine("Users_User_PerformanceTest: GetManyUserDataByCriteria -- Could not perform search. NO AVERAGE SEARCH TIME AVAILABLE.");
                }
                if (numSerializations > 0)
                {
                    avgSerializeDataSecs /= numSerializations;
                    TestHelper.ReportValueToConsole("Users_User_PerformanceTest: GetManyUserDataByCriteria AVERAGE SERIALIZATION TIME (in seconds)", avgSerializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Users_User_PerformanceTest: GetManyUserDataByCriteria -- Could not serialize. NO AVERAGE SERIALIZATION TIME AVAILABLE.");
                }
                if (numDeserializations > 0)
                {
                    avgDeserializeDataSecs /= numDeserializations;
                    TestHelper.ReportValueToConsole("Users_User_PerformanceTest: GetManyUserDataByCriteria AVERAGE DESERIALIZATION TIME (in seconds)", avgDeserializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Users_User_PerformanceTest: GetManyUserDataByCriteria -- Could not deserialize. NO AVERAGE DESERIALIZATION TIME AVAILABLE.");
                }
                ...
                Console.WriteLine("******************** Users_User_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Users_User_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
