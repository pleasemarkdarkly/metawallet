...
namespace MW.MComm.WalletSolution.UnitTest.Users
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Users"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Users.AccessMode))]
    public partial class AccessMode_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public AccessMode_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for AccessMode.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Users_AccessMode_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Users_AccessMode_BasicTest START ********************");
                ...
                BLL.Users.AccessMode item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Users_AccessMode_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Users_AccessMode_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Users_AccessMode_BasicTest: Add AccessMode count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = AccessMode_Manager.AddOneAccessModeTest(true, Results);
                        if (Results.IsCollision == true)
                        {
                            throw new UnitTestException(UnitTestErrorType.Warning, "Add collision encountered for AccessMode: " + item.PrimaryKey.ToString(), null);
                        }
                        else
                        {
                            MOD.Data.NamedObjectCollection entityID = new MOD.Data.NamedObjectCollection();
                            entityID["AccessModeCode"] = item.AccessModeCode;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Users_AccessMode_BasicTest: Add succeeded for AccessMode", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Users.AccessMode)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for AccessMode", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for AccessMode", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if AccessMode Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Users_AccessMode_BasicTest: Update AccessMode count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = AccessMode_Manager.UpdateOneAccessModeTest((int)loopID["AccessModeCode"], true, Results);
                        TestHelper.ReportValueToConsole("Users_AccessMode_BasicTest: Update succeeded for AccessMode", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for AccessMode", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Users_AccessMode_BasicTest: Delete AccessMode count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = AccessMode_Manager.DeleteOneAccessModeTest((int)loopID["AccessModeCode"], true, Results);
                            TestHelper.ReportValueToConsole("Users_AccessMode_BasicTest: Delete succeeded for AccessMode", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for AccessMode", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Users_AccessMode_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Users_AccessMode_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
