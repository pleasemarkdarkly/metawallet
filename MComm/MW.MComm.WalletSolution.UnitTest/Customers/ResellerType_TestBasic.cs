...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for ResellerType instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Customers"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Customers.ResellerType))]
    public partial class ResellerType_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public ResellerType_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for ResellerType.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Customers_ResellerType_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Customers_ResellerType_BasicTest START ********************");
                ...
                BLL.Customers.ResellerType item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Customers_ResellerType_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Customers_ResellerType_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Customers_ResellerType_BasicTest: Add ResellerType count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = ResellerType_Manager.AddOneResellerTypeTest(true, Results);
                        if (Results.IsCollision == true)
                        {
                            throw new UnitTestException(UnitTestErrorType.Warning, "Add collision encountered for ResellerType: " + item.PrimaryKey.ToString(), null);
                        }
                        else
                        {
                            MOD.Data.NamedObjectCollection entityID = new MOD.Data.NamedObjectCollection();
                            entityID["ResellerTypeCode"] = item.ResellerTypeCode;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Customers_ResellerType_BasicTest: Add succeeded for ResellerType", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Customers.ResellerType)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for ResellerType", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for ResellerType", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if ResellerType Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Customers_ResellerType_BasicTest: Update ResellerType count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = ResellerType_Manager.UpdateOneResellerTypeTest((int)loopID["ResellerTypeCode"], true, Results);
                        TestHelper.ReportValueToConsole("Customers_ResellerType_BasicTest: Update succeeded for ResellerType", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for ResellerType", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Customers_ResellerType_BasicTest: Delete ResellerType count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = ResellerType_Manager.DeleteOneResellerTypeTest((int)loopID["ResellerTypeCode"], true, Results);
                            TestHelper.ReportValueToConsole("Customers_ResellerType_BasicTest: Delete succeeded for ResellerType", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for ResellerType", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Customers_ResellerType_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Customers_ResellerType_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
