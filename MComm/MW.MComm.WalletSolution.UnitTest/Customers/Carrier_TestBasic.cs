...
namespace MW.MComm.WalletSolution.UnitTest.Customers
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Customers"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Customers.Carrier))]
    public partial class Carrier_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for Carrier.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Customers_Carrier_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Customers_Carrier_BasicTest START ********************");
                ...
                BLL.Customers.Carrier item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Customers_Carrier_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Customers_Carrier_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Customers_Carrier_BasicTest: Add Carrier count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = Carrier_Manager.AddOneCarrierTest(true, Results);
                        ...
                        else
                        {
                            ...
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Customers_Carrier_BasicTest: Add succeeded for Carrier", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Customers.Carrier)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for Carrier", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for Carrier", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if Carrier Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Customers_Carrier_BasicTest: Update Carrier count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = Carrier_Manager.UpdateOneCarrierTest((Guid)loopID["MetaPartnerID"], true, Results);
                        TestHelper.ReportValueToConsole("Customers_Carrier_BasicTest: Update succeeded for Carrier", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for Carrier", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Customers_Carrier_BasicTest: Delete Carrier count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = Carrier_Manager.DeleteOneCarrierTest((Guid)loopID["MetaPartnerID"], true, Results);
                            TestHelper.ReportValueToConsole("Customers_Carrier_BasicTest: Delete succeeded for Carrier", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for Carrier", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Customers_Carrier_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Customers_Carrier_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
