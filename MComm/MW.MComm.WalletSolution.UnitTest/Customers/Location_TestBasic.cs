...
namespace MW.MComm.WalletSolution.UnitTest.Customers
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Customers"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Customers.Location))]
    public partial class Location_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public Location_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for Location.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Customers_Location_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Customers_Location_BasicTest START ********************");
                ...
                BLL.Customers.Location item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Customers_Location_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Customers_Location_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Customers_Location_BasicTest: Add Location count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = Location_Manager.AddOneLocationTest(true, Results);
                        ...
                        else
                        {
                            ...
                            entityID["LocationID"] = item.LocationID;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Customers_Location_BasicTest: Add succeeded for Location", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Customers.Location)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for Location", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for Location", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if Location Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Customers_Location_BasicTest: Update Location count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = Location_Manager.UpdateOneLocationTest((Guid)loopID["LocationID"], true, Results);
                        TestHelper.ReportValueToConsole("Customers_Location_BasicTest: Update succeeded for Location", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for Location", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Customers_Location_BasicTest: Delete Location count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = Location_Manager.DeleteOneLocationTest((Guid)loopID["LocationID"], true, Results);
                            TestHelper.ReportValueToConsole("Customers_Location_BasicTest: Delete succeeded for Location", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for Location", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Customers_Location_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Customers_Location_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
