...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for MetaPartner instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Customers"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Customers.MetaPartner))]
    public partial class MetaPartner_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public MetaPartner_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for MetaPartner.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Customers_MetaPartner_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Customers_MetaPartner_PerformanceTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Customers_MetaPartner_PerformanceTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                {
                    TestHelper.ReportValueToConsole("Customers_MetaPartner_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString((UnitTestPerformanceLevel)(i + 1)) + " time (in seconds)", perfTimeLevels[i]);
                }
                TestHelper.ReportValueToConsole("Customers_MetaPartner_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString(UnitTestPerformanceLevel.Unacceptable) + " time (in seconds) is beyond", perfTimeLevels[(((int)UnitTestPerformanceLevel.Unacceptable - 1) - 1)]);
                ...
                TestHelper.ReportValueToConsole("Customers_MetaPartner_PerformanceTest: page size", pageSize);
                TestHelper.ReportValueToConsole("Customers_MetaPartner_PerformanceTest: maxListSize", maxListSize);
                ...
                TestHelper.ReportValueToConsole("Customers_MetaPartner_PerformanceTest: GetManyMetaPartnerDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    long startTicks = 0;
                    long endTicks = 0;
                    double secondsInterval = 0.00;
                    BLL.SortableList<BLL.Customers.MetaPartner> items = null;

                    TestHelper.ReportValueToConsole("Customers_MetaPartner_PerformanceTest: Loop Iteration", i);
                    ...
                    TestHelper.ReportValueToConsole("Customers_MetaPartner_PerformanceTest: GetManyMetaPartnerDataByCriteria acceptable search time (in seconds)", acceptableTime);
                    try
                    {
                        // get many
                        Results.ResetCurrentPerfMetric();
                        Results.SetPerfMetricName("Customers.MetaPartner.GetMany." + i.ToString());
                        items = MetaPartner_Manager.GetManyMetaPartnerDataByCriteriaTest(dbOptions, Results);
                        TestHelper.ReportValueToConsole("Customers_MetaPartner_PerformanceTest: GetManyMetaPartnerDataByCriteria succeeded for MetaPartner. The count", items.Count.ToString());
                        ...
                        perfStr = "GetManyMetaPartnerDataByCriteria search time (in seconds) for MetaPartner count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional,"GetManyMetaPartnerDataByCriteria failed for MetaPartner", itemStr, ex, strExtraInfo);
                        }
                    }

                    if (items != null)
                    {
                        TestHelper.ReportValueToConsole("Customers_MetaPartner_PerformanceTest: GetManyMetaPartnerDataByCriteria acceptable serialization time (in seconds)", acceptableTime);
                        try
                        {
                            // serialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Customers.MetaPartner.Serialize." + i.ToString());
                            ...
                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyMetaPartnerDataByCriteria serialization time (in seconds) for MetaPartner count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                        ...
                        TestHelper.ReportValueToConsole("Customers_MetaPartner_PerformanceTest: GetManyMetaPartnerDataByCriteria acceptable deserialization time (in seconds)", acceptableTime);
                        try
                        {
                            // deserialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Customers.MetaPartner.Deserialize." + i.ToString());
                            ...
                            items = (BLL.SortableList<BLL.Customers.MetaPartner>)oItems;
                            int itemCount = items.Count;
                            oItems = null;
                            items = null;

                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyMetaPartnerDataByCriteria deserialization time (in seconds) for MetaPartner count " + itemCount + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                ...
                if (numGetDataSearches > 0)
                {
                    ...
                    TestHelper.ReportValueToConsole("Customers_MetaPartner_PerformanceTest: GetManyMetaPartnerDataByCriteria AVERAGE SEARCH TIME (in seconds)", avgGetDataSearchSecs);
                }
                else
                {
                    Console.WriteLine("Customers_MetaPartner_PerformanceTest: GetManyMetaPartnerDataByCriteria -- Could not perform search. NO AVERAGE SEARCH TIME AVAILABLE.");
                }
                if (numSerializations > 0)
                {
                    avgSerializeDataSecs /= numSerializations;
                    TestHelper.ReportValueToConsole("Customers_MetaPartner_PerformanceTest: GetManyMetaPartnerDataByCriteria AVERAGE SERIALIZATION TIME (in seconds)", avgSerializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Customers_MetaPartner_PerformanceTest: GetManyMetaPartnerDataByCriteria -- Could not serialize. NO AVERAGE SERIALIZATION TIME AVAILABLE.");
                }
                if (numDeserializations > 0)
                {
                    avgDeserializeDataSecs /= numDeserializations;
                    TestHelper.ReportValueToConsole("Customers_MetaPartner_PerformanceTest: GetManyMetaPartnerDataByCriteria AVERAGE DESERIALIZATION TIME (in seconds)", avgDeserializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Customers_MetaPartner_PerformanceTest: GetManyMetaPartnerDataByCriteria -- Could not deserialize. NO AVERAGE DESERIALIZATION TIME AVAILABLE.");
                }
                ...
                Console.WriteLine("******************** Customers_MetaPartner_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Customers_MetaPartner_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
