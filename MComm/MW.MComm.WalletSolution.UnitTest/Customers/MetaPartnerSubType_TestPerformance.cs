...
namespace MW.MComm.WalletSolution.UnitTest.Customers
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Customers"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Customers.MetaPartnerSubType))]
    public partial class MetaPartnerSubType_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public MetaPartnerSubType_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for MetaPartnerSubType.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Customers_MetaPartnerSubType_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Customers_MetaPartnerSubType_PerformanceTest START ********************");
                ...
                Console.WriteLine("GetAllMetaPartnerSubTypeData test count is {0}.", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Customers.MetaPartnerSubType> items = MetaPartnerSubType_Manager.GetAllMetaPartnerSubTypeDataTest(Results);
                        Console.WriteLine("GetAllMetaPartnerSubTypeData succeeded for MetaPartnerSubType.");
                        ...
                        items = (BLL.SortableList<BLL.Customers.MetaPartnerSubType>)oItems;
                        oItems = null;
                        items = null;
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllMetaPartnerSubTypeData failed for MetaPartnerSubType", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                Console.WriteLine("******************** Customers_MetaPartnerSubType_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Customers_MetaPartnerSubType_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
