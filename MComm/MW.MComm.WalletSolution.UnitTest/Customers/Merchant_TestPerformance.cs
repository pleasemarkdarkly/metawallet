...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for Merchant instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Customers"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Customers.Merchant))]
    public partial class Merchant_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public Merchant_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for Merchant.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Customers_Merchant_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Customers_Merchant_PerformanceTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Customers_Merchant_PerformanceTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                {
                    TestHelper.ReportValueToConsole("Customers_Merchant_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString((UnitTestPerformanceLevel)(i + 1)) + " time (in seconds)", perfTimeLevels[i]);
                }
                TestHelper.ReportValueToConsole("Customers_Merchant_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString(UnitTestPerformanceLevel.Unacceptable) + " time (in seconds) is beyond", perfTimeLevels[(((int)UnitTestPerformanceLevel.Unacceptable - 1) - 1)]);
                ...
                TestHelper.ReportValueToConsole("Customers_Merchant_PerformanceTest: page size", pageSize);
                TestHelper.ReportValueToConsole("Customers_Merchant_PerformanceTest: maxListSize", maxListSize);
                ...
                TestHelper.ReportValueToConsole("Customers_Merchant_PerformanceTest: GetManyMerchantDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    long startTicks = 0;
                    long endTicks = 0;
                    double secondsInterval = 0.00;
                    BLL.SortableList<BLL.Customers.Merchant> items = null;

                    TestHelper.ReportValueToConsole("Customers_Merchant_PerformanceTest: Loop Iteration", i);
                    ...
                    TestHelper.ReportValueToConsole("Customers_Merchant_PerformanceTest: GetManyMerchantDataByCriteria acceptable search time (in seconds)", acceptableTime);
                    try
                    {
                        // get many
                        Results.ResetCurrentPerfMetric();
                        Results.SetPerfMetricName("Customers.Merchant.GetMany." + i.ToString());
                        items = Merchant_Manager.GetManyMerchantDataByCriteriaTest(dbOptions, Results);
                        TestHelper.ReportValueToConsole("Customers_Merchant_PerformanceTest: GetManyMerchantDataByCriteria succeeded for Merchant. The count", items.Count.ToString());
                        ...
                        perfStr = "GetManyMerchantDataByCriteria search time (in seconds) for Merchant count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional,"GetManyMerchantDataByCriteria failed for Merchant", itemStr, ex, strExtraInfo);
                        }
                    }

                    if (items != null)
                    {
                        TestHelper.ReportValueToConsole("Customers_Merchant_PerformanceTest: GetManyMerchantDataByCriteria acceptable serialization time (in seconds)", acceptableTime);
                        try
                        {
                            // serialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Customers.Merchant.Serialize." + i.ToString());
                            ...
                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyMerchantDataByCriteria serialization time (in seconds) for Merchant count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                        ...
                        TestHelper.ReportValueToConsole("Customers_Merchant_PerformanceTest: GetManyMerchantDataByCriteria acceptable deserialization time (in seconds)", acceptableTime);
                        try
                        {
                            // deserialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Customers.Merchant.Deserialize." + i.ToString());
                            ...
                            items = (BLL.SortableList<BLL.Customers.Merchant>)oItems;
                            int itemCount = items.Count;
                            oItems = null;
                            items = null;

                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyMerchantDataByCriteria deserialization time (in seconds) for Merchant count " + itemCount + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                ...
                if (numGetDataSearches > 0)
                {
                    ...
                    TestHelper.ReportValueToConsole("Customers_Merchant_PerformanceTest: GetManyMerchantDataByCriteria AVERAGE SEARCH TIME (in seconds)", avgGetDataSearchSecs);
                }
                else
                {
                    Console.WriteLine("Customers_Merchant_PerformanceTest: GetManyMerchantDataByCriteria -- Could not perform search. NO AVERAGE SEARCH TIME AVAILABLE.");
                }
                if (numSerializations > 0)
                {
                    avgSerializeDataSecs /= numSerializations;
                    TestHelper.ReportValueToConsole("Customers_Merchant_PerformanceTest: GetManyMerchantDataByCriteria AVERAGE SERIALIZATION TIME (in seconds)", avgSerializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Customers_Merchant_PerformanceTest: GetManyMerchantDataByCriteria -- Could not serialize. NO AVERAGE SERIALIZATION TIME AVAILABLE.");
                }
                if (numDeserializations > 0)
                {
                    avgDeserializeDataSecs /= numDeserializations;
                    TestHelper.ReportValueToConsole("Customers_Merchant_PerformanceTest: GetManyMerchantDataByCriteria AVERAGE DESERIALIZATION TIME (in seconds)", avgDeserializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Customers_Merchant_PerformanceTest: GetManyMerchantDataByCriteria -- Could not deserialize. NO AVERAGE DESERIALIZATION TIME AVAILABLE.");
                }
                ...
                Console.WriteLine("******************** Customers_Merchant_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Customers_Merchant_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
