...
namespace MW.MComm.WalletSolution.UnitTest.Customers
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Search.Customers"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Customers.Carrier))]
    public partial class Carrier_TestSearch : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public Carrier_TestSearch() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the search entity test for Carrier.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Customers_Carrier_SearchTest()
        {
            try
            {
                Console.WriteLine("******************** Customers_Carrier_SearchTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Customers_Carrier_SearchTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Customers_Carrier_SearchTest: GetAllCarrierData test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Customers.Carrier> items = Carrier_Manager.GetAllCarrierDataTest(Results);
                        TestHelper.ReportValueToConsole("Customers_Carrier_SearchTest: GetAllCarrierData succeeded for Carrier. The count", items.Count.ToString());
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllCarrierData failed for Carrier", itemStr, ex, strExtraInfo);
                        }
                    }
                }
                loadCount = TestHelper.GetInt(Globals.MinimumGetAllCountPerEntity, Globals.MaximumGetAllCountPerEntity);
                TestHelper.ReportValueToConsole("Customers_Carrier_SearchTest: GetManyCarrierDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Customers.Carrier> items = Carrier_Manager.GetManyCarrierDataByCriteriaTest(Globals.getDataOptions("","Random",1,200,2000), Results);
                        TestHelper.ReportValueToConsole("Customers_Carrier_SearchTest: GetManyCarrierDataByCriteria succeeded for Carrier. The count", items.Count.ToString());
                ...
                Console.WriteLine("******************** Customers_Carrier_SearchTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Customers_Carrier_SearchTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
