...
namespace MW.MComm.WalletSolution.UnitTest.Notifications
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Notifications"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Notifications.Notification))]
    public partial class Notification_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public Notification_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for Notification.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Notifications_Notification_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Notifications_Notification_BasicTest START ********************");
                ...
                BLL.Notifications.Notification item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Notifications_Notification_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Notifications_Notification_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Notifications_Notification_BasicTest: Add Notification count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = Notification_Manager.AddOneNotificationTest(true, Results);
                        ...
                        else
                        {
                            ...
                            entityID["NotificationCode"] = item.NotificationCode;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Notifications_Notification_BasicTest: Add succeeded for Notification", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Notifications.Notification)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for Notification", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for Notification", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if Notification Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Notifications_Notification_BasicTest: Update Notification count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = Notification_Manager.UpdateOneNotificationTest((int)loopID["NotificationCode"], true, Results);
                        TestHelper.ReportValueToConsole("Notifications_Notification_BasicTest: Update succeeded for Notification", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for Notification", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Notifications_Notification_BasicTest: Delete Notification count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = Notification_Manager.DeleteOneNotificationTest((int)loopID["NotificationCode"], true, Results);
                            TestHelper.ReportValueToConsole("Notifications_Notification_BasicTest: Delete succeeded for Notification", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for Notification", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Notifications_Notification_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Notifications_Notification_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
