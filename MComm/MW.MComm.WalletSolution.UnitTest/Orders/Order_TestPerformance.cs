...
namespace MW.MComm.WalletSolution.UnitTest.Orders
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Orders"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Orders.Order))]
    public partial class Order_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public Order_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for Order.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Orders_Order_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Orders_Order_PerformanceTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Orders_Order_PerformanceTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                {
                    TestHelper.ReportValueToConsole("Orders_Order_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString((UnitTestPerformanceLevel)(i + 1)) + " time (in seconds)", perfTimeLevels[i]);
                }
                TestHelper.ReportValueToConsole("Orders_Order_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString(UnitTestPerformanceLevel.Unacceptable) + " time (in seconds) is beyond", perfTimeLevels[(((int)UnitTestPerformanceLevel.Unacceptable - 1) - 1)]);
                ...
                TestHelper.ReportValueToConsole("Orders_Order_PerformanceTest: page size", pageSize);
                TestHelper.ReportValueToConsole("Orders_Order_PerformanceTest: maxListSize", maxListSize);
                ...
                TestHelper.ReportValueToConsole("Orders_Order_PerformanceTest: GetManyOrderDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    long startTicks = 0;
                    long endTicks = 0;
                    double secondsInterval = 0.00;
                    BLL.SortableList<BLL.Orders.Order> items = null;

                    TestHelper.ReportValueToConsole("Orders_Order_PerformanceTest: Loop Iteration", i);
                    ...
                    TestHelper.ReportValueToConsole("Orders_Order_PerformanceTest: GetManyOrderDataByCriteria acceptable search time (in seconds)", acceptableTime);
                    try
                    {
                        // get many
                        Results.ResetCurrentPerfMetric();
                        Results.SetPerfMetricName("Orders.Order.GetMany." + i.ToString());
                        items = Order_Manager.GetManyOrderDataByCriteriaTest(dbOptions, Results);
                        TestHelper.ReportValueToConsole("Orders_Order_PerformanceTest: GetManyOrderDataByCriteria succeeded for Order. The count", items.Count.ToString());
                        ...
                        perfStr = "GetManyOrderDataByCriteria search time (in seconds) for Order count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                    ...
                    if (items != null)
                    {
                        TestHelper.ReportValueToConsole("Orders_Order_PerformanceTest: GetManyOrderDataByCriteria acceptable serialization time (in seconds)", acceptableTime);
                        try
                        {
                            // serialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Orders.Order.Serialize." + i.ToString());
                            ...
                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyOrderDataByCriteria serialization time (in seconds) for Order count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                        ...
                        TestHelper.ReportValueToConsole("Orders_Order_PerformanceTest: GetManyOrderDataByCriteria acceptable deserialization time (in seconds)", acceptableTime);
                        try
                        {
                            // deserialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Orders.Order.Deserialize." + i.ToString());
                            ...
                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyOrderDataByCriteria deserialization time (in seconds) for Order count " + itemCount + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                ...
                if (numGetDataSearches > 0)
                {
                    ...
                    TestHelper.ReportValueToConsole("Orders_Order_PerformanceTest: GetManyOrderDataByCriteria AVERAGE SEARCH TIME (in seconds)", avgGetDataSearchSecs);
                }
                else
                {
                    Console.WriteLine("Orders_Order_PerformanceTest: GetManyOrderDataByCriteria -- Could not perform search. NO AVERAGE SEARCH TIME AVAILABLE.");
                }
                if (numSerializations > 0)
                {
                    avgSerializeDataSecs /= numSerializations;
                    TestHelper.ReportValueToConsole("Orders_Order_PerformanceTest: GetManyOrderDataByCriteria AVERAGE SERIALIZATION TIME (in seconds)", avgSerializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Orders_Order_PerformanceTest: GetManyOrderDataByCriteria -- Could not serialize. NO AVERAGE SERIALIZATION TIME AVAILABLE.");
                }
                if (numDeserializations > 0)
                {
                    avgDeserializeDataSecs /= numDeserializations;
                    TestHelper.ReportValueToConsole("Orders_Order_PerformanceTest: GetManyOrderDataByCriteria AVERAGE DESERIALIZATION TIME (in seconds)", avgDeserializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Orders_Order_PerformanceTest: GetManyOrderDataByCriteria -- Could not deserialize. NO AVERAGE DESERIALIZATION TIME AVAILABLE.");
                }
                ...
                Console.WriteLine("******************** Orders_Order_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Orders_Order_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
