...
namespace MW.MComm.WalletSolution.UnitTest.Orders
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Orders"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Orders.OrderStatus))]
    public partial class OrderStatus_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public OrderStatus_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for OrderStatus.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Orders_OrderStatus_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Orders_OrderStatus_PerformanceTest START ********************");
                ...
                Console.WriteLine("GetAllOrderStatusData test count is {0}.", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Orders.OrderStatus> items = OrderStatus_Manager.GetAllOrderStatusDataTest(Results);
                        Console.WriteLine("GetAllOrderStatusData succeeded for OrderStatus.");
                        ...
                        items = (BLL.SortableList<BLL.Orders.OrderStatus>)oItems;
                        oItems = null;
                        items = null;
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllOrderStatusData failed for OrderStatus", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                Console.WriteLine("******************** Orders_OrderStatus_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Orders_OrderStatus_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
