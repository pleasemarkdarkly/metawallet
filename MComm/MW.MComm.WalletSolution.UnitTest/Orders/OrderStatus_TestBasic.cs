...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for OrderStatus instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Orders"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Orders.OrderStatus))]
    public partial class OrderStatus_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public OrderStatus_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for OrderStatus.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Orders_OrderStatus_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Orders_OrderStatus_BasicTest START ********************");
                ...
                BLL.Orders.OrderStatus item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Orders_OrderStatus_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Orders_OrderStatus_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Orders_OrderStatus_BasicTest: Add OrderStatus count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = OrderStatus_Manager.AddOneOrderStatusTest(true, Results);
                        if (Results.IsCollision == true)
                        {
                            throw new UnitTestException(UnitTestErrorType.Warning, "Add collision encountered for OrderStatus: " + item.PrimaryKey.ToString(), null);
                        }
                        else
                        {
                            MOD.Data.NamedObjectCollection entityID = new MOD.Data.NamedObjectCollection();
                            entityID["OrderStatusCode"] = item.OrderStatusCode;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Orders_OrderStatus_BasicTest: Add succeeded for OrderStatus", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Orders.OrderStatus)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for OrderStatus", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for OrderStatus", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if OrderStatus Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Orders_OrderStatus_BasicTest: Update OrderStatus count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = OrderStatus_Manager.UpdateOneOrderStatusTest((int)loopID["OrderStatusCode"], true, Results);
                        TestHelper.ReportValueToConsole("Orders_OrderStatus_BasicTest: Update succeeded for OrderStatus", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for OrderStatus", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Orders_OrderStatus_BasicTest: Delete OrderStatus count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = OrderStatus_Manager.DeleteOneOrderStatusTest((int)loopID["OrderStatusCode"], true, Results);
                            TestHelper.ReportValueToConsole("Orders_OrderStatus_BasicTest: Delete succeeded for OrderStatus", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for OrderStatus", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Orders_OrderStatus_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Orders_OrderStatus_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
