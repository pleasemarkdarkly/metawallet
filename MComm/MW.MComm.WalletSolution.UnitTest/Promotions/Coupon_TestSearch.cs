...
namespace MW.MComm.WalletSolution.UnitTest.Promotions
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Search.Promotions"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Promotions.Coupon))]
    public partial class Coupon_TestSearch : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the search entity test for Coupon.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Promotions_Coupon_SearchTest()
        {
            try
            {
                Console.WriteLine("******************** Promotions_Coupon_SearchTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Promotions_Coupon_SearchTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Promotions_Coupon_SearchTest: GetAllCouponData test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Promotions.Coupon> items = Coupon_Manager.GetAllCouponDataTest(Results);
                        TestHelper.ReportValueToConsole("Promotions_Coupon_SearchTest: GetAllCouponData succeeded for Coupon. The count", items.Count.ToString());
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllCouponData failed for Coupon", itemStr, ex, strExtraInfo);
                        }
                    }
                }
                loadCount = TestHelper.GetInt(Globals.MinimumGetAllCountPerEntity, Globals.MaximumGetAllCountPerEntity);
                TestHelper.ReportValueToConsole("Promotions_Coupon_SearchTest: GetManyCouponDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Promotions.Coupon> items = Coupon_Manager.GetManyCouponDataByCriteriaTest(Globals.getDataOptions("","Random",1,200,2000), Results);
                        TestHelper.ReportValueToConsole("Promotions_Coupon_SearchTest: GetManyCouponDataByCriteria succeeded for Coupon. The count", items.Count.ToString());
                ...
                Console.WriteLine("******************** Promotions_Coupon_SearchTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Promotions_Coupon_SearchTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
