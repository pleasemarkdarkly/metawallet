...
namespace MW.MComm.WalletSolution.UnitTest.Promotions
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Promotions"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Promotions.CouponType))]
    public partial class CouponType_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public CouponType_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for CouponType.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Promotions_CouponType_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Promotions_CouponType_PerformanceTest START ********************");
                ...
                Console.WriteLine("GetAllCouponTypeData test count is {0}.", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    try
                    {
                        BLL.SortableList<BLL.Promotions.CouponType> items = CouponType_Manager.GetAllCouponTypeDataTest(Results);
                        Console.WriteLine("GetAllCouponTypeData succeeded for CouponType.");
                        ...
                        items = (BLL.SortableList<BLL.Promotions.CouponType>)oItems;
                        oItems = null;
                        items = null;
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "GetAllCouponTypeData failed for CouponType", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                Console.WriteLine("******************** Promotions_CouponType_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Promotions_CouponType_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
