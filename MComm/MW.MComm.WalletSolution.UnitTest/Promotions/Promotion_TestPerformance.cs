...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for Promotion instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Performance.Promotions"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Promotions.Promotion))]
    public partial class Promotion_TestPerformance : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public Promotion_TestPerformance() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the performance entity test for Promotion.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Promotions_Promotion_PerformanceTest()
        {
            try
            {
                Console.WriteLine("******************** Promotions_Promotion_PerformanceTest START ********************");
                ...
                TestHelper.ReportValueToConsole("Promotions_Promotion_PerformanceTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                {
                    TestHelper.ReportValueToConsole("Promotions_Promotion_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString((UnitTestPerformanceLevel)(i + 1)) + " time (in seconds)", perfTimeLevels[i]);
                }
                TestHelper.ReportValueToConsole("Promotions_Promotion_PerformanceTest: " + TestHelper.GetUnitTestPerformanceLevelString(UnitTestPerformanceLevel.Unacceptable) + " time (in seconds) is beyond", perfTimeLevels[(((int)UnitTestPerformanceLevel.Unacceptable - 1) - 1)]);
                ...
                TestHelper.ReportValueToConsole("Promotions_Promotion_PerformanceTest: page size", pageSize);
                TestHelper.ReportValueToConsole("Promotions_Promotion_PerformanceTest: maxListSize", maxListSize);
                ...
                TestHelper.ReportValueToConsole("Promotions_Promotion_PerformanceTest: GetManyPromotionDataByCriteria test count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    long startTicks = 0;
                    long endTicks = 0;
                    double secondsInterval = 0.00;
                    BLL.SortableList<BLL.Promotions.Promotion> items = null;

                    TestHelper.ReportValueToConsole("Promotions_Promotion_PerformanceTest: Loop Iteration", i);
                    ...
                    TestHelper.ReportValueToConsole("Promotions_Promotion_PerformanceTest: GetManyPromotionDataByCriteria acceptable search time (in seconds)", acceptableTime);
                    try
                    {
                        // get many
                        Results.ResetCurrentPerfMetric();
                        Results.SetPerfMetricName("Promotions.Promotion.GetMany." + i.ToString());
                        items = Promotion_Manager.GetManyPromotionDataByCriteriaTest(dbOptions, Results);
                        TestHelper.ReportValueToConsole("Promotions_Promotion_PerformanceTest: GetManyPromotionDataByCriteria succeeded for Promotion. The count", items.Count.ToString());
                        ...
                        perfStr = "GetManyPromotionDataByCriteria search time (in seconds) for Promotion count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional,"GetManyPromotionDataByCriteria failed for Promotion", itemStr, ex, strExtraInfo);
                        }
                    }

                    if (items != null)
                    {
                        TestHelper.ReportValueToConsole("Promotions_Promotion_PerformanceTest: GetManyPromotionDataByCriteria acceptable serialization time (in seconds)", acceptableTime);
                        try
                        {
                            // serialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Promotions.Promotion.Serialize." + i.ToString());
                            ...
                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyPromotionDataByCriteria serialization time (in seconds) for Promotion count " + items.Count + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                        ...
                        TestHelper.ReportValueToConsole("Promotions_Promotion_PerformanceTest: GetManyPromotionDataByCriteria acceptable deserialization time (in seconds)", acceptableTime);
                        try
                        {
                            // deserialize
                            Results.ResetCurrentPerfMetric();
                            Results.SetPerfMetricName("Promotions.Promotion.Deserialize." + i.ToString());
                            ...
                            items = (BLL.SortableList<BLL.Promotions.Promotion>)oItems;
                            int itemCount = items.Count;
                            oItems = null;
                            items = null;

                            perfLevel = TestHelper.GetPerformanceLevelFromTime(avgCallsTime, perfTimeLevels);
                            perfStr = "GetManyPromotionDataByCriteria deserialization time (in seconds) for Promotion count " + itemCount + " was " + avgCallsTime + ". " + TestHelper.GetUnitTestPerformanceLevelString(perfLevel) + ".";
                            throw new UnitTestException(TestHelper.GetUnitTestErrorTypeForPerformanceLevel(perfLevel), perfStr, null);
                        }
                        catch (System.Exception ex)
                        {
                ...
                if (numGetDataSearches > 0)
                {
                    ...
                    TestHelper.ReportValueToConsole("Promotions_Promotion_PerformanceTest: GetManyPromotionDataByCriteria AVERAGE SEARCH TIME (in seconds)", avgGetDataSearchSecs);
                }
                else
                {
                    Console.WriteLine("Promotions_Promotion_PerformanceTest: GetManyPromotionDataByCriteria -- Could not perform search. NO AVERAGE SEARCH TIME AVAILABLE.");
                }
                if (numSerializations > 0)
                {
                    avgSerializeDataSecs /= numSerializations;
                    TestHelper.ReportValueToConsole("Promotions_Promotion_PerformanceTest: GetManyPromotionDataByCriteria AVERAGE SERIALIZATION TIME (in seconds)", avgSerializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Promotions_Promotion_PerformanceTest: GetManyPromotionDataByCriteria -- Could not serialize. NO AVERAGE SERIALIZATION TIME AVAILABLE.");
                }
                if (numDeserializations > 0)
                {
                    avgDeserializeDataSecs /= numDeserializations;
                    TestHelper.ReportValueToConsole("Promotions_Promotion_PerformanceTest: GetManyPromotionDataByCriteria AVERAGE DESERIALIZATION TIME (in seconds)", avgDeserializeDataSecs);
                }
                else
                {
                    Console.WriteLine("Promotions_Promotion_PerformanceTest: GetManyPromotionDataByCriteria -- Could not deserialize. NO AVERAGE DESERIALIZATION TIME AVAILABLE.");
                }
                ...
                Console.WriteLine("******************** Promotions_Promotion_PerformanceTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Promotions_Promotion_PerformanceTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
