...
namespace MW.MComm.WalletSolution.UnitTest.Promotions
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Promotions"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Promotions.DiscountType))]
    public partial class DiscountType_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public DiscountType_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for DiscountType.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Promotions_DiscountType_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Promotions_DiscountType_BasicTest START ********************");
                ...
                BLL.Promotions.DiscountType item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Promotions_DiscountType_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Promotions_DiscountType_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Promotions_DiscountType_BasicTest: Add DiscountType count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = DiscountType_Manager.AddOneDiscountTypeTest(true, Results);
                        if (Results.IsCollision == true)
                        {
                            throw new UnitTestException(UnitTestErrorType.Warning, "Add collision encountered for DiscountType: " + item.PrimaryKey.ToString(), null);
                        }
                        else
                        {
                            MOD.Data.NamedObjectCollection entityID = new MOD.Data.NamedObjectCollection();
                            entityID["DiscountTypeCode"] = item.DiscountTypeCode;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Promotions_DiscountType_BasicTest: Add succeeded for DiscountType", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Promotions.DiscountType)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for DiscountType", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for DiscountType", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if DiscountType Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Promotions_DiscountType_BasicTest: Update DiscountType count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = DiscountType_Manager.UpdateOneDiscountTypeTest((int)loopID["DiscountTypeCode"], true, Results);
                        TestHelper.ReportValueToConsole("Promotions_DiscountType_BasicTest: Update succeeded for DiscountType", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for DiscountType", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Promotions_DiscountType_BasicTest: Delete DiscountType count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = DiscountType_Manager.DeleteOneDiscountTypeTest((int)loopID["DiscountTypeCode"], true, Results);
                            TestHelper.ReportValueToConsole("Promotions_DiscountType_BasicTest: Delete succeeded for DiscountType", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for DiscountType", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Promotions_DiscountType_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Promotions_DiscountType_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
