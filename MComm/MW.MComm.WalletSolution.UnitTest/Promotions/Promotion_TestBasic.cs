...
namespace MW.MComm.WalletSolution.UnitTest.Promotions
{
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Promotions"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Promotions.Promotion))]
    public partial class Promotion_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public Promotion_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for Promotion.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Promotions_Promotion_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Promotions_Promotion_BasicTest START ********************");
                ...
                BLL.Promotions.Promotion item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Promotions_Promotion_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Promotions_Promotion_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Promotions_Promotion_BasicTest: Add Promotion count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = Promotion_Manager.AddOnePromotionTest(true, Results);
                        ...
                        else
                        {
                            ...
                            entityID["PromotionCode"] = item.PromotionCode;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Promotions_Promotion_BasicTest: Add succeeded for Promotion", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Promotions.Promotion)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for Promotion", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for Promotion", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if Promotion Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Promotions_Promotion_BasicTest: Update Promotion count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = Promotion_Manager.UpdateOnePromotionTest((int)loopID["PromotionCode"], true, Results);
                        TestHelper.ReportValueToConsole("Promotions_Promotion_BasicTest: Update succeeded for Promotion", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for Promotion", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Promotions_Promotion_BasicTest: Delete Promotion count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = Promotion_Manager.DeleteOnePromotionTest((int)loopID["PromotionCode"], true, Results);
                            TestHelper.ReportValueToConsole("Promotions_Promotion_BasicTest: Delete succeeded for Promotion", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for Promotion", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Promotions_Promotion_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Promotions_Promotion_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
