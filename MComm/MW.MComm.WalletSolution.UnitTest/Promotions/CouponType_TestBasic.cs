...
{
    // ------------------------------------------------------------------------------
    /// <summary>This class is used to run BLL method tests for CouponType instances.</summary>
    ///
    /// File History:
    ...
    /// <remarks></remarks>
    // ------------------------------------------------------------------------------
    [TestFixture, FixtureCategory("Basic.Promotions"), Author("Common"), Importance(TestImportance.Serious), TestsOn(typeof(BLL.Promotions.CouponType))]
    public partial class CouponType_TestBasic : BaseUnitTest
    {
        ...
        // ------------------------------------------------------------------------------
        public CouponType_TestBasic() : base()
        {
        }
        ...
        // ------------------------------------------------------------------------------
        /// <summary>This method is the basic entity test for CouponType.</summary>
        // ------------------------------------------------------------------------------
        [Test]
        public void Promotions_CouponType_BasicTest()
        {
            try
            {
                ...
                Console.WriteLine("******************** Promotions_CouponType_BasicTest START ********************");
                ...
                BLL.Promotions.CouponType item = null;
                int currentTicks = (int)DateTime.Now.Ticks;
                TestHelper.ReportValueToConsole("Promotions_CouponType_BasicTest: Value generation method: Random seed -- Current ticks", currentTicks.ToString());
                ...
                TestHelper.ReportValueToConsole("Promotions_CouponType_BasicTest: Next entity add load count", loadCount.ToString());
                ...
                // add items
                //
                TestHelper.ReportValueToConsole("Promotions_CouponType_BasicTest: Add CouponType count", loadCount.ToString());
                for (int i=0; i < loadCount; i++)
                {
                    Results.IsCollision = false;
                    try
                    {
                        item = CouponType_Manager.AddOneCouponTypeTest(true, Results);
                        if (Results.IsCollision == true)
                        {
                            throw new UnitTestException(UnitTestErrorType.Warning, "Add collision encountered for CouponType: " + item.PrimaryKey.ToString(), null);
                        }
                        else
                        {
                            MOD.Data.NamedObjectCollection entityID = new MOD.Data.NamedObjectCollection();
                            entityID["CouponTypeCode"] = item.CouponTypeCode;
                            idList.Add(entityID);
                            TestHelper.ReportValueToConsole("Promotions_CouponType_BasicTest: Add succeeded for CouponType", item.PrimaryKey.ToString());
                            ...
                            item = (BLL.Promotions.CouponType)oItem;
                            oItem = null;
                            item = null;
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Serialization failed for CouponType", itemStr, serialEx, strExtraInfo);
                        }
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Add failed for CouponType", itemStr, ex, strExtraInfo);
                        }
                    }
                }

                // TODO: Assert Warning if CouponType Add count is not the same as Update count

                //
                // get and update items
                //
                int loopIndex = 0;
                TestHelper.ReportValueToConsole("Promotions_CouponType_BasicTest: Update CouponType count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    try
                    {
                        item = CouponType_Manager.UpdateOneCouponTypeTest((int)loopID["CouponTypeCode"], true, Results);
                        TestHelper.ReportValueToConsole("Promotions_CouponType_BasicTest: Update succeeded for CouponType", item.PrimaryKey.ToString());
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        {
                            UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Update failed for CouponType", itemStr, ex, strExtraInfo);
                        }
                    }
                    loopIndex++;
                }
                ...
                //
                loopIndex = 0;
                TestHelper.ReportValueToConsole("Promotions_CouponType_BasicTest: Delete CouponType count", idList.Count.ToString());
                foreach (MOD.Data.NamedObjectCollection loopID in idList)
                {
                    ...
                    if (Globals.DeleteAllCreatedRecords == true || TestHelper.GetBool() == true)
                    {
                        try
                        {
                            item = CouponType_Manager.DeleteOneCouponTypeTest((int)loopID["CouponTypeCode"], true, Results);
                            TestHelper.ReportValueToConsole("Promotions_CouponType_BasicTest: Delete succeeded for CouponType", item.PrimaryKey);
                        }
                        catch (System.Exception ex)
                        {
                            string strExtraInfo = "Loop iteration #" + loopIndex.ToString();
                            ...
                            {
                                UpdateUnitTestErrorInfo(UnitTestErrorType.Functional, "Delete failed for CouponType", itemStr, ex, strExtraInfo);
                            }
                        }
                    }
                    loopIndex++;
                ...
                Console.WriteLine("******************** Promotions_CouponType_BasicTest END **********************");
                Console.WriteLine();
                Console.WriteLine();
                if ((ErrorIndex > 0) || (WarningIndex > 0))
                {
                    Console.WriteLine("==================== Promotions_CouponType_BasicTest Error/Warning Details ====");
                    Console.WriteLine();
                }
            }
            catch (System.Exception ex)
