...
namespace MW.MComm.WalletSolution.WebService.Promotions
{
	...
	public class DiscountTypeManager : System.Web.Services.WebService
	{
		...
		///
		/// <param name="item">The DiscountType to be managed.</param>
		/// <param name="performCascadeOperation">Flag, indicating if a cascade operation should be performed on the DiscountType item.</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Promotions.DiscountTypeResults AddOneDiscountType(BLL.Promotions.DiscountType item, bool performCascadeOperation)
		{
			...
			try
			{
				...
				BLL.Promotions.DiscountTypeManager.AddOneDiscountType(item, performCascadeOperation);
		...
		/// <param name="performCascadeOperation">Flag, indicating if a cascade delete should be performed on the DiscountType item.</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Promotions.DiscountTypeResults DeleteOneDiscountType(BLL.Promotions.DiscountType item, bool performCascadeOperation)
		{
			Components.Promotions.DiscountTypeResults results = new Components.Promotions.DiscountTypeResults();
			results.StatusDescription = "Results OK";
			try
			{
				// perform main method tasks
				BLL.Promotions.DiscountTypeManager.DeleteOneDiscountType(item, performCascadeOperation);
			}
			catch (System.Exception ex)
			{
				results.StatusCode = -1;
		...
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Promotions.DiscountTypeResults UpdateOneDiscountType(BLL.Promotions.DiscountType item, bool performCascadeOperation)
		{
			Components.Promotions.DiscountTypeResults results = new Components.Promotions.DiscountTypeResults();
			results.StatusDescription = "Results OK";
			try
			{
				// perform main method tasks
				BLL.Promotions.DiscountTypeManager.UpdateOneDiscountType(item, performCascadeOperation);
				results.Results.Add(item);
			}
			catch (System.Exception ex)
			{
