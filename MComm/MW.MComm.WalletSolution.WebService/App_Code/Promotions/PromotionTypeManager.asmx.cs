...
namespace MW.MComm.WalletSolution.WebService.Promotions
{
	...
	public class PromotionTypeManager : System.Web.Services.WebService
	{
		...
		///
		/// <param name="item">The PromotionType to be managed.</param>
		/// <param name="performCascadeOperation">Flag, indicating if a cascade operation should be performed on the PromotionType item.</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Promotions.PromotionTypeResults AddOnePromotionType(BLL.Promotions.PromotionType item, bool performCascadeOperation)
		{
			...
			try
			{
				...
				BLL.Promotions.PromotionTypeManager.AddOnePromotionType(item, performCascadeOperation);
		...
		/// <param name="performCascadeOperation">Flag, indicating if a cascade delete should be performed on the PromotionType item.</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Promotions.PromotionTypeResults DeleteOnePromotionType(BLL.Promotions.PromotionType item, bool performCascadeOperation)
		{
			Components.Promotions.PromotionTypeResults results = new Components.Promotions.PromotionTypeResults();
			results.StatusDescription = "Results OK";
			try
			{
				// perform main method tasks
				BLL.Promotions.PromotionTypeManager.DeleteOnePromotionType(item, performCascadeOperation);
			}
			catch (System.Exception ex)
			{
				results.StatusCode = -1;
		...
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Promotions.PromotionTypeResults UpdateOnePromotionType(BLL.Promotions.PromotionType item, bool performCascadeOperation)
		{
			Components.Promotions.PromotionTypeResults results = new Components.Promotions.PromotionTypeResults();
			results.StatusDescription = "Results OK";
			try
			{
				// perform main method tasks
				BLL.Promotions.PromotionTypeManager.UpdateOnePromotionType(item, performCascadeOperation);
				results.Results.Add(item);
			}
			catch (System.Exception ex)
			{
