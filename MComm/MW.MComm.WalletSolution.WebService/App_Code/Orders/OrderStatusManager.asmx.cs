...
namespace MW.MComm.WalletSolution.WebService.Orders
{
	...
	public class OrderStatusManager : System.Web.Services.WebService
	{
		...
		///
		/// <param name="item">The OrderStatus to be managed.</param>
		/// <param name="performCascadeOperation">Flag, indicating if a cascade operation should be performed on the OrderStatus item.</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Orders.OrderStatusResults AddOneOrderStatus(BLL.Orders.OrderStatus item, bool performCascadeOperation)
		{
			...
			try
			{
				...
				BLL.Orders.OrderStatusManager.AddOneOrderStatus(item, performCascadeOperation);
		...
		/// <param name="performCascadeOperation">Flag, indicating if a cascade delete should be performed on the OrderStatus item.</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Orders.OrderStatusResults DeleteOneOrderStatus(BLL.Orders.OrderStatus item, bool performCascadeOperation)
		{
			Components.Orders.OrderStatusResults results = new Components.Orders.OrderStatusResults();
			results.StatusDescription = "Results OK";
			try
			{
				// perform main method tasks
				BLL.Orders.OrderStatusManager.DeleteOneOrderStatus(item, performCascadeOperation);
			}
			catch (System.Exception ex)
			{
				results.StatusCode = -1;
		...
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Orders.OrderStatusResults UpdateOneOrderStatus(BLL.Orders.OrderStatus item, bool performCascadeOperation)
		{
			Components.Orders.OrderStatusResults results = new Components.Orders.OrderStatusResults();
			results.StatusDescription = "Results OK";
			try
			{
				// perform main method tasks
				BLL.Orders.OrderStatusManager.UpdateOneOrderStatus(item, performCascadeOperation);
				results.Results.Add(item);
			}
			catch (System.Exception ex)
			{
