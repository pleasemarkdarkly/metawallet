...
namespace MW.MComm.WalletSolution.WebService.Notifications
{
	...
	public class NotificationLogManager : System.Web.Services.WebService
	{
		...
		///
		/// <param name="item">The NotificationLog to be managed.</param>
		/// <param name="performCascadeOperation">Flag, indicating if a cascade operation should be performed on the NotificationLog item.</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Notifications.NotificationLogResults LogOneNotificationLog(BLL.Notifications.NotificationLog item, bool performCascadeOperation)
		{
			Components.Notifications.NotificationLogResults results = new Components.Notifications.NotificationLogResults();
			results.StatusDescription = "Results OK";
			try
			{
				// perform main method tasks
				BLL.Notifications.NotificationLogManager.LogOneNotificationLog(item, performCascadeOperation);
				results.Results.Add(item);
			}
			catch (System.Exception ex)
			{
