...
namespace MW.MComm.WalletTest.WebService.Users
{
	...
	public class UserManager : System.Web.Services.WebService
	{
		...
		///
		/// <param name="userID">A property of User item to be managed</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Users.UserResults DeleteOneUser(string userID)
		{
			Components.Users.UserResults results = new Components.Users.UserResults();
			results.StatusDescription = "Results OK";
			try
			{
				...
				item.UserID = MOD.Data.DataHelper.GetGuid(userID, MOD.Data.DefaultValue.Guid);
				BLL.Users.UserManager.DeleteOneUser(item, false);
			}
			catch (System.Exception ex)
			{
				results.StatusCode = -1;
		...
		///
		/// <param name="userID">A property of User item to be managed</param>
		/// <param name="userName">A property of User item to be managed</param>
		/// <param name="firstName">A property of User item to be managed</param>
		/// <param name="lastName">A property of User item to be managed</param>
		/// <param name="password">A property of User item to be managed</param>
		/// <param name="localeCode">A property of User item to be managed</param>
		/// <param name="isActive">A property of User item to be managed</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Users.UserResults UpsertOneUser(string userID, string userName, string firstName, string lastName, string password, int localeCode, bool isActive)
		{
			Components.Users.UserResults results = new Components.Users.UserResults();
			results.StatusDescription = "Results OK";
			try
			{
				...
				BLL.Users.UserManager.UpsertOneUser(item, false);
				results.Results.Add(item);
			}
			catch (System.Exception ex)
			{
