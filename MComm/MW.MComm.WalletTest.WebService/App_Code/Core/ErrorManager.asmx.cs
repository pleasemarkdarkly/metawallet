...
namespace MW.MComm.WalletTest.WebService.Core
{
	...
	public class ErrorManager : System.Web.Services.WebService
	{
		...
		///
		/// <param name="errorNumber">A property of Error item to be managed</param>
		/// <param name="errorTitle">A property of Error item to be managed</param>
		/// <param name="errorMessage">A property of Error item to be managed</param>
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Core.ErrorResults AddOneError(int errorNumber, string errorTitle, string errorMessage)
		{
			...
			try
			{
				...
				BLL.Core.Error item = new BLL.Core.Error();
				item.ErrorNumber = errorNumber;
				item.ErrorTitle = errorTitle;
				item.ErrorMessage = errorMessage;
				BLL.Core.ErrorManager.AddOneError(item, false);
		...
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Core.ErrorResults DeleteOneError(int errorNumber)
		{
			Components.Core.ErrorResults results = new Components.Core.ErrorResults();
			results.StatusDescription = "Results OK";
			try
			{
				...
				BLL.Core.Error item = new BLL.Core.Error();
				item.ErrorNumber = errorNumber;
				BLL.Core.ErrorManager.DeleteOneError(item, false);
			}
			catch (System.Exception ex)
			{
				results.StatusCode = -1;
		...
		// ------------------------------------------------------------------
		[WebMethod]
		public Components.Core.ErrorResults UpdateOneError(int errorNumber, string errorTitle, string errorMessage)
		{
			Components.Core.ErrorResults results = new Components.Core.ErrorResults();
			results.StatusDescription = "Results OK";
			try
			{
				...
				item.ErrorTitle = errorTitle;
				item.ErrorMessage = errorMessage;
				BLL.Core.ErrorManager.UpdateOneError(item, false);
				results.Results.Add(item);
			}
			catch (System.Exception ex)
			{
