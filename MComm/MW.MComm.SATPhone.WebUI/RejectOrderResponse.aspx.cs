...
namespace MW.MComm.SATPhone.WebUI
{
	...
	public partial class PayPhoneToPhone : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			{
				txtStatus.Text = "";
				customerPIN = Request.Form.Get("PIN");
				...
				order.Cancel(customerPhone, customerPIN);
				...
				try
				{
					MOD.Data.NamedObjectCollection eventData = BLL.Notifications.NotificationGenerator.SendNotificationsForOrderEvent(BLL.Events.EventCode.PaymentCancelled, order, order.DebitMetaPartner, order.CreditMetaPartner, Globals.MailServer, Globals.MailSenderEmail, Globals.SMSSenderPhone);

					txtStatus.Text = "\r\nA payment of " + order.OrderAmount.ToString() + " " + order.Currency.CurrencyName + " from " + customerPhone + " to " + eventData[BLL.Events.EventAttributeCode.DestinationPhone.ToString("D")] + " was rejected.";
				}
				catch
				{
