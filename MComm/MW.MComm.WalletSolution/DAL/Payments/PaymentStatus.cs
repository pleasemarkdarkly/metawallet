...
namespace MW.MComm.WalletSolution.DAL.Payments
{
	...
	public partial class PaymentStatus : Utility.BaseDataAccessObject
	{
		...
		// ------------------------------------------------------------------
		public static void AddOnePaymentStatus(PaymentStatus item, bool performCascadeOperation, MOD.Data.DatabaseOptions dbOptions, int debugLevel)
		{
			...
			try
			{
				...
				AddOnePaymentStatus(item, performCascadeOperation, dbAdapter);
			...
			catch (System.Exception ex)
			{
				...
				throw Utility.CustomAppException.WrapException(ex, "DAL.Payments.AddOnePaymentStatus");
			}
			finally
			{
		...
		/// <exception cref="System.Exception">Thrown when prior exceptions are thrown</exception>
		// ------------------------------------------------------------------
		public static void AddOnePaymentStatus(PaymentStatus item, bool performCascadeOperation, MOD.Data.SqlProcAdapter dbAdapter)
		{
			try
			{
				// perform main method tasks
				MOD.Data.NamedObjectCollection spParameters = new MOD.Data.NamedObjectCollection();
				dbAdapter.ExecuteProcNQ("spPayments_AddOnePaymentStatus", item, spParameters);
		...
		/// <exception cref="System.Exception">Thrown when prior exceptions are thrown</exception>
		// ------------------------------------------------------------------
		public static void DeleteOnePaymentStatus(PaymentStatus item, bool performCascadeOperation, MOD.Data.DatabaseOptions dbOptions, int debugLevel)
		{
			Utility.SqlManager dbManager = null;
			MOD.Data.SqlProcAdapter dbAdapter = null;
			try
			{
				...
				dbAdapter.Transaction = dbAdapter.Connection.BeginTransaction();
				// perform main method tasks
				DeleteOnePaymentStatus(item, performCascadeOperation, dbAdapter);
				// commit the transaction
				dbAdapter.Transaction.Commit();
			}
			catch (System.Exception ex)
			{
				...
				throw Utility.CustomAppException.WrapException(ex, "DAL.Payments.DeleteOnePaymentStatus");
			}
			finally
			{
				dbManager.ClearAdapter();
		...
		/// <exception cref="System.Exception">Thrown when prior exceptions are thrown</exception>
		// ------------------------------------------------------------------
		public static void DeleteOnePaymentStatus(PaymentStatus item, bool performCascadeOperation, MOD.Data.SqlProcAdapter dbAdapter)
		{
			try
			{
				// perform main method tasks
				...
				#endregion Cascade Operations
				MOD.Data.NamedObjectCollection spParameters = new MOD.Data.NamedObjectCollection();
				dbAdapter.ExecuteProcNQ("spPayments_DeleteOnePaymentStatus", item, spParameters);
				// perform sql output error handling and get output parameters
				Globals.CheckThrowSqlError(spParameters);
				#region Cascade Operations
				if (performCascadeOperation == true)
		...
		/// <exception cref="System.Exception">Thrown when prior exceptions are thrown</exception>
		// ------------------------------------------------------------------
		public static MOD.Data.SortableObjectCollection GetAllPaymentStatusData(MOD.Data.DatabaseOptions dbOptions, MOD.Data.DataOptions dataOptions, int debugLevel)
		{
			Utility.SqlManager dbManager = null;
			MOD.Data.SqlProcAdapter dbAdapter = null;
			try
			{
				...
				DataTable dt = dbAdapter.ExecuteProc("spPayments_GetAllPaymentStatusData", null, spParameters);
				// perform sql output error handling and get output parameters
				Globals.CheckThrowSqlError(spParameters);
				...
				{
					// get item
					PaymentStatus item = (PaymentStatus) DataTransformAttribute.Transform(row, new PaymentStatus(), false, filterFields);
					items.Add(item);
				}
				return items;
			}
			...
			{
				throw Utility.CustomAppException.WrapException(ex, "DAL.Payments.GetAllPaymentStatusData");
			}
			finally
			{
				dbManager.ClearAdapter();
		...
		///
		/// <param name="paymentStatusCode">A key for PaymentStatus items to be fetched</param>
		/// <param name="dbOptions">Database connection information</param>
		/// <param name="dataOptions">Data retrieval options</param>
		/// <param name="debugLevel">Debug level on error</param>
		/// <exception cref="CustomAppException">Thrown when an error is encountered in SQL</exception>
		...
		/// <exception cref="System.Exception">Thrown when prior exceptions are thrown</exception>
		// ------------------------------------------------------------------
		public static PaymentStatus GetOnePaymentStatus(int paymentStatusCode, MOD.Data.DatabaseOptions dbOptions, MOD.Data.DataOptions dataOptions, int debugLevel)
		{
			Utility.SqlManager dbManager = null;
			MOD.Data.SqlProcAdapter dbAdapter = null;
			try
			{
				...
				DataTable dt = dbAdapter.ExecuteProc("spPayments_GetOnePaymentStatus", null, spParameters);
				// perform sql output error handling and get output parameters
				Globals.CheckThrowSqlError(spParameters);
				item = null;
				// get output item
				...
				{
					// get item
					item = (PaymentStatus) DataTransformAttribute.Transform(row, new PaymentStatus(), false, filterFields);
					break;
				}
				return item;
			}
			...
			{
				throw Utility.CustomAppException.WrapException(ex, "DAL.Payments.GetOnePaymentStatus");
			}
			finally
			{
				dbManager.ClearAdapter();
		...
		/// <exception cref="System.Exception">Thrown when prior exceptions are thrown</exception>
		// ------------------------------------------------------------------
		public static void UpdateOnePaymentStatus(PaymentStatus item, bool performCascadeOperation, MOD.Data.DatabaseOptions dbOptions, int debugLevel)
		{
			Utility.SqlManager dbManager = null;
			MOD.Data.SqlProcAdapter dbAdapter = null;
			try
			{
				...
				dbAdapter.Transaction = dbAdapter.Connection.BeginTransaction();
				// perform main method tasks
				UpdateOnePaymentStatus(item, performCascadeOperation, dbAdapter);
				// commit the transaction
				dbAdapter.Transaction.Commit();
			}
			catch (System.Exception ex)
			{
				...
				throw Utility.CustomAppException.WrapException(ex, "DAL.Payments.UpdateOnePaymentStatus");
			}
			finally
			{
				dbManager.ClearAdapter();
		...
		/// <exception cref="System.Exception">Thrown when prior exceptions are thrown</exception>
		// ------------------------------------------------------------------
		public static void UpdateOnePaymentStatus(PaymentStatus item, bool performCascadeOperation, MOD.Data.SqlProcAdapter dbAdapter)
		{
			try
			{
				// perform main method tasks
				MOD.Data.NamedObjectCollection spParameters = new MOD.Data.NamedObjectCollection();
				dbAdapter.ExecuteProcNQ("spPayments_UpdateOnePaymentStatus", item, spParameters);
				// perform sql output error handling and get output parameters
				Globals.CheckThrowSqlError(spParameters);
				item.Timestamp = ((System.Byte[])(spParameters["Timestamp"]));
				#region Cascade Operations
