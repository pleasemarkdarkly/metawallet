...
namespace MW.MComm.WalletSolution.BLL.Accounts
{
	...
	public partial class StoredValueAccount : BLL.Accounts.DebitAccount
	{
		...
		protected decimal? _carrierBalance = null;
		// ------------------------------------------------------------------------------
		...
		[XmlElementAttribute()]
		[DataTransform]
		public virtual decimal? CarrierBalance
		{
			get
			{
				if (_carrierBalance == null)
				{
					...
					MComm.Nuevatel.CustomerManager.balance1Response response = s_CustomerManager.wsbalanceEnquiry1(this.MetaPartner.PrimaryPhone.PhoneNumber, this.AccountID.ToString());
					_carrierBalance = response.balance;
				}
				return _carrierBalance;
			}
			set
			{
				if (_carrierBalance != value)
				{
					_carrierBalance = value;
				}
			}
		}
		#region Methods
