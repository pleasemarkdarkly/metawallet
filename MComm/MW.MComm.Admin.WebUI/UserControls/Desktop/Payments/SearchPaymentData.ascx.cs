...
{
	// ------------------------------------------------------------------------------
	/// <summary>Search Payment Data is used to help manage Payment information.</summary>
	///
	/// File History:
	...
	/// <remarks></remarks>
	// ------------------------------------------------------------------------------
	public partial class SearchPaymentData  : Components.Desktop.BaseSearchUserControl
	{
		#region Fields
		#endregion Fields
		#region Properties
		...
		// ------------------------------------------------------------------------------
		/// <summary>Load page for Payment search.</summary>
		///
		// ------------------------------------------------------------------------------
		private void Page_Load(object sender, System.EventArgs e)
		{
			try
			{
				workingPaymentData.Visible = ProvideWorkingSet;
				base.OnLoad(); // base list control manages loading data
				if (WorkflowMode != WorkflowMode.Internal)
				{
					if (Session["SearchPayments/PaymentData"] != null)
					{
						_collection = (BLL.SortableList<BLL.Payments.Payment>)Session["SearchPayments/PaymentData"];
					}
				}
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "SearchPaymentData.Page_Load"));
			}
			finally
			{
			}
		...
		private void editPayment_PaymentAdded(BaseFormlUserControl sender, EntityEditEventArgs e)
		{
			...
			{
				Globals.AddErrorMessage(this.Page, "Collection already contains this Payment", new object[0]);
			}
			else
			{
		...
		private void dgPrimaryList_ItemDataBound(object sender, DataGridItemEventArgs e)
		{
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "SearchPaymentData.dgPrimaryList_ItemDataBound"));
			}
			finally
			{
			}
		...
		// ------------------------------------------------------------------------------
		/// <summary>Set Payment information from the ui.</summary>
		///
		// ------------------------------------------------------------------------------
		protected override void SetDataFromForm()
		{
			Session["PaymentStatusCode"] = GetSelectedValueFromDropDownList(ddlPaymentStatusCode);
		...
		protected override void LoadCollectionFromDatabase()
		{
			try
			{
				...
				int? paymentStatusCode = MOD.Data.SearchHelper.GetInt(Session["PaymentStatusCode"]);
				...
				{
					_collection = BLL.Payments.PaymentManager.GetManyPaymentDataByCriteria(paymentStatusCode, lastModifiedDateStart, lastModifiedDateEnd, out totalRecords, out maximumListSizeExceeded, this.DataOptions);
					Session["SearchPayments/PaymentData"] = _collection;
		...
		public void dgPrimaryList_ItemCommand(object sender, EventArgs e)
		{
			...
			if (args.CommandName == "Add" || args.CommandName == "Edit" || args.CommandName == "Delete")
			{
				...
				{
					workingPaymentData.AddItem(item);
				}
		...
		private void Page_PreRender(object sender, EventArgs e)
		{
			try
			{
				...
				if (IsPostBack == false)
				{
					if (ddlPaymentStatusCode.Visible == true)
					{
						Components.Lists.DataBind(ddlPaymentStatusCode, BLL.Payments.PaymentStatusManager.GetAllPaymentStatusData(Globals.DBOptions, Globals.getDataOptions("PaymentStatusName", "", true, true), Globals.DebugLevel, Globals.CurrentUserID), "PrimaryKey", "PrimaryName", true, false);
						ddlPaymentStatusCode.ClearSelection();
						try
						{
							ddlPaymentStatusCode.SelectedValue = MOD.Data.DataHelper.GetString(Session["PaymentStatusCode"], "0");
						}
						catch {}
					}
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "SearchPaymentData.Page_PreRender"));
			}
			finally
			{
			}
		...
		// ------------------------------------------------------------------------------
		/// <summary>Clear search form for Payment.</summary>
		///
		// ------------------------------------------------------------------------------
		...
		// ------------------------------------------------------------------------------
		/// <summary>Perform search for Payment items.</summary>
		///
		// ------------------------------------------------------------------------------
		private void btnSearch_Click(object sender, EventArgs e)
		{
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "SearchPaymentData.btnSearch_Click"));
			}
			finally
			{
			}
