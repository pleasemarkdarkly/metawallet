...
{
	// ------------------------------------------------------------------------------
	/// <summary>Working Meta Partner Data is used to help manage MetaPartner information.</summary>
	///
	/// File History:
	...
	/// <remarks></remarks>
	// ------------------------------------------------------------------------------
	public partial class WorkingMetaPartnerData  : Components.Desktop.BaseListUserControl
	{
		#region Fields
		#endregion Fields
		#region Properties
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "WorkingMetaPartnerData.Page_Load"));
			}
			finally
			{
			}
		...
		private void dgPrimaryList_ItemDataBound(object sender, DataGridItemEventArgs e)
		{
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "WorkingMetaPartnerData.dgPrimaryList_ItemDataBound"));
			}
			finally
			{
			}
		...
		{
			_collection = (BLL.SortableList<BLL.Customers.MetaPartner>) Session["Customers/MetaPartnerWorkingSet"];
			if (_collection != null)
			{
				((BLL.SortableList<BLL.Customers.MetaPartner>)_collection).Sort(SortColumn, SortDirection);
		...
		public void dgPrimaryList_ItemCommand(object sender, EventArgs e)
		{
			...
			if (_collection != null)
			{
				...
				else if (args.CommandName == "Select")
				{
					int hashCode = int.Parse(args.CommandArgument.ToString());
					BLL.Customers.MetaPartner item = ((BLL.SortableList<BLL.Customers.MetaPartner>)_collection).Find(hashCode);
					Session["Customers/MetaPartnerWorkingSetItem"] = item.PrimaryIndex;
				}
			}
		}
		// ------------------------------------------------------------------------------
		...
		///
		// ------------------------------------------------------------------------------
		public void AddItem(BLL.Customers.MetaPartner item)
		{
			_collection = (BLL.SortableList<BLL.Customers.MetaPartner>) Session["Customers/MetaPartnerWorkingSet"];
			if (_collection == null)
			{
				_collection = new BLL.SortableList<BLL.Customers.MetaPartner>();
			}
			...
			else
			{
				...
				TotalRecords = _collection.Count;
				Session["Customers/MetaPartnerWorkingSetItem"] = item.PrimaryIndex;
				Globals.AddStatusMessage("Meta Partner working set item added.");
			}
			Session["Customers/MetaPartnerWorkingSet"] = _collection;
		}
		// ------------------------------------------------------------------------------
		/// <summary>Item command event handler for Meta Partner list.</summary>
		///
		// ------------------------------------------------------------------------------
		public void RemoveItem(BLL.Customers.MetaPartner item)
		{
			Globals.ClearMessages();
			_collection = (BLL.SortableList<BLL.Customers.MetaPartner>) Session["Customers/MetaPartnerWorkingSet"];
			if (_collection != null)
			{
				...
				Globals.AddStatusMessage("Meta Partner working set item removed.");
			}
			Session["Customers/MetaPartnerWorkingSet"] = _collection;
		}
		// ------------------------------------------------------------------------------
		...
		private void Page_PreRender(object sender, EventArgs e)
		{
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "WorkingMetaPartnerData.Page_PreRender"));
			}
			finally
			{
			}
