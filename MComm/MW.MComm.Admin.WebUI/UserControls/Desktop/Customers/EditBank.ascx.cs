...
{
	// ------------------------------------------------------------------------------
	/// <summary>Edit Bank is used to help manage Bank information.</summary>
	///
	/// File History:
	...
	/// <remarks></remarks>
	// ------------------------------------------------------------------------------
	public partial class EditBank  : Components.Desktop.BaseFormlUserControl, Controls.IHaveUploadableFiles
	{
		#region IHaveUploadableFiles
		public string GetFileRelativePath(Controls.FileUploader ctl)
		{
			if (ctl == txtPictureImageURL)
				return BankItem.PictureImageURL;
			else
				return null;
		}
		...
		/// </summary>
		public event EntityEditEventHandler BankAdded;
		/// <summary>
		/// Raised when "Remove" button is clicked in internal mode.
		/// Container handles this event and removes entity from collection.
		/// </summary>
		public event EntityEditEventHandler BankRemoved;
		#endregion Events
		#region Declarations
		#endregion Declarations
		#region Properties
		/// <summary>
		/// The Bank currently being edited by this control.
		/// Get accessor casts base._entity to Bank
		/// Set accessor used by parent control to set the item currently being edited
		/// </summary>
		public BLL.Customers.Bank BankItem
		{
			get
			{
				return (BLL.Customers.Bank) _entity;
			}
			set
			{
				_entity = value; // only set by container
				Session["BankItem"] = _entity;
			}
		}
		#endregion Properties
		#region Event Handlers
		// ------------------------------------------------------------------------------
		/// <summary>Load page for editing Bank, getting mode and parameters.</summary>
		///
		// ------------------------------------------------------------------------------
		private void Page_Load(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (IsPostBack == true)
				{
					...
					if (_entity == null)
					{
						_entity = (BLL.Customers.Bank)Session["BankItem"];
					}
				...
				listMetaPartnerPhoneData.Collection = BankItem.PhoneList;
				listMetaPartnerEmailData.Collection = BankItem.EmailList;
				listLocationData.Collection = BankItem.LocationList;
				listBankAccountData.Collection = BankItem.BankAccountList;
				listStoredValueAccountData.Collection = BankItem.StoredValueAccountList;
				listCreditAccountData.Collection = BankItem.CreditAccountList;
				listFundAccountData.Collection = BankItem.FundAccountList;
				listBillPayAccountData.Collection = BankItem.BillPayAccountList;
				listMetaTransferAccountData.Collection = BankItem.MetaTransferAccountList;
				listLoanAccountData.Collection = BankItem.LoanAccountList;
				listMetaPartnerCouponData.Collection = BankItem.MetaPartnerCouponList;
			}
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "EditBank.Page_Load"));
			}
			finally
			{
			}
		...
		private void btnDelete_Click(object sender, EventArgs e)
		{
			...
			try
			{
				...
				if (WorkflowMode == WorkflowMode.External)
				{
					...
					BLL.Customers.BankManager.DeleteOneBank(BankItem, performCascade);
					Globals.AddStatusMessage("Bank deleted.");
				}
				else
				{
					...
					if (BankRemoved != null)
					{
						BankRemoved(this, new EntityEditEventArgs(_entity));
					}
				}
			...
			{
				doRedirect = false;
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "EditBank.btnDelete_Click"));
			}
			finally
			{
			}
			if (doRedirect == true && WorkflowMode == MOD.Data.WorkflowMode.External)
			{
				...
				queryString.Add("BankMetaPartnerID", MOD.Data.DataHelper.GetString(MOD.Data.DefaultValue.Guid, ""));
		...
		private void btnNew_Click(object sender, EventArgs e)
		{
			...
			try
			{
				...
				SetAccessModeAndDisplay(MOD.Data.AccessMode.Add);
				Globals.AddStatusMessage("Add new Bank.");
			}
			catch (CoreUtility.CustomAppException ex)
			{
				doRedirect = false;
			...
			{
				doRedirect = false;
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "EditBank.btnNew_Click"));
			}
			finally
			{
			}
		...
		// ------------------------------------------------------------------------------
		/// <summary>Reset form for Bank.</summary>
		///
		// ------------------------------------------------------------------------------
		private void btnReset_Click(object sender, EventArgs e)
		{
			try
			{
				...
				Globals.AddStatusMessage("Bank reset.");
			}
			catch (CoreUtility.CustomAppException ex)
			{
				base.HandleError(ex);
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "EditBank.btnReset_Click"));
			}
			finally
			{
			}
		...
		private void btnSave_Click(object sender, EventArgs e)
		{
			...
			try
			{
				...
				if (isValid == true)
				{
					if (WorkflowMode == WorkflowMode.Internal)
					{
						...
						{
							if (BankAdded != null && (BankItem.MetaPartnerID != MOD.Data.DefaultValue.Guid || BankItem.BankCode != MOD.Data.DefaultValue.Int || BankItem.CreatedByUserID != MOD.Data.DefaultValue.Guid || BankItem.CreatedDate != MOD.Data.DefaultValue.DateTime || BankItem.LastModifiedByUserID != MOD.Data.DefaultValue.Guid || BankItem.LastModifiedDate != MOD.Data.DefaultValue.DateTime || BankItem.Timestamp != null))
							{
								BankAdded(this, new EntityEditEventArgs(_entity));
								// create new object and store it in _entity
								CreateNewEntity();
								Session["BankItem"] = _entity;
								this.sectionLinks.OnSection("Basics");
						...
						{
							Globals.AddStatusMessage("Bank updated.");
						}
					}
					else
					{
				...
				{
					Globals.AddErrorMessage(Page, "Bank validation failed.");
				}
			}
			catch (CoreUtility.CustomAppException ex)
			{
			...
			{
				doRedirect = false;
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "EditBank.btnSave_Click"));
			}
			finally
			{
			}
			...
			{
				NameValueCollection queryString = new NameValueCollection();
				queryString.Add("MetaPartnerID", BankItem.MetaPartnerID.ToString());
				queryString.Add("BankMetaPartnerID", BankItem.MetaPartnerID.ToString());
		...
		private void btnBasics_Click(object sender, EventArgs e)
		{
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "EditBank.btnBasics_Click"));
			}
			finally
			{
			}
		...
		private void Page_PreRender(object sender, EventArgs e)
		{
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "EditBank.Page_PreRender"));
			}
			finally
			{
			}
		...
		// ------------------------------------------------------------------------------
		/// <summary>
		/// Create a new Bank object reference
		/// </summary>
		/// <returns></returns>
		// ------------------------------------------------------------------------------
		protected void CreateNewEntity()
		{
			BLL.Customers.Bank vBank = new BLL.Customers.Bank();
			_entity = vBank;
		}
		// ------------------------------------------------------------------------------
		/// <summary>Update form from Bank information.</summary>
		///
		// ------------------------------------------------------------------------------
		private void CopyEntityToForm()
		{
			BLL.Customers.Bank vBank = BankItem;
			lblMetaPartnerID.Text = MOD.Data.DataHelper.GetString(vBank.MetaPartnerID, "");
			txtBankCode.Text = MOD.Data.EditHelper.GetInt(vBank.BankCode);
			txtServiceCharges.Text = MOD.Data.EditHelper.GetString(vBank.ServiceCharges);
			...
			try
			{
				if (AccessMode == MOD.Data.AccessMode.Add)
				{
					ddlMetaPartnerSubTypeCode.SelectedValue = ddlMetaPartnerSubTypeCode.Items.FindByText("Bank").Value;
				}
				else
				{
					ddlMetaPartnerSubTypeCode.SelectedValue = MOD.Data.DataHelper.GetString(vBank.MetaPartnerSubTypeCode, "");
				}
			}
			catch {}
			...
			try
			{
				...
				sLocale.LocaleCode = MOD.Data.DataHelper.GetInt(vBank.LocaleCode, MOD.Data.DefaultValue.Int);
			...
			try
			{
				...
				sCurrency.CurrencyCode = MOD.Data.DataHelper.GetInt(vBank.CurrencyCode, MOD.Data.DefaultValue.Int);
			...
			try
			{
				...
				sDateFormat.DateFormatCode = MOD.Data.DataHelper.GetInt(vBank.DateFormatCode, MOD.Data.DefaultValue.Int);
			...
			txtTaxCode.Text = MOD.Data.EditHelper.GetString(vBank.TaxCode);
			chkIsActive.Checked = MOD.Data.DataHelper.GetBool(vBank.IsActive, MOD.Data.DefaultValue.Bool);
			txtMetaPartnerName.Text = MOD.Data.EditHelper.GetString(vBank.MetaPartnerName);
		...
		// ------------------------------------------------------------------------------
		/// <summary>Set Bank information from the form.</summary>
		///
		// ------------------------------------------------------------------------------
		private void CopyFormToEntity()
		{
			...
			if (vBank == null)
			{
				throw CoreUtility.CustomAppException.WrapException(new Exception("Bank not found"), "EditBank.CopyFormToEntity()");
			}
			try
			{
				vBank.BankCode = MOD.Data.DataHelper.GetInt(txtBankCode.Text, MOD.Data.DefaultValue.Int);
			}
			catch {}
			vBank.ServiceCharges = EditHelper.EncodeStringForDB(MOD.Data.DataHelper.GetString(txtServiceCharges.Text, null));
			if (ddlMetaPartnerSubTypeCode.SelectedValue != ((int)MOD.Data.Lists.ListDefaultSelection.None).ToString())
			{
				...
				vBank.MetaPartnerSubTypeCode = MOD.Data.DataHelper.GetInt(sMetaPartnerSubType.MetaPartnerSubTypeCode, MOD.Data.DefaultValue.Int);
				sMetaPartnerSubType = null;
			}
			if (ddlLocaleCode.SelectedValue != ((int)MOD.Data.Lists.ListDefaultSelection.None).ToString())
			{
				BLL.Environments.Locale sLocale = new BLL.Environments.Locale(ddlLocaleCode.SelectedValue, false);
				vBank.LocaleCode = MOD.Data.DataHelper.GetInt(sLocale.LocaleCode, MOD.Data.DefaultValue.Int);
				sLocale = null;
			}
			if (ddlCurrencyCode.SelectedValue != ((int)MOD.Data.Lists.ListDefaultSelection.None).ToString())
			{
				BLL.Accounts.Currency sCurrency = new BLL.Accounts.Currency(ddlCurrencyCode.SelectedValue, false);
				vBank.CurrencyCode = MOD.Data.DataHelper.GetInt(sCurrency.CurrencyCode, MOD.Data.DefaultValue.Int);
				sCurrency = null;
			}
			if (ddlDateFormatCode.SelectedValue != ((int)MOD.Data.Lists.ListDefaultSelection.None).ToString())
			{
				BLL.Environments.DateFormat sDateFormat = new BLL.Environments.DateFormat(ddlDateFormatCode.SelectedValue, false);
				vBank.DateFormatCode = MOD.Data.DataHelper.GetInt(sDateFormat.DateFormatCode, MOD.Data.DefaultValue.Int);
				sDateFormat = null;
			}
			vBank.TaxCode = EditHelper.EncodeStringForDB(MOD.Data.DataHelper.GetString(txtTaxCode.Text, null));
			vBank.IsActive = chkIsActive.Checked;
			vBank.MetaPartnerName = EditHelper.EncodeStringForDB(MOD.Data.DataHelper.GetString(txtMetaPartnerName.Text, null));
			vBank.PictureImageURL = txtPictureImageURL.SaveFile();
		}
		// ------------------------------------------------------------------------------
		/// <summary>
		/// Load entity from database, store in _entity and Session["BankItem"]
		/// </summary>
		/// <returns></returns>
		// ------------------------------------------------------------------------------
		protected void LoadEntity()
		{
			...
			{
				_entity = BLL.Customers.BankManager.GetOneBank(metaPartnerID, Globals.getDataOptions("", "", true, true));
			}
		}
		// ------------------------------------------------------------------------------
		/// <summary>Update Bank information in the database.</summary>
		///
		// ------------------------------------------------------------------------------
		private void UpsertToDatabase(bool performCascade)
		{
			...
			if (AccessMode == MOD.Data.AccessMode.Add)
			{
				BLL.Customers.BankManager.AddOneBank(vBank, performCascade);
			}
			else if (AccessMode == MOD.Data.AccessMode.Edit)
			{
				BLL.Customers.BankManager.UpdateOneBank(vBank, performCascade);
			}
			BankItem.MetaPartnerID = vBank.MetaPartnerID;
		}
		// ------------------------------------------------------------------------------
