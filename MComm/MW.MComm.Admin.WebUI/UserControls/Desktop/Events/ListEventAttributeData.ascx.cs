...
{
	// ------------------------------------------------------------------------------
	/// <summary>List Event Attribute Data is used to help manage EventAttribute information.</summary>
	///
	/// File History:
	...
	/// <remarks></remarks>
	// ------------------------------------------------------------------------------
	public partial class ListEventAttributeData  : Components.Desktop.BaseListUserControl
	{
		#region Fields
		#endregion Fields
		#region Properties
		...
		public bool UseValidation
		{
			...
			set
			{
				...
				editEventAttribute.UseValidation = value;
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (_selectedIndex > -1)
				{
					this.editEventAttribute.EventAttributeItem = (BLL.Events.EventAttribute) Collection[_selectedIndex];
				}
				this.editEventAttribute.EventAttributeAdded += new EntityEditEventHandler(editEventAttribute_EventAttributeAdded);
				this.editEventAttribute.EventAttributeRemoved += new EntityEditEventHandler(editEventAttribute_EventAttributeRemoved);
			}
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "ListEventAttributeData.Page_Load"));
			}
			finally
			{
			}
		...
		// ------------------------------------------------------------------------------
		/// <summary>Item added event handler for Event Attribute.</summary>
		///
		// ------------------------------------------------------------------------------
		private void editEventAttribute_EventAttributeAdded(BaseFormlUserControl sender, EntityEditEventArgs e)
		{
		...
		// ------------------------------------------------------------------------------
		/// <summary>Item removed event handler for Event Attribute.</summary>
		///
		// ------------------------------------------------------------------------------
		private void editEventAttribute_EventAttributeRemoved(BaseFormlUserControl sender, EntityEditEventArgs e)
		{
			Globals.ClearMessages();
			_collection.Remove(e.Entity);
			Globals.AddStatusMessage("Event Attribute removed.");
		}
		// ------------------------------------------------------------------------------
		...
		private void dgPrimaryList_ItemDataBound(object sender, DataGridItemEventArgs e)
		{
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "ListEventAttributeData.dgPrimaryList_ItemDataBound"));
			}
			finally
			{
			}
		...
		protected override void LoadCollectionFromDatabase()
		{
			...
			{
				_collection = BLL.Events.EventAttributeManager.GetAllEventAttributeDataByBaseAttributeID(baseAttributeID, Globals.DBOptions, DataOptions, Globals.DebugLevel, Globals.CurrentUserID);
			}
			else
			{
				_collection = BLL.Events.EventAttributeManager.GetAllEventAttributeData(Globals.DBOptions, DataOptions, Globals.DebugLevel, Globals.CurrentUserID);
			}
		}
		// ------------------------------------------------------------------------------
		...
		public void dgPrimaryList_ItemCommand(object sender, EventArgs e)
		{
			...
			if (args.CommandName == "Edit" || args.CommandName == "Delete")
			{
				...
				{
					this.editEventAttribute.EventAttributeItem = item;
					this.editEventAttribute.AccessMode = MOD.Data.AccessMode.Edit;
				}
				else if (args.CommandName == "Delete")
				{
					Globals.ClearMessages();
					_collection.Remove(item);
					if (this.editEventAttribute.EventAttributeItem == item)
					{
						this.editEventAttribute.AccessMode = MOD.Data.AccessMode.Add;
					}
					Globals.AddStatusMessage("Event Attribute removed.");
				}
			}
		...
		private void Page_PreRender(object sender, EventArgs e)
		{
			try
			{
				...
				if (this.WorkflowMode == MOD.Data.WorkflowMode.Internal)
				{
					editEventAttribute.Visible = true;
					btnNew.Visible = false;
				}
				else
				{
					editEventAttribute.Visible = false;
					btnNew.Visible = true;
				}
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "ListEventAttributeData.Page_PreRender"));
			}
			finally
			{
			}
		...
		{
			Globals.ClearMessages();
			Response.Redirect(Page.ResolveUrl(string.Format("EditEventAttribute.aspx?mode=" + MOD.Data.AccessMode.Add.ToString() + "&BaseAttributeID=" + MOD.Data.DataHelper.GetString(MOD.Data.DefaultValue.Guid, "") + "")));
		}
