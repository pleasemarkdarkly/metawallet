...
{
	// ------------------------------------------------------------------------------
	/// <summary>Search Meta Transfer Account Data is used to help manage MetaTransferAccount information.</summary>
	///
	/// File History:
	...
	/// <remarks></remarks>
	// ------------------------------------------------------------------------------
	public partial class SearchMetaTransferAccountData  : Components.Desktop.BaseSearchUserControl
	{
		#region Fields
		#endregion Fields
		#region Properties
		...
		public bool UseValidation
		{
			...
			set
			{
				...
				editMetaTransferAccount.UseValidation = value;
			}
		}
		...
		// ------------------------------------------------------------------------------
		/// <summary>Load page for Meta Transfer Account search.</summary>
		///
		// ------------------------------------------------------------------------------
		private void Page_Load(object sender, System.EventArgs e)
		{
			try
			{
				workingMetaTransferAccountData.Visible = ProvideWorkingSet;
				base.OnLoad(); // base list control manages loading data
				if (WorkflowMode != WorkflowMode.Internal)
				{
					if (Session["SearchAccounts/MetaTransferAccountData"] != null)
					{
						_collection = (BLL.SortableList<BLL.Accounts.MetaTransferAccount>)Session["SearchAccounts/MetaTransferAccountData"];
					}
				}
				// Set current item into child control
				if (_selectedIndex > -1)
				{
					this.editMetaTransferAccount.MetaTransferAccountItem = (BLL.Accounts.MetaTransferAccount) Collection[_selectedIndex];
				}
				this.editMetaTransferAccount.MetaTransferAccountAdded += new EntityEditEventHandler(editMetaTransferAccount_MetaTransferAccountAdded);
				this.editMetaTransferAccount.MetaTransferAccountRemoved += new EntityEditEventHandler(editMetaTransferAccount_MetaTransferAccountRemoved);
			}
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "SearchMetaTransferAccountData.Page_Load"));
			}
			finally
			{
			}
		...
		// ------------------------------------------------------------------------------
		/// <summary>Item added event handler for Meta Transfer Account.</summary>
		///
		// ------------------------------------------------------------------------------
		private void editMetaTransferAccount_MetaTransferAccountAdded(BaseFormlUserControl sender, EntityEditEventArgs e)
		{
			if (_collection.Contains(e.Entity))
			{
				Globals.AddErrorMessage(this.Page, "Collection already contains this Meta Transfer Account", new object[0]);
			}
			else
			{
		...
		// ------------------------------------------------------------------------------
		/// <summary>Item removed event handler for Meta Transfer Account.</summary>
		///
		// ------------------------------------------------------------------------------
		private void editMetaTransferAccount_MetaTransferAccountRemoved(BaseFormlUserControl sender, EntityEditEventArgs e)
		{
			_collection.Add(e.Entity);
		}
		// ------------------------------------------------------------------------------
		...
		private void dgPrimaryList_ItemDataBound(object sender, DataGridItemEventArgs e)
		{
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "SearchMetaTransferAccountData.dgPrimaryList_ItemDataBound"));
			}
			finally
			{
			}
		...
		// ------------------------------------------------------------------------------
		/// <summary>Set Meta Transfer Account information from the ui.</summary>
		///
		// ------------------------------------------------------------------------------
		protected override void SetDataFromForm()
		{
			Session["MetaTransferAccountNumber"] = GetStringFromTextBox(txtMetaTransferAccountNumber);
			Session["PaymentInstitutionCode"] = GetSelectedValueFromDropDownList(ddlPaymentInstitutionCode);
		...
		protected override void LoadCollectionFromDatabase()
		{
			try
			{
				...
				string metaTransferAccountNumber = MOD.Data.SearchHelper.GetString(Session["MetaTransferAccountNumber"]);
				int? paymentInstitutionCode = MOD.Data.SearchHelper.GetInt(Session["PaymentInstitutionCode"]);
				...
				{
					_collection = BLL.Accounts.MetaTransferAccountManager.GetManyMetaTransferAccountDataByCriteria(metaTransferAccountNumber, paymentInstitutionCode, accountName, currencyCode, lastModifiedDateStart, lastModifiedDateEnd, out totalRecords, out maximumListSizeExceeded, this.DataOptions);
					Session["SearchAccounts/MetaTransferAccountData"] = _collection;
		...
		public void dgPrimaryList_ItemCommand(object sender, EventArgs e)
		{
			...
			if (args.CommandName == "Add" || args.CommandName == "Edit" || args.CommandName == "Delete")
			{
				...
				{
					workingMetaTransferAccountData.AddItem(item);
				}
				else if (args.CommandName == "Edit")
				{
					this.editMetaTransferAccount.MetaTransferAccountItem = item;
				}
		...
		private void Page_PreRender(object sender, EventArgs e)
		{
			try
			{
				...
				if (IsPostBack == false)
				{
					txtMetaTransferAccountNumber.Text = MOD.Data.DataHelper.GetString(Session["MetaTransferAccountNumber"], "");
					if (ddlPaymentInstitutionCode.Visible == true)
					{
						Components.Lists.DataBind(ddlPaymentInstitutionCode, BLL.Accounts.PaymentInstitutionManager.GetAllPaymentInstitutionData(Globals.DBOptions, Globals.getDataOptions("PaymentInstitutionName", "", true, true), Globals.DebugLevel, Globals.CurrentUserID), "PrimaryKey", "PrimaryName", true, false);
						ddlPaymentInstitutionCode.ClearSelection();
						try
						{
							ddlPaymentInstitutionCode.SelectedValue = MOD.Data.DataHelper.GetString(Session["PaymentInstitutionCode"], "0");
						}
						catch {}
					}
				...
				if (this.WorkflowMode == MOD.Data.WorkflowMode.Internal)
				{
					editMetaTransferAccount.Visible = true;
					btnNew.Visible = false;
				}
				else
				{
					editMetaTransferAccount.Visible = false;
					btnNew.Visible = true;
				}
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "SearchMetaTransferAccountData.Page_PreRender"));
			}
			finally
			{
			}
		...
		// ------------------------------------------------------------------------------
		/// <summary>Request to create a new Meta Transfer Account.</summary>
		///
		// ------------------------------------------------------------------------------
		private void btnNew_Click(object sender, EventArgs e)
		{
			Globals.ClearMessages();
			Response.Redirect(Page.ResolveUrl(string.Format("EditMetaTransferAccount.aspx?mode=" + MOD.Data.AccessMode.Add.ToString() + "&AccountID=" + MOD.Data.DataHelper.GetString(MOD.Data.DefaultValue.Guid, "") + "&MetaTransferAccountAccountID=" + MOD.Data.DataHelper.GetString(MOD.Data.DefaultValue.Guid, "") + "")));
		}
		
		// ------------------------------------------------------------------------------
		/// <summary>Clear search form for Meta Transfer Account.</summary>
		///
		// ------------------------------------------------------------------------------
		private void btnClear_Click(object sender, EventArgs e)
		{
			...
			this.TotalRecords = 0;
			_collection = null;
			txtMetaTransferAccountNumber.Text = "";
			ddlPaymentInstitutionCode.ClearSelection();
			txtAccountName.Text = "";
			ddlCurrencyCode.ClearSelection();
		...
		// ------------------------------------------------------------------------------
		/// <summary>Perform search for Meta Transfer Account items.</summary>
		///
		// ------------------------------------------------------------------------------
		private void btnSearch_Click(object sender, EventArgs e)
		{
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "SearchMetaTransferAccountData.btnSearch_Click"));
			}
			finally
			{
			}
