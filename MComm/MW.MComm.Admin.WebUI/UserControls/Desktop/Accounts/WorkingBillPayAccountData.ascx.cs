...
{
	// ------------------------------------------------------------------------------
	/// <summary>Working Bill Pay Account Data is used to help manage BillPayAccount information.</summary>
	///
	/// File History:
	...
	/// <remarks></remarks>
	// ------------------------------------------------------------------------------
	public partial class WorkingBillPayAccountData  : Components.Desktop.BaseListUserControl
	{
		#region Fields
		#endregion Fields
		#region Properties
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "WorkingBillPayAccountData.Page_Load"));
			}
			finally
			{
			}
		...
		private void dgPrimaryList_ItemDataBound(object sender, DataGridItemEventArgs e)
		{
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "WorkingBillPayAccountData.dgPrimaryList_ItemDataBound"));
			}
			finally
			{
			}
		...
		{
			_collection = (BLL.SortableList<BLL.Accounts.BillPayAccount>) Session["Accounts/BillPayAccountWorkingSet"];
			if (_collection != null)
			{
				((BLL.SortableList<BLL.Accounts.BillPayAccount>)_collection).Sort(SortColumn, SortDirection);
		...
		public void dgPrimaryList_ItemCommand(object sender, EventArgs e)
		{
			...
			if (_collection != null)
			{
				...
				else if (args.CommandName == "Select")
				{
					int hashCode = int.Parse(args.CommandArgument.ToString());
					BLL.Accounts.BillPayAccount item = ((BLL.SortableList<BLL.Accounts.BillPayAccount>)_collection).Find(hashCode);
					Session["Accounts/BillPayAccountWorkingSetItem"] = item.PrimaryIndex;
				}
			}
		}
		// ------------------------------------------------------------------------------
		...
		///
		// ------------------------------------------------------------------------------
		public void AddItem(BLL.Accounts.BillPayAccount item)
		{
			_collection = (BLL.SortableList<BLL.Accounts.BillPayAccount>) Session["Accounts/BillPayAccountWorkingSet"];
			if (_collection == null)
			{
				_collection = new BLL.SortableList<BLL.Accounts.BillPayAccount>();
			}
			...
			else
			{
				...
				TotalRecords = _collection.Count;
				Session["Accounts/BillPayAccountWorkingSetItem"] = item.PrimaryIndex;
				Globals.AddStatusMessage("Bill Pay Account working set item added.");
			}
			Session["Accounts/BillPayAccountWorkingSet"] = _collection;
		}
		// ------------------------------------------------------------------------------
		/// <summary>Item command event handler for Bill Pay Account list.</summary>
		///
		// ------------------------------------------------------------------------------
		public void RemoveItem(BLL.Accounts.BillPayAccount item)
		{
			Globals.ClearMessages();
			_collection = (BLL.SortableList<BLL.Accounts.BillPayAccount>) Session["Accounts/BillPayAccountWorkingSet"];
			if (_collection != null)
			{
				...
				Globals.AddStatusMessage("Bill Pay Account working set item removed.");
			}
			Session["Accounts/BillPayAccountWorkingSet"] = _collection;
		}
		// ------------------------------------------------------------------------------
		...
		private void Page_PreRender(object sender, EventArgs e)
		{
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "WorkingBillPayAccountData.Page_PreRender"));
			}
			finally
			{
			}
