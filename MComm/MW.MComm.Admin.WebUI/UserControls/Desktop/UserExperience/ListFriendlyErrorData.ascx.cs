...
{
	// ------------------------------------------------------------------------------
	/// <summary>List Friendly Error Data is used to help manage FriendlyError information.</summary>
	///
	/// File History:
	...
	/// <remarks></remarks>
	// ------------------------------------------------------------------------------
	public partial class ListFriendlyErrorData  : Components.Desktop.BaseListUserControl
	{
		#region Fields
		#endregion Fields
		#region Properties
		...
		public bool UseValidation
		{
			...
			set
			{
				...
				editFriendlyError.UseValidation = value;
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (_selectedIndex > -1)
				{
					this.editFriendlyError.FriendlyErrorItem = (BLL.UserExperience.FriendlyError) Collection[_selectedIndex];
				}
				this.editFriendlyError.FriendlyErrorAdded += new EntityEditEventHandler(editFriendlyError_FriendlyErrorAdded);
				this.editFriendlyError.FriendlyErrorRemoved += new EntityEditEventHandler(editFriendlyError_FriendlyErrorRemoved);
			}
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "ListFriendlyErrorData.Page_Load"));
			}
			finally
			{
			}
		...
		// ------------------------------------------------------------------------------
		/// <summary>Item added event handler for Friendly Error.</summary>
		///
		// ------------------------------------------------------------------------------
		private void editFriendlyError_FriendlyErrorAdded(BaseFormlUserControl sender, EntityEditEventArgs e)
		{
			Globals.ClearMessages();
			if (_collection.Contains(e.Entity))
			{
				Globals.AddErrorMessage(Page, "Collection already contains this Friendly Error");
			}
			else
			{
		...
		// ------------------------------------------------------------------------------
		/// <summary>Item removed event handler for Friendly Error.</summary>
		///
		// ------------------------------------------------------------------------------
		private void editFriendlyError_FriendlyErrorRemoved(BaseFormlUserControl sender, EntityEditEventArgs e)
		{
			Globals.ClearMessages();
			_collection.Remove(e.Entity);
			Globals.AddStatusMessage("Friendly Error removed.");
		}
		// ------------------------------------------------------------------------------
		...
		private void dgPrimaryList_ItemDataBound(object sender, DataGridItemEventArgs e)
		{
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "ListFriendlyErrorData.dgPrimaryList_ItemDataBound"));
			}
			finally
			{
			}
		...
		protected override void LoadCollectionFromDatabase()
		{
			...
			if (errorNumber!= MOD.Data.DefaultValue.Int)
			{
				_collection = BLL.UserExperience.FriendlyErrorManager.GetAllFriendlyErrorDataByErrorNumber(errorNumber, Globals.DBOptions, DataOptions, Globals.DebugLevel, Globals.CurrentUserID);
			}
			else if (localeCode!= MOD.Data.DefaultValue.Int)
			{
				_collection = BLL.UserExperience.FriendlyErrorManager.GetAllFriendlyErrorDataByLocaleCode(localeCode, Globals.DBOptions, DataOptions, Globals.DebugLevel, Globals.CurrentUserID);
			}
			else
			{
				_collection = BLL.UserExperience.FriendlyErrorManager.GetAllFriendlyErrorData(Globals.DBOptions, DataOptions, Globals.DebugLevel, Globals.CurrentUserID);
			}
		}
		// ------------------------------------------------------------------------------
		...
		public void dgPrimaryList_ItemCommand(object sender, EventArgs e)
		{
			...
			if (args.CommandName == "Edit" || args.CommandName == "Delete")
			{
				...
				{
					this.editFriendlyError.FriendlyErrorItem = item;
					this.editFriendlyError.AccessMode = MOD.Data.AccessMode.Edit;
				}
				else if (args.CommandName == "Delete")
				{
					Globals.ClearMessages();
					_collection.Remove(item);
					if (this.editFriendlyError.FriendlyErrorItem == item)
					{
						this.editFriendlyError.AccessMode = MOD.Data.AccessMode.Add;
					}
					Globals.AddStatusMessage("Friendly Error removed.");
				}
			}
		...
		private void Page_PreRender(object sender, EventArgs e)
		{
			try
			{
				...
				if (this.WorkflowMode == MOD.Data.WorkflowMode.Internal)
				{
					editFriendlyError.Visible = true;
					btnNew.Visible = false;
				}
				else
				{
					editFriendlyError.Visible = false;
					btnNew.Visible = true;
				}
			...
			{
				base.HandleError(CoreUtility.CustomAppException.WrapException(ex, "ListFriendlyErrorData.Page_PreRender"));
			}
			finally
			{
			}
		...
		{
			Globals.ClearMessages();
			Response.Redirect(Page.ResolveUrl(string.Format("EditFriendlyError.aspx?mode=" + MOD.Data.AccessMode.Add.ToString() + "&ErrorNumber=" + MOD.Data.DataHelper.GetString(MOD.Data.DefaultValue.Int, "") + "&LocaleCode=" + MOD.Data.DataHelper.GetString(MOD.Data.DefaultValue.Int, "") + "")));
		}
